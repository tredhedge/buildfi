...
<div style="${sectionStyle}">
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;justify-content:center;margin:16px 0">
    ...
    ${kpiBox(
      ...
      successPct >= 80 ? "#1a7a4c" : successPct >= 60 ? "#c4944a" : "#b91c1c"
    )}
    ${kpiBox(
  ...
  ${ai.summaryAI ? `<div style="${bodyStyle};margin-top:12px;padding:12px 16px;background:#faf8f4;border-left:3px solid #c4944a;border-radius:0 6px 6px 0">${escapeHTML(ai.summaryAI)}</div>` : ""}
</div>
...
<div style="${sectionStyle}">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:13px;margin-top:8px">
    <thead>
      <tr style="border-bottom:2px solid #c4944a">
    ...
    <tbody>
      ${comparison.history.map((h, i) => `
        <tr style="border-bottom:1px solid #e8e4db;${i === 0 ? "font-weight:700;background:#faf8f4" : ""}">
          ...
          <td style="text-align:center;padding:8px;color:${Math.round(h.successRate * 100) >= 80 ? "#1a7a4c" : Math.round(h.successRate * 100) >= 60 ? "#c4944a" : "#b91c1c"}">${Math.round(h.successRate * 100)}%</td>
