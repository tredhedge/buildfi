...
function renderReport_v5(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  h += '<div style="background:linear-gradient(135deg,#4680C0,#6B9CD4);border-radius:10px;padding:20px;color:#fff;text-align:center">'
    + '<div style="font-size:14px;font-weight:800;margin-bottom:8px">' + t("Bilan 360","Snapshot 360") + '</div>'
...
function renderReport_v6(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  if (priorities.length > 0) {
    dcLever = priorities[0].name + (priorities[0].rate ? ' (' + priorities[0].rate + ')' : '');
  } else if (D.savingsRate < 10 && D.age < 55) {
    dcLever = t(
      "Augmenter les cotisations mensuelles pourrait am\u00e9liorer la trajectoire de votre plan.",
      "Increasing monthly contributions could improve your plan\u2019s trajectory."
    );
  } else if (D.tfsa < D.rrsp * 0.3 && D.totalSavings > 10000) {
    dcLever = t(
      "Le compte d\u2019\u00e9pargne libre d\u2019imp\u00f4t (" + (D.prov === "QC" ? "C\u00c9LI" : "TFSA") + ") appara\u00eet sous-utilis\u00e9 par rapport au r\u00e9gime enregistr\u00e9.",
      "The Tax-Free Savings Account appears underused relative to the registered plan."
    );
  } else {
    dcLever = t(
  ...
  h += '<div style="font-family:var(--mono);font-size:36px;font-weight:900;color:' + gi.c + ';line-height:1">' + D.grade + '</div>';
  h += '<div><div style="font-size:15px;font-weight:700;color:' + gi.c + '">' + gi.g + '</div>';
  ...
  h += '<div style="font-size:13px;color:var(--tm);line-height:1.7">' + dcInterpretation + '</div>';
  h += '</div>';
  ...
  if (gi.g === 'F') actionHintText = t("Votre plan n\u00e9cessite des ajustements importants. <strong>3 leviers concrets</strong> sont identifi\u00e9s en section 7.", "Your plan needs significant adjustments. <strong>3 concrete levers</strong> are identified in section 7.");
  ...
  else if (gi.g === 'B' || gi.g === 'B+') actionHintText = t("Plan solide. L\u2019optimisation fiscale (section 8) serait le levier le plus efficace \u00e0 ce stade.", "Solid plan. Tax optimization (section 8) would likely be the most effective lever at this stage.");
    ...
    else if (D.coveragePct >= 70) actionHintText = t("Plan robuste. Les revenus gouvernementaux et de pension couvrent l\u2019essentiel des besoins \u2014 l\u2019\u00e9pargne personnelle pourrait servir de coussin suppl\u00e9mentaire.", "Solid plan. Government and pension income covers most of your needs \u2014 personal savings could serve as an additional buffer.");
  ...
  h += '<div style="background:linear-gradient(135deg,#1A1208 0%,#2D2010 100%);color:#fff;border-radius:14px;padding:24px 28px;margin:8px 0 12px;">'
    ...
    + t("Vous avez " + D.age + " ans. Vous visez la retraite \u00e0 " + D.retAge + " ans avec un revenu de " + f$(D.retSpM) + " par mois. Votre \u00e9pargne totalise " + f$(D.totalSavings) + ". Les revenus garantis (" + qppLabel + " + PSV" + (D.hasPension ? " + pension" : "") + ") pourraient couvrir " + f$(D.govMonthly) + " par mois" + (mirrorGapEss > 0 ? " \u2014 le portefeuille devrait combler " + f$(mirrorGapEss) + " par mois" : "") + ".",
       "You are " + D.age + " years old. You target retirement at " + D.retAge + " with an income of " + f$(D.retSpM) + " per month. Your savings total " + f$(D.totalSavings) + ". Guaranteed income (" + qppLabel + " + OAS" + (D.hasPension ? " + pension" : "") + ") could cover " + f$(D.govMonthly) + " per month" + (mirrorGapEss > 0 ? " \u2014 the portfolio would need to fill " + f$(mirrorGapEss) + " per month" : "") + ".")
    + '</div></div>';
  ...
  h += '</div>';
  if (quizQ.couple === "yes") {
    h += '<div class="co coam" style="margin-top:10px">' + t("<strong>Note :</strong> Ce bilan mod\u00e9lise une seule personne. Pour une analyse de couple compl\u00e8te (revenus combin\u00e9s, fractionnement du revenu, strat\u00e9gies conjointes), le Bilan 360 ou Expert est disponible.", "<strong>Note:</strong> This assessment models a single person. For a full couple analysis (combined income, income splitting, joint strategies), the Snapshot 360 or Expert tier is available.") + '</div>';
  }
  h += '</div>';
  ...
  h += '<a href="https://www.buildfi.ca/checkout?plan=intermediaire&from=rapport&grade=' + encodeURIComponent(D.grade) + '&succ=' + D.successPct + '" target="_blank" rel="noopener" style="display:block;text-align:center;padding:18px;background:linear-gradient(135deg,var(--gd),var(--gl));color:#fff;border-radius:var(--rad);text-decoration:none;font-weight:700;font-size:15px">' + t("Interm\u00e9diaire \u2014 59 $","Intermediate \u2014 $59") + '<div style="font-size:11px;font-weight:400;opacity:.8;margin-top:4px">' + t("16 sections \u00b7 analyse de d\u00e9caissement \u00b7 analyse " + qppLabel,"16 sections \u00b7 decumulation analysis \u00b7 " + qppLabel + " analysis") + '</div></a>';
  h += '<a href="https://www.buildfi.ca/checkout?plan=expert&from=rapport&grade=' + encodeURIComponent(D.grade) + '&succ=' + D.successPct + '" target="_blank" rel="noopener" style="display:block;text-align:center;padding:18px;border:2px solid var(--bd);color:var(--tx);border-radius:var(--rad);text-decoration:none;font-weight:700;font-size:15px">' + t("Expert \u2014 129 $","Expert \u2014 $129") + '<div style="font-size:11px;font-weight:400;color:var(--tm);margin-top:4px">' + t("28 sections \u00b7 immobilier \u00b7 couple \u00b7 incorpor\u00e9","28 sections \u00b7 real estate \u00b7 couple \u00b7 incorporated") + '</div></a>';
  h += '</div>';
  ...
  if (D.snapshot5yr && D.snapshot5yr.length) {
    ...
    h += '</tbody></table></div>';
    h += '<div class="ex">' + t("Le Bilan 360 offre cette projection pour chaque ann\u00e9e avec des percentiles d\u00e9taill\u00e9s.", "The Snapshot 360 provides this for every year with detailed percentiles.") + '</div></div>';
  }
  ...
  if (D.heuristics && D.heuristics.length) {
    ...
    h += '<div style="font-size:14px;color:var(--ts);line-height:1.7;margin-bottom:16px">' + t("Le Bilan utilise des heuristiques pour compl\u00e9ter les donn\u00e9es que vous n\u2019avez pas fournies. Le Bilan 360 permet de pr\u00e9ciser chacun de ces param\u00e8tres.", "The Snapshot uses heuristics to fill in data you did not provide. The Snapshot 360 lets you specify each parameter.") + '</div>';
    h += '<div class="c">';
  ...
  (function() {
    ...
    h += '<div style="font-size:11px;color:var(--tm);margin-top:10px;font-style:italic">'
      + t("Ces r\u00e9ponses ont orient\u00e9 la s\u00e9lection des observations et le ton de l\u2019analyse.", "These responses shaped the selection of observations and the tone of the analysis.")
      + '</div></div>';
  })();
  ...
  h += '<a href="https://www.buildfi.ca/outils/dettes" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:14px;text-decoration:none;background:#fff;border:1px solid rgba(196,154,26,.22);border-radius:9px;padding:13px 16px;margin-bottom:10px;transition:border-color .15s,box-shadow .15s" onmouseover="this.style.borderColor=\'rgba(196,154,26,.55)\';this.style.boxShadow=\'0 2px 8px rgba(196,154,26,.12)\'" onmouseout="this.style.borderColor=\'rgba(196,154,26,.22)\';this.style.boxShadow=\'none\'">';
  h += '<span style="font-size:24px;flex-shrink:0">\ud83e\uddee</span>';
  ...
  h += '<a href="https://www.buildfi.ca/' + t("guide-101-les-bases-de-vos-finances.pdf","guide-101-your-financial-basics.pdf") + '" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:14px;text-decoration:none;background:#fff;border:1px solid rgba(196,154,26,.22);border-radius:9px;padding:13px 16px;transition:border-color .15s,box-shadow .15s" onmouseover="this.style.borderColor=\'rgba(196,154,26,.55)\';this.style.boxShadow=\'0 2px 8px rgba(196,154,26,.12)\'" onmouseout="this.style.borderColor=\'rgba(196,154,26,.22)\';this.style.boxShadow=\'none\'">';
  h += '<span style="font-size:24px;flex-shrink:0">\ud83d\udcd8</span>';
...
':root{' +
'  --gd:#c49a1a;--gl:#D4A85A;--gbg:rgba(196,154,26,.07);' +
