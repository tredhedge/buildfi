...
function buildWhatIf(p, mc, D, fr) {
  try {
    ...
    var strongPlan = D.successPct >= 85;
    ...
    if (strongPlan) {
      // Strong plans: stress-test resilience instead of showing improvements
      // 1. Spend 500$/mo more
      var ps1 = Object.assign({}, p); ps1.retSpM = ps1.retSpM + 500;
      var mcs1 = quickSim(ps1);
      if (mcs1) { var pcts1 = Math.round(mcs1.succ * 100); scenarios.push({type:"down", title:fr?"D\u00e9penser 500 $/mois de plus":"Spend $500/mo more", detail:D.retSpM+" $ \u2192 "+ps1.retSpM+" $/"+(fr?"mois":"mo"), newPct:pcts1, newGrade:gradeFor(pcts1)}); }
      // 2. Retire 2 years earlier
      var earlierAge = Math.max((p.age || D.age) + 1, p.retAge - 2);
      if (earlierAge < p.retAge) {
        var ps2 = Object.assign({}, p); ps2.retAge = earlierAge;
        var mcs2 = quickSim(ps2);
        if (mcs2) { var pcts2 = Math.round(mcs2.succ * 100); scenarios.push({type:"down", title:fr?"Prendre la retraite 2 ans plus t\u00f4t":"Retire 2 years earlier", detail:D.retAge+" "+(fr?"ans":"yr")+" \u2192 "+ps2.retAge+" "+(fr?"ans":"yr"), newPct:pcts2, newGrade:gradeFor(pcts2)}); }
      }
      // 3. Reduce contributions by 200$/mo
      if (annualContrib >= 2400) {
        var ps3 = Object.assign({}, p); ps3.rrspC = Math.max(0, (ps3.rrspC||0) - 2400);
        var mcs3 = quickSim(ps3);
        if (mcs3) { var pcts3 = Math.round(mcs3.succ * 100); var oldCm = Math.round(annualContrib / 12); scenarios.push({type:"down", title:fr?"R\u00e9duire les cotisations de 200 $/mois":"Reduce contributions by $200/mo", detail:oldCm+" $ \u2192 "+(oldCm-200)+" $/"+(fr?"mois":"mo"), newPct:pcts3, newGrade:gradeFor(pcts3)}); }
      }
    } else {
      // Weaker plans: show improvement scenarios
      ...
      var mc3 = quickSim(p3);
      if (mc3) { var pct3 = Math.round(mc3.succ * 100); var oldC = Math.round(annualContrib/12); scenarios.push({type:"up", title:fr?"Augmenter les cotisations de 200 $/mois":"Increase contributions by $200/mo", detail:oldC+" $ \u2192 "+(oldC+200)+" $/"+(fr?"mois":"mo"), newPct:pct3, newGrade:gradeFor(pct3)}); }
    }
    ...
    // 5. Combined scenario for weak plans (all 3 improvement levers at once)
    if (!strongPlan) {
      var pC = Object.assign({}, p);
      pC.retSpM = Math.max(0, pC.retSpM - 500);
      pC.retAge = pC.retAge + 2;
      pC.rrspC = (pC.rrspC||0) + 2400;
      var mcC = quickSim(pC);
      if (mcC) {
        var pctC = Math.round(mcC.succ * 100);
        scenarios.push({type:"combined", title:fr?"Les 3 leviers combin\u00e9s":"All 3 levers combined", detail:fr?"-500 $/mois + 2 ans + 200 $/mois":"−$500/mo + 2 yrs + $200/mo", newPct:pctC, newGrade:gradeFor(pctC)});
      }
    }
    return scenarios;
  } catch(e) {
...
function buildSnapshot5yr(mc, D) {
  try {
    ...
    unique.forEach(function(targetAge) {
      ...
      var bal = Math.max(0, Math.round(((best.aRR||0) + (best.aTF||0) + (best.aNR||0) + (best.aDC||0) + (best.aPE||0) + (best.aPM||0) + (best.aCRR||0) + (best.aCTF||0) + (best.aCNR||0) + (best.aLIRA||0) + (best.aCLIRA||0)) / deflator));
      var revGov = Math.round(((best.rrq||0) + (best.psv||0) + (best.pen||0)) / deflator);
      var dep = Math.round((best.spend||0) / deflator);
      var desiredFromSavings = Math.max(0, dep - revGov);
      var revEp = bal > 0 ? desiredFromSavings : 0;
      var shortfall = bal <= 0 ? desiredFromSavings : 0;
      rows.push({age:targetAge, bal:bal, revGov:revGov, revEp:revEp, dep:dep, shortfall:shortfall});
    });
    return rows;
  } catch(e) {
...
function extractReportData(mc, p) {
  ...
  // Grade scale — canonical 8-level (from report-shared.ts)
...
function buildAIPrompt(D, p, fr, quiz) {
  ...
  var cplSlot = q.couple === "yes" ? "Client has a partner but Essentiel models ONE person. Mention this once in snapshot_intro." : "";
  ...
  if (rp.debtBal > 0 && signals.debtDragOverSavings)
    ...
    obs2Instr = "Solo modeling note: this analysis models one person. A couple's combined income, shared housing, and survivor benefits could significantly change the picture.";
  ...
  var sys = "You narrate buildfi.ca Essentiel retirement reports.\n"
    ...
    + "- " + (profile.literacy === "advanced" ? "Grade 12 reading level. Precise financial vocabulary OK (withdrawal rate, coverage ratio, real return). Still keep sentences short and declarative." : "Grade 10 reading level. Short sentences. No jargon.") + " Never use acronyms — write in full: " + gP + ", " + oN + ".\n"
    + "- JARGON BAN: Never use 'résiduel', 'patrimoine résiduel', 'percentile', 'taux de retrait', 'décaissement', 'véhicule fiscal', 'portefeuille'. Instead say: 'ce qui resterait', 'l'argent qui resterait', 'dans les scénarios les plus/moins favorables', 'frais de gestion de X %', 'rythme de retrait', 'retraits', 'type de compte', 'épargne/placements'. Always prefer the word a 15-year-old would use.\n"
    ...
    + (fr ? "- 'niveau de dépenses visé', 'objectif plus exigeant', 'marge plus limitée', 'structure plus défensive', 'plus exposé aux actions', 'plus contraint'\n"
         : "- 'target spending level', 'more demanding objective', 'tighter margin', 'more defensive structure', 'growth-oriented allocation', 'more constrained'\n")
    + "\n=== VOICE ===\n"
    + voiceInstr + "\n"
    ...
    + "\n=== DENSITY (ESSENTIAL TIER) ===\n"
    + "Maximum 2 sentences per slot. One idea per sentence. Lead with the most reassuring or most striking data point. Define any financial term inline. No stacking of numbers.\n"
    + "\n=== OBSERVATION ORTHOGONALITY ===\n"
    + "obs_1, obs_2, obs_3 must each cover a DIFFERENT analytical dimension:\n"
    + "- Financial (rates, amounts, costs)\n"
    + "- Temporal (timeline, sequence, compounding runway)\n"
    + "- Behavioral (spending, flexibility, habits)\n"
...
// ============================================================
// AI PROMPT BUILDER — OPUS (15-slot mega-prompt for Opus 4.6)
// Separate from buildAIPrompt() to allow independent iteration.
// Selected via ANTHROPIC_MODEL env var in webhook.
// ============================================================
function buildAIPromptOpus(D, p, fr, quiz, whatIfResults) {
  var q = p._quiz || {}, rp = p._report || {};
  var isQC = p.prov === "QC";
  var gP = fr ? (isQC ? "Régime de rentes du Québec" : "Régime de pensions du Canada") : (isQC ? "Quebec Pension Plan" : "Canada Pension Plan");
  var oN = fr ? "Pension de la Sécurité de la vieillesse" : "Old Age Security";
  ...
  var voiceMatrix = {
    ...
    "warm+advanced": "Write like a calm, experienced colleague. Technical precision is welcome but always wrapped in reassurance. Acknowledge sophistication. Use ratios and rates directly, but frame every risk as manageable variability.",
    "balanced+basic": "Write like a clear, friendly explainer. Simple sentences. One number per sentence max. Always follow a number with what it means in plain words. Avoid stacking data points.",
    "balanced+intermediate": "Write like a sharp financial journalist. Mix data with narrative. Each slot should feel like a mini-paragraph that tells a story: here's the number, here's what it means for your life, here's the nuance.",
    ...
    "data-forward+advanced": "Write like a portfolio analyst's concise briefing. Maximum density. Reference withdrawal rates, coverage ratios, real vs nominal, fee drag. Every sentence must carry information. Zero fluff."
  };
  var voiceKey = plan.tone + "+" + profile.literacy;
  var voiceInstr = voiceMatrix[voiceKey] || voiceMatrix["balanced+intermediate"];
  ...
  // ── 3. NARRATIVE ARC ──
  var arcMap = {
    security: "NARRATIVE ARC: This person seeks safety. Open with what is solid and protected. Frame every number through 'what is secure.' Close each slot with a stabilizing thought.",
    growth: "NARRATIVE ARC: This person is building momentum. Open with trajectory and compounding power. Frame numbers as progress markers. Close each slot with forward-looking potential.",
    optimization: "NARRATIVE ARC: Already in good shape, wants to fine-tune. Open with efficiency metrics. Skip generic reassurance. Close each slot with a specific efficiency insight.",
    "catch-up": "NARRATIVE ARC: Started late or faces a gap. Be honest but never defeatist. Open with what they CAN still influence. Frame numbers as starting points, not verdicts."
  };
  var narrativeArc = arcMap[profile.narrativeTheme] || arcMap.growth;
  ...
  var worryCombos = [];
  if (worrySet.runout && worrySet.health)
    worryCombos.push("WORRY PATTERN [existential]: Client fears running out AND health costs. Reinforce safety nets. Longevity slots get extra depth.");
  if (worrySet.tax && worrySet.legacy)
    worryCombos.push("WORRY PATTERN [estate-optimizer]: Tax efficiency AND legacy concerns. tax_fees_insight explores bracket advantage. succession_note must feel earned.");
  if (worrySet.market && worrySet.inflation)
    worryCombos.push("WORRY PATTERN [investor-anxiety]: Market drops AND purchasing power fears. Frame all amounts as inflation-adjusted (they are).");
  if (worrySet.runout && worrySet.inflation && worrySet.health)
    worryCombos.push("WORRY PATTERN [maximum-anxiety]: 3+ fears active. Lead EVERY slot with the reassuring data point first.");
  if (worries.length === 0)
    worryCombos.push("WORRY PATTERN [confident/disengaged]: No worries selected. Mirror savings habits or situation positives in snapshot_intro.");
  var worryBlock = worryCombos.join("\n");

  var worryMap = {runout:"EXPAND projection slots. Emphasize median vs pessimistic scenarios.",tax:"EXPAND tax_fees_insight. Compare effective rates in detail.",inflation:"All amounts are already inflation-adjusted — state this explicitly once.",health:"Acknowledge health cost uncertainty in projection_watch.",market:"Reference that simulations already model 5,000 scenarios including crashes.",legacy:"Connect estate projection to stated legacy concern."};
  var wInstr = worries.map(function(w){return worryMap[w]||"";}).filter(Boolean).join(" ");

  // ── 5. CONTEXT SLOTS ──
  var ptSlot = D.ptM > 0 ? "Part-time bridge: " + D.ptM + "$/mo x " + D.ptYrs + "yrs." : "";
  var cplSlot = q.couple === "yes" ? "Client has a partner but Essentiel models ONE person. Mention this once in snapshot_intro." : "";
  var propSlot = rp.mortBal > 0 ? (rp.mortFreeAge > p.retAge ? "MORTGAGE IN RETIREMENT: extends " + (rp.mortFreeAge - p.retAge) + " years past retirement." : "Mortgage paid off by age " + rp.mortFreeAge + ".") : "";

  var winMap = {home:fr?"Reconnaître l'achat immobilier comme un actif concret.":"Acknowledge home purchase as tangible asset.",debtfree:fr?"Souligner l'absence de dettes.":"Acknowledge debt-free status.",saving:fr?"Reconnaître l'habitude d'épargne.":"Acknowledge consistent saving habit.",investing:fr?"Reconnaître les connaissances en investissement.":"Acknowledge investing knowledge.",business:fr?"Reconnaître l'esprit entrepreneurial.":"Acknowledge entrepreneurial ability.",none:fr?"Ce bilan est un point de départ.":"This assessment is a starting point."};
  var winInstr = "WIN: " + (winMap[q.win] || winMap.none);

  var emphasisLines = [];
  if (plan.emphasizeDebt) emphasisLines.push("EMPHASIS: Debt is the primary friction. Every dollar of interest is a dollar not compounding.");
  if (plan.emphasizeFees) emphasisLines.push("EMPHASIS: Fee drag is significant at " + Math.round(D.merWeighted * 10000) + " basis points.");
  if (plan.emphasizeGov) emphasisLines.push("EMPHASIS: Government coverage is only " + D.coveragePct + "%. The gap requires sustained personal savings.");
  ...
  if (signals.highEffortLowResult) emphasisLines.push("EMPHASIS: Strong savings effort (" + savingsRate + "%) but success rate is " + D.successPct + "%.");
  var emphasisBlock = emphasisLines.length > 0 ? emphasisLines.join("\n") + "\n" : "";

  var riskOrder = plan.worstCasePlacement === "standard"
    ? "RISK ORDER: Stress test early, keep tone calm."
    : "RISK ORDER: Do not lead with worst-case. Build confidence first, mention risk later.";

  // ── 6. REPORT MODE + THREAD DETERMINATION ──
  var isGuaranteedIncomeProfile = D.coveragePct >= 80 && D.successPct >= 85;
  var reportMode = "solid_accumulation";
  if (D.successPct < 50 && rp.debtBal > 0) reportMode = "critical_debt";
  else if (D.successPct < 50) reportMode = "critical_gap";
  else if (isGuaranteedIncomeProfile) reportMode = "solid_pension";

  var isFireProfile = p.retAge <= 50;
  var govCoverageRatio = D.retSpM > 0 ? D.govMonthly / D.retSpM : 0;

  var threadHint;
  if (rp.debtBal > 0 && (signals.debtDragOverSavings || (rp.debts || []).some(function(d){return d.rate >= 0.10;})))
    threadHint = "debt-drag";
  else if (D.successPct >= 85 && govCoverageRatio >= 0.70)
    threadHint = "pension-fortress";
  else if (signals.timeLeverage && D.successPct >= 70)
    threadHint = "time-leverage";
  else if (D.successPct < 50 && D.gapMonthly > 0)
    threadHint = "gap-structural";
  else if (D.successPct >= 40 && D.successPct < 80)
    threadHint = "catch-up-viable";
  else
    threadHint = "optimization-fiscal";

  // ── 7. CONTEXT-AWARE HINTS ──
  var gradeDiagHint;
  if (D.successPct < 30) {
    gradeDiagHint = reportMode === "critical_debt"
      ? "Annual debt cost of " + D.debtAnnualCost + "$ erodes savings capacity. Diagnosis: debt drag is the structural bottleneck."
      : "Monthly gap of " + D.gapMonthly + "$ between spending and guaranteed income is unsustainable. Diagnosis: the gap is structural.";
  } else if (D.successPct < 70) {
    gradeDiagHint = "Plan is viable but fragile. Identify the single most impactful adjustment. Savings rate: " + savingsRate + "%.";
  } else if (D.successPct < 85) {
    gradeDiagHint = "Solid base. The plan holds in most scenarios. Tax efficiency is likely the most significant lever.";
  } else {
    gradeDiagHint = reportMode === "solid_pension"
      ? "Guaranteed income covers " + D.coveragePct + "% of needs. Savings serve as cushion, flexibility, or estate."
      : "Plan holds in " + D.successPct + "% of scenarios. Focus on optimization: tax efficiency, fee reduction, estate structure.";
  }

  var biggestRiskHint;
  if (D.successPct === 0)
    biggestRiskHint = "Plan non-viable. Structural cause: " + (reportMode === "critical_debt" ? "debt drag of " + D.debtAnnualCost + "$/yr" : "spending of " + D.retSpM + "$/mo exceeds all income sources") + ".";
  else if (reportMode === "critical_debt")
    biggestRiskHint = "Debt cost of " + D.debtAnnualCost + "$/yr competes with savings of " + (D.monthlyContrib * 12) + "$/yr. The interest-to-savings ratio is the risk.";
  else if (D.withdrawalRatePct > 8)
    biggestRiskHint = "Withdrawal rate of " + D.withdrawalRatePct + "% is unsustainable long-term.";
  else if (isFireProfile)
    biggestRiskHint = "FIRE profile: " + Math.max(0, D.qppAge - p.retAge) + " years of self-funding before government income. Sequence-of-returns risk is critical.";
  else if (D.rP5F <= 0)
    biggestRiskHint = "In the worst 5% of scenarios, savings deplete. P5 wealth at death: " + D.rP5F + "$.";
  else if (D.successPct >= 90)
    biggestRiskHint = "No structural risk. Main exposure: longevity beyond " + D.deathAge + " or sustained inflation above " + (D.inf * 100).toFixed(1) + "%.";
  else
    biggestRiskHint = "The gap of " + D.gapMonthly + "$/mo between spending and guaranteed income drives depletion risk in adverse scenarios.";

  var bestLeverHint;
  if (reportMode === "critical_debt") {
    var worstDebt = (rp.debts || []).reduce(function(a,b){return (a.rate||0) > (b.rate||0) ? a : b;}, {rate:0, type:"debt", bal:0});
    bestLeverHint = "Highest-rate debt: " + worstDebt.type + " at " + Math.round((worstDebt.rate||0) * 100) + "% on " + worstDebt.bal + "$. Each dollar repaid = guaranteed " + Math.round((worstDebt.rate||0) * 100) + "% return.";
  } else if (reportMode === "critical_gap")
    bestLeverHint = "Gap of " + D.gapMonthly + "$/mo. Each 100$/mo reduction in spending or increase in savings extends portfolio duration.";
  else if (reportMode === "solid_pension")
    bestLeverHint = "Guaranteed income exceeds or nearly meets spending. Lever: withdrawal timing, " + gP + " deferral (up to +42% at 70), or estate structure.";
  else if (D.successPct >= 85)
    bestLeverHint = "Tax efficiency: effective rate drops from " + D.taxCurrentEffective + "% to " + D.taxRetirementEffective + "%. Fee reduction: " + (D.merWeighted * 100).toFixed(2) + "% MER = " + D.feeCostLifetime + "$ lifetime cost.";
  else
    bestLeverHint = "Savings rate of " + savingsRate + "% with " + yrsToRet + " years to grow. Contribution consistency is the dominant factor.";

  var successionHint = "";
  if (D.successPct >= 75 && D.rMedF > 100000) {
    if (worrySet.legacy)
      successionHint = "Client cares about legacy. " + Math.round(D.rMedF) + "$ median residual. Frame as achieving their stated goal.";
    else if (q.couple === "yes")
      successionHint = "Couple context: " + Math.round(D.rMedF) + "$ residual could mean years of income security for surviving partner.";
    else if (D.rMedF > 500000)
      successionHint = Math.round(D.rMedF) + "$ residual. Registered accounts are taxable at death. TFSA passes tax-free.";
    else
      successionHint = Math.round(D.rMedF) + "$ residual suggests a safety margin for health, home repairs, or a modest legacy.";
  }

  // ── 8. WHAT-IF SUMMARY (optional, passed from webhook) ──
  whatIfResults = whatIfResults || [];
  var whatIfSummary = whatIfResults.map(function(wi) {
    return {label: wi.title || "", newPct: wi.newPct || 0, delta: (wi.newPct || 0) - D.successPct};
  });

  // ── 9. ENRICHED DATA BLOCK ──
  var data = {
    profile:{age:D.age, retAge:D.retAge, prov:D.prov, sex:D.sex, couple:q.couple, qppAge:D.qppAge, oasAge:D.oasAge, lifestyle:q.lifestyle, riskProfile:q.risk},
    savings:{total:D.totalSavings, retBal:D.retYearBalance, rrsp:D.rrsp, tfsa:D.tfsa, nr:D.nr, monthlyContrib:D.monthlyContrib, savingsRate:savingsRate},
    debt:rp.debtBal > 0 ? (rp.debts || []).map(function(d){return {type:d.type,bal:d.bal,rate:d.rate,annualCost:d.annualCost,months:d.months};}) : null,
    ...
    whatIf:whatIfSummary.length > 0 ? whatIfSummary : null
  };
  ...
  // ── SYSTEM PROMPT (OPUS — enhanced) ──
  var sys = "You narrate buildfi.ca Essentiel retirement reports (Opus tier — richer analysis, 16 slots).\n"
    + "You write like a human financial planner who has studied this person's file for an hour — not like a template engine.\n"
    + "Each slot must feel like it was written specifically for THIS person. If two different profiles could produce the same text, you've failed.\n"
    + "\n=== COMPLIANCE (AMF / OSFI — NON-NEGOTIABLE) ===\n"
    + "This is an EDUCATIONAL tool, NOT financial advice.\n"
    + "1. Facts from DATA may use present tense.\n"
    + "2. Any implication, projection, or outcome MUST use conditional tense (pourrait/serait/could/would/permettrait/constituerait/ajouterait).\n"
    + "3. NEVER use: devriez, devrait, recommandons, conseillons, il faut, vos devez, assurez-vous, you should, you must, we recommend, considérez, optimisez, priorisez, plan d'action, recommandation(s).\n"
    + "4. Observational language ONLY. Describe what numbers show; never prescribe actions.\n"
    + "5. No debt shaming. State mathematical cost only.\n"
    + "6. NEVER suggest combining scenarios or adding their effects together.\n"
    + "7. SAFE conditional words you CAN use: permettrait, constituerait, ajouterait, représenterait, signifierait, impliquerait, équivaudrait. These are observational conditionals, not prescriptive.\n"
    + "\n=== ANTI-HALLUCINATION (CRITICAL — ZERO TOLERANCE) ===\n"
    + "- Use ONLY numbers from the DATA block. Do NOT invent, round differently, estimate, or extrapolate.\n"
    + "- No external averages, no typical ranges, no invented thresholds, no made-up benchmarks.\n"
    + "- NEVER translate percentages into ratio phrases ('1 in 4', '9 sur 10'). Use exact DATA percentages only.\n"
    + "- If you are tempted to write a number not in DATA, STOP. Delete the sentence.\n"
    + "- DOUBLE-CHECK: every number in your output must have a direct source in DATA.\n"
    + "\n=== LANGUAGE ===\n"
    + "- " + (fr ? "French (vous). Naturel, pas robotique. Variez les tournures de phrase." : "English. Natural, not robotic. Vary sentence structure.") + "\n"
    + "- " + (profile.literacy === "advanced" ? "Grade 12 reading level. Precise financial vocabulary OK. Short declarative sentences." : "Grade 10 reading level. Short sentences. No jargon.") + " Never use acronyms — write in full: " + gP + ", " + oN + ".\n"
    + "- JARGON BAN: Never use 'résiduel', 'patrimoine résiduel', 'percentile', 'taux de retrait', 'décaissement', 'véhicule fiscal', 'portefeuille'. Instead say: 'ce qui resterait', 'l'argent qui resterait', 'dans les scénarios les plus/moins favorables', 'frais de gestion de X %', 'rythme de retrait', 'retraits', 'type de compte', 'épargne/placements'.\n"
    + "- NEVER reference external averages or what 'most investors/portfolios' do.\n"
    + "\n=== FILLER BAN (ZERO TOLERANCE) ===\n"
    + "NEVER write: 'il est important de noter', 'il convient de souligner', 'il convient de noter', 'dans ce contexte', 'par ailleurs', 'en outre', 'il est à noter', 'notons que', 'soulignons que', 'mentionnons que', 'it is important to note', 'it should be noted', 'worth noting', 'in this context'.\n"
    + "Every sentence must carry information. If you can delete it without losing meaning, delete it.\n"
    + "\n=== VOCABULARY BAN ===\n"
    + "NEVER use: modeste, raisonnable, agressif, prudent, confortable, élevé/faible (for spending), ambitieux.\n"
    + "Use instead: " + (fr ? "'niveau de dépenses visé', 'objectif plus exigeant', 'marge plus limitée', 'structure plus défensive'" : "'target spending level', 'more demanding objective', 'tighter margin', 'more defensive structure'") + "\n"
    + "\n=== VOICE ===\n" + voiceInstr + "\n"
    + "\n=== PARAGRAPH FORMULA (MANDATORY) ===\n"
    + "Every analytical paragraph: OBSERVATION (specific number, present tense) → CAUSE (why) → IMPLICATION (conditional tense, translate into months/years/dollars).\n"
    + "\n=== INSIGHT HIERARCHY ===\n"
    + "Write slots in this conceptual order:\n"
    + "1. thread (classify) → 2. grade_diagnostic (synthesize) → 3. biggest_risk + best_lever (focus)\n"
    + "→ 4. snapshot_intro (frame) → 5. projection/income/savings/debt (analyze)\n"
    + "→ 6. priority_rationale + whatif_interpretation (contextualize)\n"
    + "→ 7. tax_fees_insight + succession_note (optimize) → 8. closing_observation (close)\n"
    + "Each slot must build on the thread, not repeat it.\n"
    + "\n=== SLOT ORTHOGONALITY ===\n"
    + "Every slot covers a DIFFERENT analytical dimension. If two slots could merge without losing info, one is redundant.\n"
    + "biggest_risk ≠ projection_watch (risk = cause, watch = consequence)\n"
    + "best_lever ≠ priority_rationale (lever = what, rationale = why this ordering)\n"
    + "savings_analysis ≠ income_analysis (savings = accumulation, income = retirement flows)\n"
    + "\n=== UNIQUENESS ===\n"
    + "Test: if you replaced the numbers and the text still works unchanged, rewrite it.\n"
    ...
    + "Instead: reference the specific number, timeline, or account type that makes this person unique.\n"
    + (isFireProfile
      ? "\n=== FIRE PROFILE ===\n"
        + "INTENTIONAL early retirement. Do NOT frame as failure.\n"
        + "Critical bridge: " + Math.max(0, D.qppAge - p.retAge) + " years before " + gP + "/" + oN + ".\n"
        + "Bridge cost: " + (D.retSpM * 12 * Math.max(0, D.qppAge - p.retAge)) + "$.\n"
        + "Sequence-of-returns risk is the primary threat.\n"
      : "")
    + (govCoverageRatio >= 0.70
      ? "\n=== PENSION-DOMINANT PROFILE ===\n"
        + "Government covers " + Math.round(govCoverageRatio * 100) + "% of spending.\n"
        + "Savings are a cushion, not primary income. Do NOT suggest increasing contributions.\n"
      : "")
    + "\n=== OUTPUT ===\n"
    + "Output ONLY a single valid JSON object with exactly 16 keys. No markdown, no preamble, no code fences, no trailing commas.\n"
    + "If you cannot comply with a slot, output an empty string for that slot.\n";

  // ── USER PROMPT (16 slots) ──
  var usr = "=== VOICE ===\n" + voiceInstr + "\n\n"
    + "=== NARRATIVE ARC ===\n" + narrativeArc + "\n\n"
    + emphasisBlock
    + riskOrder + "\n\n"
    ...
    + (cplSlot ? cplSlot + "\n" : "") + (propSlot ? propSlot + "\n" : "") + (ptSlot ? ptSlot + "\n" : "")
    + "\n=== DATA ===\n" + JSON.stringify(data)
    + '\n\n=== SLOT INSTRUCTIONS (16 slots) ===\n'
    + 'Return a JSON object with exactly these 16 keys. 2-4 sentences per slot unless noted. Each sentence must carry information.\n\n'
    + '{"thread":"CLASSIFY this profile into exactly one thread keyword. Suggested: ' + threadHint + ' — override if DATA tells a different story. Options: debt-drag | time-leverage | pension-fortress | gap-structural | catch-up-viable | optimization-fiscal. Output ONLY the keyword.",'
    + '"grade_diagnostic":"Grade ' + D.grade + ' (' + D.successPct + '%). ' + gradeDiagHint + ' Write a 2-3 sentence diagnostic that synthesizes what this grade MEANS for this specific person. Not generic — reference the specific numbers that drive the grade.",'
    + '"biggest_risk":"' + biggestRiskHint + ' Identify the single biggest risk factor and quantify it using DATA numbers only. Do NOT list multiple risks — pick THE ONE that matters most for this profile.",'
    + '"best_lever":"' + bestLeverHint + ' Identify the single most impactful lever. Be specific: name the account type, the dollar amount, the rate. NEVER default to CÉLI/TFSA generically — pick the lever that is actually most impactful for THIS profile based on DATA.",'
    + '"snapshot_intro":"Grade ' + D.grade + ' (' + D.successPct + '%). ' + (yrsToRet <= 5 && yrsToRet > 0 ? 'Proximity to retirement.' : yrsToRet > 30 ? 'Long runway ahead.' : '') + ' Weave in WIN acknowledgment: ' + (winMap[q.win] || winMap.none) + ' Set the emotional tone. Connect to the thread.",'
    + '"projection_good":"Median wealth at death: ' + D.rMedF + '$. P75: ' + D.rP75F + '$. In most scenarios, the plan ' + (D.rMedF > 0 ? 'leaves money unspent' : 'depletes') + '. Frame what ' + D.rMedF + '$ means concretely — years of additional coverage or buffer.",'
    + '"projection_watch":"P5 (worst 5%): ' + D.rP5F + '$. P25: ' + D.rP25F + '$. ' + (D.rP5F <= 0 ? 'In worst scenarios, savings run out.' : 'Pessimistic scenarios still leave cushion.') + ' Focus on what causes the spread between P25 and median — market volatility, inflation, or longevity.' + (isFireProfile ? ' Sequence-of-returns risk is critical.' : '') + '",'
    + '"income_analysis":"[COMPLIANCE-SENSITIVE: use conditional tense. Safe: permettrait, couvrirait, représenterait. Banned: devriez, il faut, recommandons.] ' + gP + ' ' + D.qppMonthly + '$/mo + ' + oN + ' ' + D.oasMonthly + '$/mo' + (D.dbPensionMonthly > 0 ? ' + pension ' + D.dbPensionMonthly + '$/mo' : '') + ' = ' + D.govMonthly + '$/mo covering ' + D.coveragePct + '% of spending. Gap: ' + D.gapMonthly + '$/mo from savings. Total retirement draw: ' + (D.gapMonthly * 12 * Math.max(0, D.deathAge - D.retAge)) + '$ over ' + Math.max(0, D.deathAge - D.retAge) + ' years. Withdrawal rate: ' + D.withdrawalRatePct + '%. MINIMUM 3 sentences. Do NOT just restate the coverage percentage — interpret WHAT it means: translate the gap into concrete monthly or annual amounts, explain what happens during the bridge period before government income starts, and frame how many years of autonomy the savings would provide at this draw rate.",'
    + '"savings_analysis":"Savings ' + D.totalSavings + '$ → projected ' + D.retYearBalance + '$ at retirement. ' + (D.rrsp > 0 ? 'RRSP: ' + D.rrsp + '$ ' : '') + (D.tfsa > 0 ? 'TFSA: ' + D.tfsa + '$ ' : '') + (D.nr > 0 ? 'Non-reg: ' + D.nr + '$ ' : '') + 'Rate: ' + savingsRate + '% (' + D.monthlyContrib + '$/mo). Autonomy: ~' + (D.retYearBalance > 0 ? Math.round(D.retYearBalance / Math.max(1, D.gapMonthly * 12) * 10) / 10 : 0) + ' years at current gap.",'
    + (rp.debtBal > 0 ? '"debt_analysis":"Total debt: ' + rp.debtBal + '$ costing ' + D.debtAnnualCost + '$/yr. ' + (signals.debtDragOverSavings ? 'Interest (' + D.debtAnnualCost + '$) exceeds annual savings (' + (D.monthlyContrib * 12) + '$). ' : '') + 'Debts: ' + (rp.debts || []).map(function(d){return d.type + ' ' + d.bal + '$ at ' + Math.round(d.rate * 100) + '%';}).join(', ') + '. No shaming — state mathematical cost only.",' : '"debt_analysis":"",')
    + '"priority_rationale":"[COMPLIANCE-SENSITIVE: observe, do not prescribe. Safe: représenterait, signifierait, permettrait. Banned: devriez, recommandons, priorisez, optimisez.] ' + (rp.debtBal > 0 && signals.debtDragOverSavings
      ? 'Debt cost exceeds savings: interest of ' + D.debtAnnualCost + '$/yr vs contributions of ' + (D.monthlyContrib * 12) + '$/yr. Explain what this race means for wealth accumulation.'
      : D.withdrawalRatePct > 4
      ? 'Withdrawal rate of ' + D.withdrawalRatePct + '% exceeds the 4% threshold. Explain what 1% reduction in withdrawal rate would represent in monthly spending.'
      : signals.timeLeverage
      ? 'With ' + yrsToRet + ' years to retirement, each dollar compounds ' + Math.round(Math.pow(1 + (D.expReturn || 0.05), yrsToRet)) + 'x. Frame the power of consistency.'
      : 'Identify the single most interesting tension or opportunity in this profile that other slots haven\'t covered.') + '",'
    + '"whatif_interpretation":"' + (whatIfSummary.length > 0
      ? 'What-if results: ' + whatIfSummary.map(function(w){return w.label + ' → ' + (w.delta > 0 ? '+' : '') + w.delta + ' pts (' + w.newPct + '%)';}).join('; ') + '. Identify which scenario has the most impact and why. If a scenario shows 0% change, explain why. Do NOT suggest combining scenarios.'
      : 'No what-if data available. Write a brief observation about what single change would most move the needle for this profile.') + '",'
    + '"tax_fees_insight":"[COMPLIANCE-SENSITIVE: observe, do not prescribe. Safe: représenterait, équivaudrait, permettrait. Banned: devriez, optimisez, considérez.] Effective rate ' + D.taxCurrentEffective + '% now → ' + D.taxRetirementEffective + '% retirement. ' + (D.taxCurrentEffective > D.taxRetirementEffective ? 'Each dollar in retirement keeps ' + (D.taxCurrentEffective - D.taxRetirementEffective).toFixed(1) + ' cents more.' : D.taxCurrentEffective < D.taxRetirementEffective ? 'Retirement withdrawals face a higher rate.' : 'Rates are similar.') + ' Fees: ' + (D.merWeighted * 100).toFixed(2) + '% MER = ' + D.feeCostLifetime + '$ lifetime cost. ' + (D.feeCostLifetime > 100000 ? 'Translate into years of retirement spending.' : D.feeCostLifetime > 30000 ? 'Translate into months of income lost.' : 'Fees are moderate.') + (signals.rrspHeavy ? ' RRSP is ' + signals.rrspHeavy + '% of total — FERR conversions add taxable income.' : '') + (signals.tfsaHeavy ? ' TFSA is ' + signals.tfsaHeavy + '% — withdrawals are tax-free.' : '') + '",'
    + (successionHint ? '"succession_note":"' + successionHint + ' Max 400 chars. Conditional tense mandatory.",' : '"succession_note":"",')
    + '"table_narrative":"The 5-year snapshot table shows wealth at key ages. Read the table for the client: describe the trajectory (growth phase → peak → drawdown), identify the inflection point where government income begins, and quantify the rate of decline during retirement. If the portfolio depletes before death, state at what age and what the annual shortfall would be. If wealth grows throughout, frame it as estate potential. Connect back to the thread (' + threadHint + '). Use specific ages and dollar amounts from DATA only.",'
    + '"closing_observation":"Write a personalized closing that ties back to the thread (' + threadHint + '). Reference one specific number from DATA that captures this person\'s situation. NOT generic — must fail the substitution test. Max 300 chars."}';
...
function renderReport_v5(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  function aiSlot(text) {
    if (aiLoading) return '<div style="font-size:14px;color:#888;line-height:1.75;margin:12px 0;padding:12px 16px;background:#FDFBF7;border-left:3px solid #c49a1a;border-radius:4px;font-style:italic">\u2026</div>';
    if (!text) return '';
    return '<div style="font-size:14px;color:#444;line-height:1.75;margin:12px 0;padding:12px 16px;background:#FDFBF7;border-left:3px solid #c49a1a;border-radius:4px">' + escHtml(text) + '</div>';
  }
  ...
  function bridge(textFr, textEn) {
    return '<div style="font-size:12px;color:var(--tm);font-style:italic;margin:0 0 16px;padding:0 2px;line-height:1.75">' + t(textFr, textEn) + '</div>';
  }
  ...
  function wealthChartSVG() {
    ...
    for (var i = 0; i < ages.length; i++) { var x = P + i * (bw + 1), bH = Math.max(0.5, vals[i] / mx * CH), y = PT + CH - bH; svg += '<rect x="' + x + '" y="' + y + '" width="' + bw + '" height="' + bH + '" fill="' + (ages[i] >= D.retAge ? "#c49a1a" : "#D4A85A") + '" rx="1" opacity="' + (ages[i] >= D.retAge ? 1 : 0.65) + '"/>'; if (i % 5 === 0) svg += '<text x="' + (x + bw/2) + '" y="' + (H-8) + '" text-anchor="middle" font-size="8" fill="#aaa" font-family="monospace">' + ages[i] + '</text>'; }
    if (ri >= 0) svg += '<line x1="' + (P+ri*(bw+1)) + '" x2="' + (P+ri*(bw+1)) + '" y1="' + PT + '" y2="' + (PT+CH) + '" stroke="#c49a1a" stroke-dasharray="4,3" stroke-width="1.5"/><text x="' + (P+ri*(bw+1)+4) + '" y="' + (PT+12) + '" font-size="9" fill="#c49a1a" font-weight="700">' + t("Retraite","Retirement") + '</text>';
  ...
  function fanChartSVG() {
    ...
    var svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;height:auto">';
    for (var g = 0; g <= 4; g++) { var yy = PT + CH - CH * g / 4; svg += '<line x1="' + P + '" x2="' + (W-PR) + '" y1="' + yy + '" y2="' + yy + '" stroke="#E8E0D4" stroke-width="0.5"/><text x="' + (P-6) + '" y="' + (yy+3) + '" text-anchor="end" font-size="9" fill="#aaa" font-family="monospace">' + Math.round(mx*g/4/1000) + 'K</text>'; }
    svg += '<polygon points="' + pts(p95) + ' ' + pts(p5.slice().reverse()) + '" fill="rgba(196,154,26,0.08)"/>';
    svg += '<polygon points="' + pts(p75) + ' ' + pts(p25.slice().reverse()) + '" fill="rgba(196,154,26,0.20)"/>';
    svg += '<polyline points="' + pts(p50) + '" fill="none" stroke="#c49a1a" stroke-width="2.5" stroke-linejoin="round"/>';
    ...
    svg += '<rect x="' + (P+8) + '" y="4" width="12" height="12" fill="rgba(196,154,26,0.20)" rx="2"/><text x="' + (P+24) + '" y="13" font-size="9" fill="#888">' + t("Probable (milieu 50%)","Likely (middle 50%)") + '</text>';
    svg += '<rect x="' + (P+195) + '" y="4" width="12" height="12" fill="rgba(196,154,26,0.08)" rx="2"/><text x="' + (P+211) + '" y="13" font-size="9" fill="#888">' + t("Possible (90% des cas)","Possible (90% of cases)") + '</text>';
    svg += '<line x1="' + (P+390) + '" x2="' + (P+412) + '" y1="10" y2="10" stroke="#c49a1a" stroke-width="2.5"/><text x="' + (P+417) + '" y="13" font-size="9" fill="#888">' + t("Résultat médian","Median outcome") + '</text>';
    svg += '</svg>';
    return svg;
  }
  ...
  // Header
  h += '<div style="display:flex;justify-content:space-between;margin-bottom:36px;flex-wrap:wrap"><div><h1 style="font-size:30px;font-weight:800;color:var(--g);letter-spacing:-0.5px">buildfi.ca</h1><div style="font-size:14px;color:#888;margin-top:2px">' + t("Bilan","Snapshot") + ' — ' + new Date().toLocaleDateString(fr?"fr-CA":"en-CA") + '</div></div><div style="text-align:right;font-size:10px;color:#bbb;line-height:1.8">v11.12.9<br>' + D.nSim.toLocaleString() + ' simulations<br>' + D.prov + '</div></div>';
  ...
  // Timeline
  var tlEv = [{a:D.age,l:t("Aujourd'hui","Today"),c:"#4680C0",on:true},{a:D.retAge,l:t("Retraite","Retirement"),c:"#c49a1a"},{a:D.qppAge,l:t("Prestations gouvernementales","Government benefits"),c:"#2A8C46"},{a:D.avgDeath,l:t("Âge médian de décès","Median death age"),c:"#888"}];
  h += '<div style="position:relative;padding:28px 20px 16px;background:#FDFBF7;border:1px solid var(--bd);border-radius:10px;margin:14px 0"><div style="position:absolute;top:38px;left:50px;right:50px;height:3px;background:linear-gradient(90deg,#4680C0,#c49a1a,#2A8C46,#888);border-radius:2px;opacity:.2"></div><div style="display:flex;justify-content:space-between;position:relative">';
  ...
  if (rp.debtBal > 0 || rp.mortBal > 0) {
    ...
    h += '<div style="font-size:13px;font-weight:700;color:#1A1208;margin:0 0 14px;padding-bottom:8px;border-bottom:1px solid #E8E0D0">' + t("Comparatif \u2014 impact financier par cat\u00e9gorie","Comparative \u2014 financial impact by category") + '</div>';
    priorities.forEach(function(p, i) {
      h += '<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #E8E0D0">';
      h += '<span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:#c49a1a;color:#fff;font-size:12px;font-weight:800;flex-shrink:0">' + (i+1) + '</span>';
      h += '<div style="flex:1"><div style="font-weight:700;font-size:15px;color:#1A1208">' + p.name + '</div><div style="font-size:12px;color:#666;margin-top:2px">' + p.why + '</div></div>';
  ...
  h += '<svg width="180" height="180" viewBox="0 0 180 180"><circle cx="90" cy="90" r="70" fill="none" stroke="#2A8C46" stroke-width="22" stroke-dasharray="' + gDash + ' ' + (dC-gDash) + '" transform="rotate(-90 90 90)" opacity=".85"/><circle cx="90" cy="90" r="70" fill="none" stroke="#c49a1a" stroke-width="22" stroke-dasharray="' + (dC-gDash) + ' ' + gDash + '" stroke-dashoffset="' + (-gDash) + '" transform="rotate(-90 90 90)" opacity=".85"/><text x="90" y="86" text-anchor="middle" font-size="22" font-weight="800" fill="#1A1A1A" font-family="monospace">' + fDol(totMo).replace(" $","") + '</text><text x="90" y="104" text-anchor="middle" font-size="10" fill="#888">/' + t("mois","mo") + '</text></svg>';
  ...
  h += '<div style="display:flex;align-items:center;gap:10px"><div style="width:14px;height:14px;border-radius:4px;background:#c49a1a;flex-shrink:0"></div><div><div style="font-size:14px;font-weight:700;color:#333">' + t("Épargne et placements","Savings and investments") + '</div><div style="font-size:12px;color:#888;font-family:monospace">' + fDol(sMo) + '/' + t("mois","mo") + ' — ' + (100-govPct) + '%</div></div></div>';
  h += '</div></div>';
  // Benefits detail
  ...
  if(costDelay>0){
    ...
    h+=card(
      ...
      +'<strong>'+t("Ce que cela signifie :","What this means:")+'</strong> '+t(
        "Chaque ann\u00e9e d\u2019attente co\u00fbte davantage que la pr\u00e9c\u00e9dente, car la croissance compos\u00e9e amplifie l\u2019\u00e9cart. L\u2019effet est proportionnel au d\u00e9lai, pas au montant.",
        "Each year of waiting costs more than the last, because compound growth amplifies the gap. The effect is proportional to the delay, not the amount."
      )+'</div>'
      +'</div></div>',
  ...
  // Footer
  h += '<div style="text-align:center;padding-top:20px;border-top:1px solid var(--bd);font-size:10px;color:#bbb">buildfi.ca — Bilan — v11.12.9 — ' + D.nSim + ' simulations — ' + D.prov + '</div>';
...
function renderReport_v6(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  var renderedWhatIf = (D.whatIf || []).slice();
  if (D.successPct >= 85) {
    renderedWhatIf = renderedWhatIf.filter(function(w) {
      var deltaAbs = Math.abs(w.newPct - D.successPct);
      return deltaAbs >= 2 || w.newGrade !== gi.g;
    }).sort(function(a, b) {
      return Math.abs(b.newPct - D.successPct) - Math.abs(a.newPct - D.successPct);
    }).slice(0, 2);
  } else if (D.successPct >= 50) {
    renderedWhatIf = renderedWhatIf.filter(function(w) {
      return Math.abs(w.newPct - D.successPct) >= 1;
    }).slice(0, 3);
  }
  if (renderedWhatIf.length) {
    renderedWhatIf.forEach(function(w) {
      var isCombined = w.type === "combined", isUp = w.type === "up" || isCombined, delta = w.newPct - D.successPct;
      var bCol = isCombined ? "var(--g)" : isUp ? "var(--gn)" : "var(--rd)";
      var bgCol = isCombined ? "rgba(196,154,26,.06)" : isUp ? "rgba(42,140,70,.04)" : "rgba(204,68,68,.04)";
      var borderCol = isCombined ? "rgba(196,154,26,.3)" : isUp ? "rgba(42,140,70,.2)" : "rgba(204,68,68,.2)";
      var ngc = w.newGrade.charAt(0) === "A" ? "var(--gn)" : w.newGrade === "B+" ? "var(--gn)" : w.newGrade === "B" ? "var(--am)" : w.newGrade === "C" ? "var(--am)" : "var(--rd)";
      var icon = isCombined ? "\ud83c\udfaf " : isUp ? "\u2197 " : "\u26a0 ";
      whatIfCardsHTML += '<div style="background:' + bgCol + ';border:1px solid ' + borderCol + ';border-left:4px solid ' + bCol + ';border-radius:10px;padding:18px 20px;margin-bottom:12px">';
      whatIfCardsHTML += '<div style="font-size:15px;font-weight:700;margin-bottom:2px">' + icon + w.title + '</div>';
  ...
  incStreams.push({label: t("\u00c9pargne","Savings"), monthly: D.gapMonthly, color: "#c49a1a"});
  ...
  h += '<div>';
  h += '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="48" viewBox="0 0 220 48"><g><rect x="0" y="32" width="28" height="8" rx="2" fill="#1a2744"/><rect x="4" y="22" width="26" height="8" rx="2" fill="#1a2744" opacity="0.50"/><rect x="8" y="12" width="24" height="8" rx="2" fill="#c49a1a"/></g><text x="40" y="38" font-family="DM Sans,sans-serif" font-size="34" font-weight="700" letter-spacing="-0.5"><tspan fill="#1a2744">build</tspan><tspan fill="#c49a1a">fi</tspan></text></svg>';
  h += '<div style="font-size:13px;color:var(--tm);margin-top:4px">' + t("Bilan","Snapshot") + ' \u2014 ' + date + '</div></div>';
  ...
  if (D.successPct < 70) {
    ...
    dcVuln = t(
      "Une p\u00e9riode de " + vulnYrs + " an" + (vulnYrs > 1 ? "s" : "") + " sans revenus garantis complets s\u00e9pare votre retraite du d\u00e9but de vos prestations gouvernementales.",
      "A " + vulnYrs + "-year gap without full guaranteed income separates your retirement from the start of government benefits."
    );
  ...
  } else if (D.debtBal > D.totalSavings * 0.3 && D.debtBal > 0) {
    dcVuln = t(
      "Les dettes repr\u00e9sentent " + Math.round(D.debtBal / D.totalSavings * 100) + "\u00a0% de l\u2019\u00e9pargne totale \u2014 cette proportion pourrait freiner la croissance du patrimoine.",
      "Debts represent " + Math.round(D.debtBal / D.totalSavings * 100) + "% of total savings \u2014 this proportion could slow wealth growth."
    );
  } else if (D.medRuin < 999 && D.medRuin < D.deathAge - 3) {
    dcVuln = t(
  ...
  // Lever (data-driven by mode)
  if (reportMode === "critical_debt" && D.debts && D.debts.length) {
    var worstD = D.debts.reduce(function(a,b){return a.rate > b.rate ? a : b;});
    var debtLabel = fr ? {cc:"Carte de cr\u00e9dit",student:"Pr\u00eat \u00e9tudiant",car:"Pr\u00eat auto",loc:"Marge de cr\u00e9dit"} : {cc:"Credit card",student:"Student loan",car:"Car loan",loc:"Line of credit"};
    dcLever = (debtLabel[worstD.type] || worstD.type) + " \u2014 " + fDol(Math.round(worstD.bal)) + " " + t("\u00e0","at") + " " + Math.round(worstD.rate * 100) + "\u00a0% \u2014 " + t("chaque dollar rembours\u00e9 \u00e9quivaut \u00e0 un rendement garanti de " + Math.round(worstD.rate * 100) + "\u00a0%.", "every dollar repaid equals a guaranteed " + Math.round(worstD.rate * 100) + "% return.");
  } else if (reportMode === "critical_gap") {
    dcLever = t(
      "L\u2019\u00e9cart mensuel de " + fDol(D.gapMonthly) + " entre les d\u00e9penses vis\u00e9es et les revenus garantis d\u00e9termine la dur\u00e9e du portefeuille. Chaque r\u00e9duction de 100 $/mois prolongerait la couverture.",
      "The monthly gap of " + fDol(D.gapMonthly) + " between target spending and guaranteed income determines portfolio duration. Each $100/mo reduction would extend coverage."
    );
  } else if (reportMode === "solid_pension") {
    var surplusM = D.govMonthly - D.retSpM;
    dcLever = surplusM > 0
      ? t("Les revenus garantis d\u00e9passent les d\u00e9penses de " + fDol(surplusM) + "/mois. Le portefeuille (" + fDol(D.totalSavings) + ") pourrait servir de coussin, succession ou flexibilit\u00e9.", "Guaranteed income exceeds spending by " + fDol(surplusM) + "/mo. The portfolio (" + fDol(D.totalSavings) + ") could serve as a cushion, estate, or flexibility.")
      : t("Les revenus garantis couvrent " + D.coveragePct + "\u00a0% des besoins. L\u2019efficacit\u00e9 du d\u00e9caissement et le timing " + qppLabel + "/PSV seraient les dimensions cl\u00e9s.", "Guaranteed income covers " + D.coveragePct + "% of needs. Withdrawal efficiency and " + qppLabel + "/OAS timing would be the key dimensions.");
  } else if (priorities.length > 0) {
    dcLever = priorities[0].name + (priorities[0].rate ? " (" + priorities[0].rate + ")" : "") + " \u2014 " + priorities[0].why;
  } else if (D.savingsRate < 10 && D.age < 55) {
    dcLever = t(
      "Taux d\u2019\u00e9pargne actuel de " + D.savingsRate + "\u00a0% du revenu. Chaque hausse de 1\u00a0% ajoute environ " + fDol(Math.round(D.sal * 0.01 / 12)) + "/mois en cotisations.",
      "Current savings rate of " + D.savingsRate + "% of income. Each 1% increase adds roughly " + fDol(Math.round(D.sal * 0.01 / 12)) + "/mo in contributions."
    );
  } else {
    var yrsLeft = D.retAge - D.age;
    dcLever = t(
      "Avec " + yrsLeft + " ans avant la retraite et un taux de r\u00e9ussite de " + D.successPct + "\u00a0%, la constance des cotisations (" + fDol(monthlyContrib) + "/mois) reste le facteur dominant.",
      "With " + yrsLeft + " years to retirement and a " + D.successPct + "% success rate, contribution consistency (" + fDol(monthlyContrib) + "/mo) remains the dominant factor."
    );
  }
  // Opus AI overrides (fallback to static values computed above)
  if (ai.biggest_risk) dcVuln = ai.biggest_risk;
  if (ai.best_lever) dcLever = ai.best_lever;
  var dcInterpretation = (ai && ai.snapshot_intro) ? ai.snapshot_intro : t(
    "Votre note de " + gi.g + " (" + D.successPct + "\u00a0%) indique " + (D.successPct >= 80 ? "une position solide" : D.successPct >= 60 ? "une position correcte avec des pistes d\u2019am\u00e9lioration" : "une situation qui m\u00e9rite attention") + ". Les sections suivantes d\u00e9taillent chaque dimension de votre plan.",
    "Your grade of " + gi.g + " (" + D.successPct + "%) indicates " + (D.successPct >= 80 ? "a solid position" : D.successPct >= 60 ? "a reasonable position with room for improvement" : "a situation that warrants attention") + ". The following sections detail each dimension of your plan."
  );
  h += '<div style="background:#fff;border:1px solid var(--bd);border-left:4px solid var(--g);border-radius:12px;padding:24px 28px;margin-bottom:32px;box-shadow:0 2px 8px rgba(0,0,0,.03)">';
  ...
  h += '<div style="font-size:14px;color:var(--ts);line-height:1.65">' + dcVuln + '</div></div>';
  ...
  h += '<div style="font-size:14px;color:var(--ts);line-height:1.65">' + dcLever + '</div></div>';
  h += '<div style="height:1px;background:var(--bd);margin:16px 0"></div>';
  h += '<div style="font-size:12px;font-weight:700;color:var(--ts);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">' + t("Ce que cela signifie","What this means") + '</div>';
  h += '<div style="font-size:14px;color:var(--ts);line-height:1.65">' + dcInterpretation + '</div>';
  h += '</div>';

  // ── A+ enrichment: mode-specific next-level insights ──
  if (D.successPct >= 90) {
    var enrichItems = [];
    if (reportMode === "solid_pension") {
      enrichItems.push(t("Le timing " + qppLabel + " (report \u00e0 70 ans = +42\u00a0% de prestation) pourrait lib\u00e9rer le portefeuille pour la flexibilit\u00e9.", qppLabel + " timing (defer to 70 = +42% benefit) could free the portfolio for flexibility."));
      if (D.taxCurrentEffective > 20) enrichItems.push(t("L\u2019\u00e9cart entre taux effectif actuel (" + D.taxCurrentEffective.toFixed(1) + "\u00a0%) et retraite (" + D.taxRetirementEffective.toFixed(1) + "\u00a0%) sugg\u00e8re un potentiel d\u2019efficacit\u00e9 fiscale.", "The gap between current effective rate (" + D.taxCurrentEffective.toFixed(1) + "%) and retirement (" + D.taxRetirementEffective.toFixed(1) + "%) suggests tax efficiency potential."));
      if (D.rMedF > 100000) enrichItems.push(t("Patrimoine m\u00e9dian au d\u00e9c\u00e8s de " + fDol(D.rMedF) + " \u2014 la planification successorale (REER \u2192 FERR, C\u00c9LI non impos\u00e9 au d\u00e9c\u00e8s) pourrait pr\u00e9server davantage pour les h\u00e9ritiers.", "Median estate of " + fDol(D.rMedF) + " \u2014 estate planning (RRSP\u2192RRIF, TFSA tax-free at death) could preserve more for heirs."));
    } else {
      if (quizQ && quizQ.couple === "yes") enrichItems.push(t("En contexte de couple, le fractionnement du revenu de retraite pourrait r\u00e9duire le taux effectif combin\u00e9.", "As a couple, retirement income splitting could reduce the combined effective rate."));
      var ytrE = D.retAge - D.age;
      if (ytrE <= 7) enrichItems.push(t("Avec la retraite dans " + ytrE + " ans, l\u2019ordre de d\u00e9caissement (C\u00c9LI vs REER vs non enregistr\u00e9) pourrait avoir un impact fiscal significatif.", "With retirement " + ytrE + " years away, withdrawal order (TFSA vs RRSP vs non-registered) could have significant tax impact."));
      if (D.rMedF > 500000) enrichItems.push(t("Patrimoine m\u00e9dian au d\u00e9c\u00e8s de " + fDol(D.rMedF) + ". Les comptes enregistr\u00e9s sont imposables au d\u00e9c\u00e8s \u2014 le C\u00c9LI passe libre d\u2019imp\u00f4t.", "Median estate of " + fDol(D.rMedF) + ". Registered accounts are taxable at death \u2014 TFSA passes tax-free."));
      if (D.oasMonthly > 0 && D.retSpM * 12 > 90000) enrichItems.push(t("Revenu de retraite projet\u00e9 au-dessus du seuil de r\u00e9cup\u00e9ration de la PSV (~90\u00a0000 $). Une strat\u00e9gie de d\u00e9caissement pourrait att\u00e9nuer la r\u00e9cup\u00e9ration.", "Projected retirement income above OAS clawback threshold (~$90,000). A withdrawal strategy could mitigate the clawback."));
    }
    if (enrichItems.length > 0) {
      h += '<div style="background:var(--gnbg);border:1px solid rgba(42,140,70,.15);border-radius:10px;padding:18px 22px;margin-bottom:24px">';
      h += '<div style="font-size:12px;font-weight:700;color:var(--gn);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">' + t("Dimensions \u00e0 explorer","Dimensions to explore") + '</div>';
      enrichItems.forEach(function(item) {
        h += '<div style="font-size:13px;color:var(--ts);line-height:1.7;margin-bottom:6px;padding-left:14px;border-left:2px solid rgba(42,140,70,.2)">' + item + '</div>';
      });
      h += '<div style="font-size:11px;color:var(--tm);margin-top:8px;font-style:italic">' + t("Ces observations sont g\u00e9n\u00e9rales et ne constituent pas des recommandations personnalis\u00e9es.", "These are general observations and do not constitute personalized recommendations.") + '</div>';
      h += '</div>';
    }
  }
  ...
  var tocItems = [
    ...
    [t("Annexes","Appendices"), "#sec-annexes"]
  ];
  if (whatIfCardsHTML) tocItems.splice(6, 0, [t("Renforcer","Strengthen"), "#sec-renforcer"]);
  ...
  // ── Report mode routing (4 modes) ──
  var isGuaranteedIncomeProfile = D.coveragePct >= 80 && D.successPct >= 85;
  var reportMode = "solid_accumulation";
  if (D.successPct < 50 && D.debtBal > 0) reportMode = "critical_debt";
  else if (D.successPct < 50) reportMode = "critical_gap";
  else if (isGuaranteedIncomeProfile) reportMode = "solid_pension";

  // Chantier 5: action hint micro-phrase under grade badge (data-driven by mode)
  var actionHintText = '';
  if (gi.g === 'F' || gi.g === 'D') {
    if (reportMode === "critical_debt") {
      actionHintText = t("Le co\u00fbt annuel des dettes (" + fDol(D.debtAnnualCost) + ") repr\u00e9sente un frein direct \u00e0 la capacit\u00e9 d\u2019\u00e9pargne. Les leviers identifi\u00e9s sont en section 7.", "Annual debt cost (" + fDol(D.debtAnnualCost) + ") directly limits saving capacity. Identified levers are in section 7.");
    } else {
      actionHintText = t("L\u2019\u00e9cart mensuel de " + fDol(D.gapMonthly) + " entre les d\u00e9penses vis\u00e9es et les revenus garantis est le facteur dominant. Voir section 7.", "The monthly gap of " + fDol(D.gapMonthly) + " between target spending and guaranteed income is the dominant factor. See section 7.");
    }
  }
  else if (gi.g === 'C' || gi.g === 'C+') actionHintText = t("Base correcte. Les ajustements cibl\u00e9s en section 7 pourraient am\u00e9liorer la r\u00e9silience du plan.", "Decent foundation. Targeted adjustments in section 7 could improve plan resilience.");
  else if (gi.g === 'B' || gi.g === 'B+') actionHintText = t("Plan solide. L\u2019efficacit\u00e9 fiscale (section 8) serait le levier le plus significatif \u00e0 ce stade.", "Solid plan. Tax efficiency (section 8) would likely be the most significant lever at this stage.");
  else if (['A','A-','A+'].indexOf(gi.g) >= 0) {
    var ytr = D.retAge - D.age;
    if (reportMode === "solid_pension") actionHintText = t("Les revenus garantis couvrent " + D.coveragePct + "\u00a0% des besoins. L\u2019\u00e9pargne pourrait servir de coussin pour l\u2019impr\u00e9vu, la flexibilit\u00e9 ou la succession.", "Guaranteed income covers " + D.coveragePct + "% of needs. Savings could serve as a cushion for the unexpected, flexibility, or estate.");
    else if (ytr <= 5) actionHintText = t("Plan robuste. Avec la retraite dans " + ytr + " ans, les dimensions cl\u00e9s seraient le d\u00e9caissement et le moment du d\u00e9clenchement de la " + qppLabel + "/PSV.", "Solid plan. With retirement " + ytr + " years away, key dimensions would be withdrawal strategy and " + qppLabel + "/OAS timing.");
    else if (D.age >= 55 && ytr <= 12 && D.successPct >= 95) actionHintText = t("Malgr\u00e9 un d\u00e9marrage plus tardif, le plan ressort tr\u00e8s solide aujourd'hui. L'\u00e9pargne accumul\u00e9e et vos cotisations actuelles compensent le retard initial; la priorit\u00e9 est maintenant de s\u00e9curiser le d\u00e9caissement.", "Despite a later start, the plan is very strong today. Accumulated savings and current contributions have offset the initial delay; the priority now is to secure decumulation.");
    else if (D.age <= 40) actionHintText = t("Plan robuste avec " + ytr + " ans devant vous. Le temps est le facteur dominant \u2014 la constance des cotisations amplifie l\u2019effet compos\u00e9.", "Solid plan with " + ytr + " years ahead. Time is the dominant factor \u2014 consistent contributions amplify the compounding effect.");
    else actionHintText = t("Plan robuste dans la quasi-totalit\u00e9 des sc\u00e9narios. L\u2019efficacit\u00e9 fiscale et la planification successorale seraient les prochaines dimensions \u00e0 explorer.", "Plan holds in the vast majority of scenarios. Tax efficiency and estate planning would be the next dimensions to explore.");
  }
  if (ai.grade_diagnostic) actionHintText = ai.grade_diagnostic;
  ...
  // Objective callout — surfaced from quiz answer (AI fills it when available)
  (function() {
    var objTxt = ai.objectif || '';
    if (!objTxt) {
      var ov = (quizQ && quizQ.objective) || '';
      if (ov === 'growth') objTxt = fr ? "Faire cro\u00eetre votre patrimoine activement." : "Actively grow your wealth.";
      else if (ov === 'security') objTxt = fr ? "Assurer votre s\u00e9curit\u00e9 financi\u00e8re \u00e0 la retraite." : "Ensure financial security in retirement.";
      else if (ov === 'balance') objTxt = fr ? "\u00c9quilibrer qualit\u00e9 de vie et accumulation patrimoniale." : "Balance quality of life with wealth accumulation.";
      else if (ov === 'legacy') objTxt = fr ? "Constituer un patrimoine durable pour vos h\u00e9ritiers." : "Build lasting wealth for your heirs.";
    }
    if (objTxt) h += '<div style="font-size:13px;color:var(--ts);line-height:1.75;margin-top:10px"><strong>' + t("Votre objectif", "Your objective") + '</strong> \u2014 ' + objTxt + '</div>';
  })();
  h += '</div></div></div>';
  ...
  var mirrorGapEss = Math.max(0, D.retSpM - D.govMonthly);
  var mirrorYrs = D.retAge - D.age;
  var horizonCtx = '';
  if (mirrorYrs >= 25) {
    // Young profiles: emphasize the compounding horizon as the dominant force
    var doubleYrs = Math.round(72 / ((D.expReturn || 0.05) * 100));
    horizonCtx = t(" Avec " + mirrorYrs + " ans devant vous, chaque dollar \u00e9pargn\u00e9 aujourd\u2019hui pourrait doubler environ tous les " + doubleYrs + " ans.",
                    " With " + mirrorYrs + " years ahead, every dollar saved today could roughly double every " + doubleYrs + " years.");
  }
  h += '<div style="background:linear-gradient(135deg,#1A1208 0%,#2D2010 100%);color:#fff;border-radius:14px;padding:24px 28px;margin:8px 0 12px;">'
    ...
    + t("Vous avez " + D.age + " ans. Vous visez la retraite \u00e0 " + D.retAge + " ans avec un revenu de " + f$(D.retSpM) + " par mois. Votre \u00e9pargne totalise " + f$(D.totalSavings) + ". Les revenus garantis (" + qppLabel + " + PSV" + (D.hasPension ? " + pension" : "") + ") pourraient couvrir " + f$(D.govMonthly) + " par mois" + (mirrorGapEss > 0 ? " \u2014 le portefeuille devrait combler " + f$(mirrorGapEss) + " par mois" : "") + "." + horizonCtx,
       "You are " + D.age + " years old. You target retirement at " + D.retAge + " with an income of " + f$(D.retSpM) + " per month. Your savings total " + f$(D.totalSavings) + ". Guaranteed income (" + qppLabel + " + OAS" + (D.hasPension ? " + pension" : "") + ") could cover " + f$(D.govMonthly) + " per month" + (mirrorGapEss > 0 ? " \u2014 the portfolio would need to fill " + f$(mirrorGapEss) + " per month" : "") + "." + horizonCtx)
    + '</div></div>';
  // C2: Human-readable probability translation (from report-shared.ts)
  ...
  h += '<div style="background:#FDFBF7;border:1px solid #E8E0D0;border-radius:6px;padding:12px 16px;margin:0 0 16px;font-size:13px;color:#555;line-height:1.7">'
    ...
    + t("Ce bilan projette votre trajectoire de retraite à travers " + D.nSim.toLocaleString("fr-CA") + " scénarios économiques différents — rendements variables, inflation, longévité. Le taux de réussite indique dans combien de ces scénarios votre épargne pourrait durer jusqu\u2019à la fin de la retraite.",
       "This assessment projects your retirement trajectory across " + D.nSim.toLocaleString("en-CA") + " different economic scenarios — variable returns, inflation, longevity. The success rate indicates in how many of these scenarios your savings could last through retirement.")
    + '</div>';
  ...
  if (D.medRuin && D.medRuin < 999) {
    var ruinPctEss = Math.round((1 - D.succ) * 100);
    h += '<div style="background:#FDF5F5;border:1px solid #E8C0C0;border-radius:12px;padding:16px 18px;margin:12px 0;">'
      ...
      + '<div style="font-size:12px;color:#666;">' + t("Dans " + ruinPctEss + " % des sc\u00e9narios, l\u2019\u00e9pargne pourrait s\u2019\u00e9puiser vers cet \u00e2ge.", "In " + ruinPctEss + "% of scenarios, savings could deplete around this age.") + '</div></div></div></div>';
  }
  h += '</div>';
  ...
  h += bridge(
    "Ces r\u00e9sultats d\u00e9coulent directement de votre profil \u2014 voici les param\u00e8tres qui les alimentent.",
    "These results flow directly from your profile \u2014 here are the parameters that drive them."
  );
  ...
  h += '</div>';
  if (quizQ.couple === "yes") {
    h += '<div class="co coam" style="margin-top:10px">' + t("<strong>Note :</strong> Ce bilan mod\u00e9lise une seule personne. Une analyse de couple tiendrait compte des revenus combin\u00e9s, du fractionnement du revenu et des strat\u00e9gies conjointes.", "<strong>Note:</strong> This assessment models a single person. A couple analysis would account for combined income, income splitting, and joint strategies.") + '</div>';
  }
  h += '</div>';
  ...
  h += bridge(
    "\u00c0 " + D.age + "\u00a0ans, avec " + f$(totalSav) + " d\u2019\u00e9pargne actuelle, voici l\u2019\u00e9ventail des trajectoires possibles vers la retraite.",
    "At age " + D.age + ", with " + f$(totalSav) + " in current savings, here is the range of possible trajectories toward retirement."
  );
  h += '<div class="kg">';
  ...
  h += bridge(
    "Ce patrimoine projet\u00e9 devra couvrir l'\u00e9cart entre vos revenus gouvernementaux et votre cible de " + f$(D.retSpM) + "/mois.",
    "This projected wealth will need to cover the gap between government income and your target of " + f$(D.retSpM) + "/mo."
  );
  h += '<div class="ai">' + (ai.income_analysis || ai.gov_explanation || t("Les revenus gouvernementaux (" + qppLabel + " + PSV) couvriraient environ " + D.coveragePct + " % de vos d\u00e9penses projet\u00e9es. Le reste proviendrait de votre \u00e9pargne.", "Government income (" + qppLabel + " + OAS) would cover about " + D.coveragePct + "% of your projected spending. The rest would come from your savings.")) + '</div>';
  h += '<div class="c"><div style="display:flex;align-items:center;gap:32px;flex-wrap:wrap">';
  ...
  h += bridge(
    "La structure de votre \u00e9pargne d\u00e9termine la part du d\u00e9ficit qui serait combl\u00e9e par vos placements plut\u00f4t que par les revenus gouvernementaux.",
    "The structure of your savings determines how much of the income gap would be filled from investments rather than government income."
  );
  h += '<div class="ai">' + (ai.savings_analysis || ai.savings_context || t("Votre \u00e9pargne actuelle de " + f$(totalSav) + " pourrait atteindre " + f$(D.retYearBalance) + " \u00e0 " + D.retAge + " ans, selon le sc\u00e9nario m\u00e9dian.", "Your current savings of " + f$(totalSav) + " could grow to " + f$(D.retYearBalance) + " by age " + D.retAge + " in the median scenario.")) + '</div>';
  h += '<div class="c">';
  ...
  else if (!ai.savings_analysis) {
    var savFallback = D.grade === "A+" || D.grade === "A" || D.grade === "B+"
      ? t("Votre taux d\u2019\u00e9pargne de " + savingsRate + " % contribue \u00e0 la solidit\u00e9 de votre plan.", "Your savings rate of " + savingsRate + "% contributes to the strength of your plan.")
      : D.grade === "B" || D.grade === "C+"
      ? t("Votre taux d\u2019\u00e9pargne de " + savingsRate + " % est un point de d\u00e9part. L\u2019\u00e9cart entre votre \u00e9pargne actuelle et l\u2019objectif de retraite reste significatif.", "Your savings rate of " + savingsRate + "% is a starting point. The gap between your current savings and your retirement target remains significant.")
      : t("Votre taux d\u2019\u00e9pargne de " + savingsRate + " % ne suffit pas actuellement \u00e0 combler l\u2019\u00e9cart projet\u00e9. Le d\u00e9ficit d\u2019accumulation est le facteur dominant de votre note.", "Your savings rate of " + savingsRate + "% is not currently sufficient to close the projected gap. The accumulation deficit is the dominant factor in your grade.");
    h += '<div class="ai">' + savFallback + '</div>';
  }
  ...
  if (D.debtBal > 0) {
    ...
    if (ai.debt_analysis) h += '<div class="ai">' + ai.debt_analysis + '</div>';
  }
  ...
  if (priorities && priorities.length) {
    ...
    h += bridge(
      "Votre \u00e9pargne, vos dettes et votre fiscalit\u00e9 d\u00e9terminent l\u2019ordre des leviers ci-dessous \u2014 class\u00e9s par rendement ajust\u00e9 pour le risque.",
      "Your savings, debts, and tax situation determine the order of levers below \u2014 ranked by risk-adjusted return."
    );
    ...
    if (ai.priority_rationale || ai.obs_2) h += '<div class="ai">' + (ai.priority_rationale || ai.obs_2) + '</div>';
    ...
    if (D.debtBal > 0) {
      h += '<a href="https://www.buildfi.ca/outils/dettes" target="_blank" rel="noopener" class="np" style="display:flex;align-items:center;gap:14px;text-decoration:none;background:rgba(204,68,68,.04);border:1px solid rgba(204,68,68,.18);border-left:4px solid var(--rd);border-radius:9px;padding:14px 16px;margin-top:14px;transition:border-color .15s,box-shadow .15s" onmouseover="this.style.borderColor=\'rgba(204,68,68,.4)\';this.style.boxShadow=\'0 2px 8px rgba(204,68,68,.1)\'" onmouseout="this.style.borderColor=\'rgba(204,68,68,.18)\';this.style.boxShadow=\'none\'">';
  ...
  if (!whatIfInsertedEarly && whatIfCardsHTML) {
    ...
    var wiIntro = D.successPct >= 85
      ? t("Votre plan r\u00e9siste dans la quasi-totalit\u00e9 des sc\u00e9narios. Ces cartes montrent comment il r\u00e9agirait \u00e0 des changements de conditions.", "Your plan holds in the vast majority of scenarios. These cards show how it would react to changing conditions.")
      : t("Chaque carte montre l\u2019effet d\u2019un seul changement sur votre taux de r\u00e9ussite.", "Each card shows the effect of a single change on your success rate.");
    sn++;
    h += '<div class="sg" id="sec-renforcer"><div class="sh"><span class="sn">' + sn + '</span>' + wiTitle + '</div>';
    h += '<div style="font-size:14px;color:var(--ts);line-height:1.7;margin-bottom:20px">' + wiIntro + '</div>';
    h += whatIfCardsHTML;
    h += '<div class="ex">' + t("Ces sc\u00e9narios ne sont pas des recommandations. Ils illustrent l\u2019effet math\u00e9matique de chaque changement.", "These are not recommendations. They illustrate the mathematical effect of each change.") + '</div>';
    if (ai.whatif_interpretation) h += '<div class="ai">' + ai.whatif_interpretation + '</div>';
    h += '</div>';
  }

  // ═══ 8. FISCALITÉ & FRAIS ═══
  h += '<details class="c" id="sec-annexes" style="padding:0;margin-top:12px">';
  h += '<summary style="list-style:none;cursor:pointer;padding:16px 18px;display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--gd);text-transform:uppercase;letter-spacing:.8px;border-bottom:1px solid var(--bdl)">';
  h += '<span class="summary-arrow" style="font-size:10px;transition:transform .2s">\u25b6</span>';
  h += t("Annexes techniques (fiscalit\u00e9, \u00e9volution, hypoth\u00e8ses, m\u00e9thodologie)","Technical appendices (taxes, evolution, assumptions, methodology)");
  h += '</summary>';
  h += '<div style="padding:18px 18px 6px">';
  h += '<div class="co cog" style="margin:0 0 18px">' + t("Le rapport principal se termine ci-dessus. Ouvrez cette annexe uniquement si vous souhaitez plus de d\u00e9tails techniques.", "The main report ends above. Open this appendix only if you want more technical detail.") + '</div>';

  sn++;
  h += '<div class="sg" id="sec-fiscalite"><div class="sh"><span class="sn">' + sn + '</span>' + t("Votre fiscalit\u00e9 et vos frais","Your taxes & fees") + '</div>';
  h += bridge(
    "L\u2019\u00e9cart entre votre taux effectif actuel (" + D.taxCurrentEffective.toFixed(1) + "\u00a0%) et votre taux pr\u00e9vu \u00e0 la retraite repr\u00e9sente un levier potentiel.",
    "The gap between your current effective rate (" + D.taxCurrentEffective.toFixed(1) + "\u00a0%) and your projected retirement rate represents a potential lever."
  );
  ...
  h += '</div>';
  if (ai.tax_fees_insight || ai.obs_3) h += '<div class="ai">' + (ai.tax_fees_insight || ai.obs_3) + '</div>';
  // AI slot: succession_note (optional, only for A/A+ with significant residual estate)
  ...
  if (D.snapshot5yr && D.snapshot5yr.length) {
    ...
    var snapshotHasShortfall = D.snapshot5yr.some(function(row) { return row.shortfall > 0; });
    var withdrawalHeader = snapshotHasShortfall ? t("Retrait ou manque","Withdrawal or shortfall") : t("Retrait \u00e9pargne","From savings");
    h += '<div class="c" style="padding:0;overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px">';
    h += '<thead><tr style="background:#F5F0E8;border-bottom:2px solid #E8E0D0">';
    [t("\u00c2ge","Age"),t("Patrimoine","Wealth"),t("Rev. gouv.","Gov. income"),withdrawalHeader,t("D\u00e9penses vis\u00e9es","Target spending")].forEach(function(th){
      h += '<th style="padding:10px 12px;text-align:right;font-weight:600;color:#555;font-size:13px">' + th + '</th>';});
    h += '</tr></thead><tbody>';
    D.snapshot5yr.forEach(function(row, i){
      ...
      var flowCell = row.shortfall > 0
        ? f$(row.revEp) + '/' + t("an","yr") + '<div style="font-size:9px;color:var(--rd);font-weight:600;margin-top:1px">' + t("manque\u00a0: ","shortfall: ") + f$(row.shortfall) + '/' + t("an","yr") + '</div>'
        : f$(row.revEp) + '/' + t("an","yr");
      h += '<td style="padding:10px 12px;text-align:right;font-family:var(--mono);color:' + (row.shortfall > 0 ? 'var(--rd)' : 'var(--am)') + '">' + flowCell + '</td>';
      h += '<td style="padding:10px 12px;text-align:right;font-family:var(--mono)">' + f$(row.dep) + '/' + t("an","yr") + '</td></tr>';});
    h += '</tbody></table></div>';
    if (ai.table_narrative) h += '<div class="ai" style="margin:10px 14px 0">' + ai.table_narrative + '</div>';
    if (snapshotHasShortfall) {
      h += '<div class="co" style="margin:12px 14px 14px;background:rgba(204,68,68,.04);border-left:3px solid var(--rd);padding:10px 14px;border-radius:0 8px 8px 0;font-size:12px;color:#555;line-height:1.7">' + t("Une fois le portefeuille \u00e9puis\u00e9, le manque annuel non financ\u00e9 appara\u00eet en rouge. Les d\u00e9penses vis\u00e9es restent affich\u00e9es pour montrer l\u2019\u00e9cart.", "Once the portfolio is depleted, the unfunded annual shortfall appears in red. Target spending is still shown to illustrate the gap.") + '</div>';
    }
    h += '</div>';
  }
  ...
  if (D.heuristics && D.heuristics.length) {
    ...
    h += '<div style="font-size:14px;color:var(--ts);line-height:1.7;margin-bottom:16px">' + t("Ce bilan utilise des hypoth\u00e8ses raisonnables pour les donn\u00e9es que vous n\u2019avez pas fournies. Chaque param\u00e8tre ci-dessous peut \u00eatre ajust\u00e9.", "This assessment uses reasonable assumptions for data you did not provide. Each parameter below can be adjusted.") + '</div>';
    h += '<div class="c">';
  ...
  // ═══ "CE QUE VOUS AVEZ DIT" — closing loop on qualitative quiz inputs ═══
  (function() {
    var qzz = quizQ || {};
    var objLabels3 = {
    ...
    var wL3 = (qzz.worries || []).map(function(w) {
      return {runout:fr?"Manquer d\u2019argent":"Running out",tax:fr?"Imp\u00f4ts":"Taxes",inflation:"Inflation",health:fr?"Sant\u00e9":"Health",market:fr?"March\u00e9s":"Markets",legacy:fr?"H\u00e9ritage":"Legacy"}[w] || w;
    });
    var rows3 = [];
    if (qzz.objective && objLabels3[qzz.objective]) rows3.push([t("Objectif principal","Primary objective"), objLabels3[qzz.objective]]);
    if (wL3.length > 0) rows3.push([t("Pr\u00e9occupations","Focus areas"), wL3.join(" \u00b7 ")]);
    if (qzz.confidence) rows3.push([t("Niveau financier","Financial knowledge"), qzz.confidence + "/5"]);
    if (rows3.length === 0) return;
    h += '<div class="c" style="margin-bottom:24px;padding:16px 20px">';
    h += '<div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:var(--gd);margin-bottom:12px">' + t("Ce que vous avez dit","What you told us") + '</div>';
    rows3.forEach(function(r) {
      h += '<div class="dr"><span class="dl">' + r[0] + '</span><span class="dv">' + r[1] + '</span></div>';
    });
    h += '<div style="font-size:11px;color:var(--tm);margin-top:10px;font-style:italic">'
      + t("Ces facteurs ont \u00e9t\u00e9 int\u00e9gr\u00e9s dans l\u2019analyse ci-dessus.", "These factors were integrated into the analysis above.")
      + '</div></div>';
  })();

  // ── Closing frame — recontextualization before methodology ─
  var closingGapEss = Math.max(0, D.retSpM - D.govMonthly);
  h += '<div style="background:linear-gradient(135deg,#1A1208 0%,#2D2010 100%);color:#fff;border-radius:14px;padding:28px;margin:24px 0;">'
    ...
    + (D.successPct >= 85
      ...
      : D.successPct >= 50
      ? t("Votre plan affiche un taux de r\u00e9ussite de " + D.successPct + " %. L\u2019\u00e9cart entre le revenu cible et les revenus garantis (" + f$(closingGapEss) + " par mois) repr\u00e9sente la pression principale. Les sections pr\u00e9c\u00e9dentes identifient les leviers qui pourraient influencer ce r\u00e9sultat.",
           "Your plan shows a " + D.successPct + "% success rate. The gap between target income and guaranteed income (" + f$(closingGapEss) + " per month) represents the main pressure. Previous sections identify the levers that could influence this outcome.")
    ...
    + '</div>'
    + (ai.closing_observation ? '<div style="font-size:14px;line-height:1.8;color:#E8E0D4;margin-top:12px;padding-top:12px;border-top:1px solid rgba(196,154,26,.25);font-style:italic">' + ai.closing_observation + '</div>' : '')
    + '</div>';
  ...
  // ═══ BONUS RESOURCES (conditional — only when debt is active) ═══
  if (D.debtBal > 0) {
  h += '<div class="np" style="background:linear-gradient(135deg,rgba(196,154,26,.09) 0%,rgba(212,168,90,.04) 100%);border:1.5px solid rgba(196,154,26,.28);border-radius:12px;padding:20px 22px;margin-bottom:20px">';
  ...
  h += '<a href="https://www.buildfi.ca/outils/dettes" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:14px;text-decoration:none;background:#fff;border:1px solid rgba(196,154,26,.22);border-radius:9px;padding:13px 16px;transition:border-color .15s,box-shadow .15s" onmouseover="this.style.borderColor=\'rgba(196,154,26,.55)\';this.style.boxShadow=\'0 2px 8px rgba(196,154,26,.12)\'" onmouseout="this.style.borderColor=\'rgba(196,154,26,.22)\';this.style.boxShadow=\'none\'">';
  h += '<span style="font-size:24px;flex-shrink:0">\ud83e\uddee</span>';
  ...
  h += t("Connaissez-vous quelqu\u2019un qui gagnerait \u00e0 faire le point sur sa retraite ?", "Know someone who could benefit from taking stock of their retirement?");
  h += '<br><a href="https://www.buildfi.ca/?ref=rapport" style="color:var(--gd);text-decoration:none;font-weight:600">' + t("Partagez buildfi.ca \u2192","Share buildfi.ca \u2192") + '</a>';
  h += '</div>';
  ...
  h += '<svg xmlns="http://www.w3.org/2000/svg" width="140" height="34" viewBox="0 0 220 48"><g><rect x="0" y="32" width="28" height="8" rx="2" fill="#1a2744"/><rect x="4" y="22" width="26" height="8" rx="2" fill="#1a2744" opacity="0.50"/><rect x="8" y="12" width="24" height="8" rx="2" fill="#c49a1a"/></g><text x="40" y="38" font-family="DM Sans,sans-serif" font-size="34" font-weight="700" letter-spacing="-0.5"><tspan fill="#1a2744">build</tspan><tspan fill="#c49a1a">fi</tspan></text></svg>';
...
'<title>buildfi.ca \u2014 ' + (fr ? "Bilan" : "Snapshot") + ' \u2014 ' + date + '</title>' +
...
'  <meta charset="UTF-8">' +
'  <meta name="viewport" content="width=device-width,initial-scale=1">' +
'  <title>buildfi.ca \u2014 ' + (fr ? "Bilan" : "Snapshot") + '</title>' +
...
'      -webkit-print-color-adjust:exact;print-color-adjust:exact}' +
'    :root{' +
'      --g:#c49a1a;--gl:#D4A85A;--gn:#2A8C46;--gnl:#3AA856;--am:#B89830;' +
'      --rd:#CC4444;--bl:#4680C0;--pr:#c49a1a;--tx:#1A1A1A;--ts:#555;--tm:#888;' +
'      --bg:#FEFCF9;--bgc:#fff;--bd:#E8E0D4;--bdl:#F0ECE4;' +
'      --gbg:rgba(196,154,26,.08);--gnbg:rgba(42,140,70,.06);--ambg:rgba(184,152,48,.06);' +
'      --rdbg:rgba(204,68,68,.06);--bga:rgba(196,154,26,.04);' +
'    }' +
...
export { validateMC, extractReportData, buildAIPrompt, buildAIPromptOpus, buildWhatIf };
