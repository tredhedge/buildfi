...
function buildSnapshot5yr(mc, D) {
  try {
    ...
    unique.forEach(function(targetAge) {
      ...
      if (!best) return;
      var bal = Math.round((best.aRR||0) + (best.aTF||0) + (best.aNR||0) + (best.aDC||0));
      // D1: enforce monotonically decreasing patrimoine
      bal = Math.min(bal, prevBal);
      prevBal = bal;
      var revGov = Math.round((best.rrq||0) + (best.psv||0) + (best.pen||0));
      var dep = Math.round((best.spend||0));
...
function gradeInfo(s, fr) {
  ...
  if(s>=.70)return{g:"B",l:t("Fragile","Fragile"),c:"var(--am)"};
  if(s>=.50)return{g:"C",l:t("Risqué","Risky"),c:"var(--am)"};
...
function buildPDfallback(D) {
  ...
  for(var y=0;y<=D.deathAge-D.age;y++){
    ...
    if(!ret){d.push({age:a,rp5:accum*0.7,rp25:accum*0.85,rp50:accum,rp75:accum*1.15,rp95:accum*1.4});continue;}
...
function renderReport_v6(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  var rp = params._report || {};
  var priorities = buildPriority(D, params, fr);
  var hasDC = D.hasDC || D.penType === "dc";
  ...
  // ═══ 1. NOTE ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Votre note de retraite","Your retirement grade") + '</div>';
  ...
  h += '</div></div></div>';
  h += '<div class="co cog">' + t("Tous les montants sont en <strong>dollars d\u2019aujourd\u2019hui</strong> \u2014 ajust\u00e9s pour l\u2019inflation projet\u00e9e \u00e0 " + (D.inf*100).toFixed(1) + " % par ann\u00e9e.", "All amounts are in <strong>today\u2019s dollars</strong> \u2014 adjusted for projected inflation of " + (D.inf*100).toFixed(1) + "% per year.") + '</div></div>';

  // ═══ 2. PROFIL ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Votre profil","Your profile") + '</div>';
  ...
  // ═══ 3. PROJECTION ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Projection de votre patrimoine","Your wealth projection") + '</div>';
  h += '<div class="kg">';
  h += '<div class="kp krd"><div class="kv">' + f$(D.rP5F) + '</div><div class="kl">' + t("Pessimiste","Pessimistic") + '</div><div class="ks">' + t("Dans 5 % des sc\u00e9narios les plus difficiles","In the most difficult 5% of scenarios") + '</div></div>';
  ...
  // ═══ 4. REVENUS ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Vos revenus \u00e0 la retraite","Your retirement income") + '</div>';
  ...
  // ═══ 5. ÉPARGNE ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Votre \u00e9pargne","Your savings") + '</div>';
  ...
  var acctRows = [
    [t("REER","RRSP"), D.rrsp, params.rrspC||0],
    [t("C\u00c9LI","TFSA"), D.tfsa, params.tfsaC||0],
    [t("Non enregistr\u00e9","Non-reg."), D.nr, params.nrC||0]
  ];
  ...
  if (priorities && priorities.length) {
    sn++;
    h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Comparatif \u2014 impact financier par cat\u00e9gorie","Comparative \u2014 financial impact by category") + '</div>';
    h += '<div style="font-size:14px;color:var(--ts);line-height:1.7;margin-bottom:16px">' + t("Bas\u00e9 sur votre taux marginal de " + D.taxCurrentMarginal.toFixed(1) + " %, vos dettes et votre type d\u2019employeur.", "Based on your " + D.taxCurrentMarginal.toFixed(1) + "% marginal rate, debts, and employer type.") + '</div>';
    priorities.forEach(function(p, i) {
      h += '<div style="display:flex;align-items:center;gap:12px;padding:14px 16px;border-bottom:1px solid var(--bdl)">';
      h += '<span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:' + p.color + ';color:#fff;font-size:12px;font-weight:800;flex-shrink:0">' + (i+1) + '</span>';
      h += '<div style="flex:1"><div style="font-weight:700;font-size:15px">' + p.name + '</div><div style="font-size:12px;color:var(--ts);margin-top:2px">' + p.why + '</div></div>';
  ...
  // ═══ 8. FISCALITÉ & FRAIS ═══
  sn++;
  h += '<div class="sg"><div class="sh"><span class="sn">' + sn + '</span>' + t("Votre fiscalit\u00e9 et vos frais","Your taxes & fees") + '</div>';
  h += '<div class="c"><div class="g2" style="margin-bottom:16px">';
  h += '<div style="text-align:center;padding:16px;border-radius:8px;background:var(--gbg)"><div style="font-size:12px;color:var(--ts);margin-bottom:4px">' + t("Taux effectif actuel","Current effective rate") + '</div><div class="mono" style="font-size:24px;color:var(--gd)">' + D.taxCurrentEffective.toFixed(1) + ' %</div></div>';
  ...
  if (D.snapshot5yr && D.snapshot5yr.length) {
    ...
    D.snapshot5yr.forEach(function(row, i){
      var bg = i % 2 === 0 ? 'transparent' : 'var(--gbg)';
      var balColor = row.bal > 300000 ? 'var(--gn)' : row.bal > 100000 ? 'var(--am)' : 'var(--rd)';
      h += '<tr style="background:' + bg + ';border-bottom:1px solid var(--bdl)">';
      h += '<td style="padding:10px 12px;text-align:right;font-weight:700">' + row.age + ' ' + t("ans","yr") + (row.age === D.retAge ? ' <span style="font-size:9px;color:var(--gd)">' + t("(retraite)","(ret.)") + '</span>' : '') + '</td>';
  ...
  h += '<div style="background:var(--rdbg);border:1px solid #E8C8C8;border-radius:var(--rad);padding:24px;font-size:13px;color:#666;line-height:1.85;text-align:center;margin-bottom:28px">';
  h += '<strong style="color:var(--rd);font-size:14px;display:block;margin-bottom:8px">' + t("AVERTISSEMENT IMPORTANT","IMPORTANT DISCLAIMER") + '</strong>';
  ...
  h += '<div class="c" style="margin-bottom:16px;padding:18px"><div style="font-size:14px;font-weight:700;color:var(--ts);margin-bottom:10px">' + t("Ce que contient le rapport Interm\u00e9diaire :","What the Intermediate report includes:") + '</div>';
  h += '<div style="font-size:13px;color:var(--ts);line-height:2">\u2022 ' + t("16 sections sp\u00e9cialis\u00e9es d\u2019analyse","16 specialized analysis sections") + '<br>\u2022 ' + t("Strat\u00e9gie de d\u00e9caissement optimale","Optimal decumulation strategy") + '<br>\u2022 ' + t("Report du " + qppLabel + " \u00e0 70 ans (+42 %)","Deferring " + qppLabel + " to age 70 (+42%)") + '<br>\u2022 ' + t("Simulation de sc\u00e9narios personnalis\u00e9s","Custom scenario simulation") + '</div></div>';
...
'  --am:#B89830;--ambg:rgba(184,152,48,.06);' +
