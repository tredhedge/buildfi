...
function renderReport(D, mc, stratData, params, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  h += '<div style="text-align:center;margin-bottom:32px;padding:28px 20px;background:linear-gradient(135deg,#FFF8ED,#FDF3E0);border-radius:16px;border:1px solid rgba(196,148,74,.2)">'
    ...
    + '<div style="font-size:13px;color:var(--ts);margin-bottom:4px">' + t("Patrimoine estim\u00e9 \u00e0 la retraite", "Est. wealth at retirement") + ': <strong>' + f$(D.retBal) + '</strong></div>'
  ...
  // ═══ S1: Dashboard ═══
  h += secH("1", t("Tableau de bord", "Dashboard"), t("11 indicateurs cl\u00e9s de votre plan", "11 key indicators of your plan"));
  ...
  h += kp(f$(D.rMedF), t("Patrimoine r\u00e9siduel m\u00e9dian", "Median residual wealth"), D.rMedF > 0 ? "var(--gn)" : "var(--rd)", t("\u00c0 " + D.avgDeath + " ans (fin d'horizon)", "At age " + D.avgDeath + " (end of horizon)"));
  ...
  if (D.oasMonthly > 0 && D.oasMonthly < 668) {
    h += '<div style="margin-top:8px;padding:10px 12px;background:var(--ambg);border-radius:8px;font-size:12px;color:var(--am);line-height:1.6">'
      + t("PSV partielle \u2014 les revenus de retraite projet\u00e9s pourraient d\u00e9passer le seuil de r\u00e9cup\u00e9ration (~93 000 $/an en 2024), r\u00e9duisant la prestation de base.", "Partial OAS \u2014 projected retirement income may exceed the clawback threshold (~$93,000/yr in 2024), reducing the base benefit.")
      + '</div>';
  }
  ...
  if (costDelay > 0 && !(D.retAge - D.age <= 5 && D.coveragePct >= 90)) {
  ...
  if (stratData && stratData.length === 5) {
    ...
    stratData.forEach(function(s, i) {
      ...
      var diff = i === 0 ? "\u2014" : ((ppDelta >= 0 ? "+" : "") + ppDelta + "pp");
      ...
      h += '<tr style="border-bottom:1px solid var(--bdl);background:' + (i === 0 ? "var(--gbg)" : i % 2 === 0 ? "#FAFAF8" : "var(--bgc)") + '">'
        ...
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;font-weight:600">' + f$(Math.max(0, s.medEstateNet)) + '</td></tr>';
    });
    h += '</tbody></table></div>';
    ...
    if (worstDelta <= -5) {
      h += '<div style="font-size:12px;padding:10px 14px;background:#FFF8F0;border-left:3px solid var(--am);border-radius:6px;margin-top:10px;color:var(--ts)">'
