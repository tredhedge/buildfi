...
function renderReport(D, mc, stratData, params, ai, aiLoading, lang, costDelay, minReturn, obsLabels) {
  ...
  var logoSVG = '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="48" viewBox="0 0 220 48">'
    ...
    + '</g>'
    + '<text x="40" y="38" font-family="\'DM Sans\',sans-serif" font-size="34" font-weight="700" letter-spacing="-0.5">'
  ...
  var tn10 = null; if ((params._report || {}).homeVal > 0 || ((params._report || {}).rentals || []).length > 0 || (params._report || {}).hasRental) { tn10 = tocN(); tocItems.push({ n: tn10, l: t("Immobilier", "Real estate") }); }
  ...
  var _rp = params._report || {};
  var _rentals = _rp.rentals || [];
  if (_rp.homeVal > 0 || _rentals.length > 0 || _rp.hasRental) {
    h += secH(nextSecInter(), t("Analyse immobili\u00e8re", "Real estate analysis"), t("R\u00e9sidence, biens locatifs et strat\u00e9gie", "Residence, rental properties and strategy"));

    // Primary residence card
    if (_rp.homeVal > 0) {
      var projHV = Math.round(_rp.homeVal * Math.pow(1.035, D.retAge - D.age));
      var projMB = _rp.mortBal > 0 && _rp.mortPayment > 0 ? (function() {
        var b = _rp.mortBal, mp = _rp.mortPayment, mr = 0.055 / 12;
        var yrs = D.retAge - D.age;
        for (var i = 0; i < yrs * 12 && b > 0; i++) { b = b * (1 + mr) - mp; } return Math.max(0, Math.round(b));
      })() : 0;
      h += card(
        kvr(t("Valeur estim\u00e9e actuelle", "Est. current value"), f$(_rp.homeVal))
        + kvr(t("Solde hypoth\u00e9caire", "Mortgage balance"), f$(_rp.mortBal))
        + kvr(t("\u00c9quit\u00e9 nette actuelle", "Current net equity"), f$(_rp.equity))
        + (_rp.mortPayment > 0 ? kvr(t("Paiement mensuel estim\u00e9", "Est. monthly payment"), f$(_rp.mortPayment)) : "")
        + (_rp.mortFreeAge > 0 ? kvr(t("Fin de l'hypoth\u00e8que", "Mortgage-free age"), _rp.mortFreeAge + " " + t("ans", "yrs")) : "")
    ...
    // Per-property rental cards (new multi-rental format)
    if (_rentals.length > 0) {
      h += '<div style="margin-top:18px"><div style="font-size:12px;font-weight:800;color:var(--g);text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px">'
        + t("Biens locatifs", "Rental properties") + '</div>';
      _rentals.forEach(function(r, idx) {
        var prjRV = Math.round(r.value * Math.pow(1.035, D.retAge - D.age));
        h += '<div style="background:var(--bgc);border:1px solid var(--bd);border-radius:10px;padding:14px 16px;margin-bottom:10px">'
          + '<div style="font-size:13px;font-weight:700;color:var(--tx);margin-bottom:8px">'
          + escHtml(r.name || (t("Bien locatif #", "Rental #") + (idx + 1))) + '</div>'
          + kvr(t("Valeur estim\u00e9e", "Est. value"), f$(r.value))
          + kvr(t("Solde hypoth\u00e9caire", "Mortgage"), f$(r.mortgage))
          + kvr(t("\u00c9quit\u00e9 nette", "Net equity"), f$(r.equity))
          + kvr(t("Revenu brut annuel", "Annual gross income"), f$(r.income))
          + kvr(t("Charges annuelles", "Annual expenses"), f$(r.expenses))
          + kvr(t("Cash-flow net", "Net cash-flow"), f$(r.cashFlow) + t("/an", "/yr"))
          + kvr(t("Valeur projet\u00e9e \u00e0 la retraite", "Projected value at retirement"), f$(prjRV))
          + '</div>';
      });
      // Aggregate row (only if 2+ properties)
      if (_rentals.length >= 2) {
        var _tV = 0, _tM = 0, _tI = 0, _tE = 0;
        _rentals.forEach(function(r) { _tV += r.value; _tM += r.mortgage; _tI += r.income; _tE += r.expenses; });
        h += '<div style="background:var(--gbg);border:1px solid rgba(196,154,26,.15);border-radius:10px;padding:14px 16px;margin-top:6px">'
          + '<div style="font-size:12px;font-weight:800;color:var(--g);margin-bottom:6px">'
          + t("Total \u2014 " + _rentals.length + " biens locatifs", "Total \u2014 " + _rentals.length + " rental properties") + '</div>'
          + kvr(t("Valeur totale", "Total value"), f$(_tV))
          + kvr(t("Hypoth\u00e8ques totales", "Total mortgages"), f$(_tM))
          + kvr(t("\u00c9quit\u00e9 nette totale", "Total net equity"), f$(_tV - _tM))
          + kvr(t("Cash-flow net total", "Total net cash-flow"), f$(_tI - _tE) + t("/an", "/yr"))
          + '</div>';
      }
      h += '</div>';
    } else if (_rp.hasRental && (_rp.rentalIncome || _rp.rentalExpenses)) {
      // Backward compat: old scalar format (pre-multi-rental)
      h += card(
        kvr(t("Revenu locatif brut annuel", "Annual gross rental income"), f$(_rp.rentalIncome || 0))
        + kvr(t("Cash-flow locatif net estim\u00e9", "Est. net rental cash-flow"), f$((_rp.rentalIncome || 0) - (_rp.rentalExpenses || 0)))
      );
    }
  ...
  // Contextual next step (soft, no pricing)
  h += '<div class="no-print" style="margin-top:24px;padding:16px 20px;border:1px solid var(--bd);border-radius:var(--rad);font-size:13px;color:var(--ts);line-height:1.8;text-align:center">'
    + t("Ce bilan couvre 16 sections. Pour tester vos propres sc\u00e9narios en temps r\u00e9el — RESP, incorporation, conversion REER-FERR — le simulateur interactif est disponible sur ", "This report covers 16 sections. To test your own scenarios in real time — RESP, incorporation, RRSP-RRIF conversion — the interactive simulator is available at ")
    + '<a href="https://www.buildfi.ca/expert/landing" style="color:var(--g);text-decoration:none;font-weight:600">buildfi.ca</a>'
    + '</div>';
