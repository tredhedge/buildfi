...
export function extractReportDataInter(mc, params) {
  ...
  // Grade mapping (Intermediaire scale — 10 levels)
  var grade;
...
function buildPriority(D, p, fr) {
  ...
  (p._report.debts || []).filter(function(d) { return d.rate >= 0.10; }).forEach(function(d) {
    ...
    priorities.push({ name: (tl[d.type] || d.type) + " (" + Math.round(d.bal).toLocaleString() + " $)",
      ...
      why: fr ? "Taux de " + pct(d.rate) + " garanti. Le remboursement est g\u00e9n\u00e9ralement prioris\u00e9." : "Guaranteed " + pct(d.rate) + " rate. Repayment is often favoured.",
...
function renderReport(D, mc, stratData, params, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  h += '<div style="background:linear-gradient(135deg,#1A1208,#2C1F0A);border-radius:16px;padding:32px 28px;margin-top:24px;color:#fff;position:relative;overflow:hidden">'
    ...
    + '<div><span style="font-size:36px;font-weight:900;color:var(--g)">139 $</span>'
...
// ══════════════════════════════════════════════════════════════════════
export function renderReportHTMLInter(D, mc, stratData, params, lang, ai, costDelay, minReturn) {
  var fr = lang === "fr";
