...
export function extractReportDataInter(mc, params) {
  ...
  var qppM = Math.round((qppRow.rrq || 0) / 12);
  var oasM = Math.round((oasRow.psv || 0) / 12);
  ...
  var margRate = taxInfo ? taxInfo.marg / 100 : 0.30;
  ...
  // Grade — canonical 8-level scale (matches api-helpers.ts gradeFromSuccess)
  var grade;
  ...
  // Build 5-yr projection table with smoothstep interpolation
  var projTable = [];
  var finalP25 = mc.p25F || 0, finalP50 = mc.medF || 0, finalP75 = mc.p75F || 0;
  var horizYrs = Math.max(1, p.deathAge - p.retAge);
  for (var tAge = p.retAge; tAge <= p.deathAge; tAge += 5) {
    ...
    var netInc = Math.round((mr5.rrq || 0) + (mr5.psv || 0) + (mr5.pen || 0) + (mr5.ret || 0) + (mr5.ret2 || 0));
    var tax5 = Math.round(mr5.tax || 0);
    var tRatio = Math.max(0, Math.min(1, (tAge - p.retAge) / horizYrs));
    var smooth = tRatio * tRatio * (3 - 2 * tRatio);
    projTable.push({
...
function buildPriority(D, p, fr) {
  ...
  if (margRate > 0.4) {
    ...
    priorities.push({ name: fr ? "Compte d'\u00e9pargne libre d'imp\u00f4t (CELI)" : "Tax-Free Savings Account (TFSA)",
      rate: pct(expRet), impact: Math.round(7000 * Math.pow(1 + expRet, p.deathAge - p.retAge) - 7000),
      why: fr ? "Taux marginal \u2264 40%. CELI souvent prioris\u00e9." : "Marginal rate \u2264 40%. TFSA often favoured.",
...
function fPct(n) { if (n == null || isNaN(n)) return "\u2014"; return Math.round(n) + "%"; }
function gCol(s) { return s >= 0.9 ? "#2A8C46" : s >= 0.75 ? "#B89830" : "#CC4444"; }

// ============================================================
// INTERNAL renderReport (port from source lines 3427-4187)
// ============================================================
function renderReport(D, mc, stratData, params, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  function benchBar(label, userVal, q1, q2, q3, format, note) {
    ...
    return '<div style="margin-bottom:16px">'
      ...
      + '<span>P25: ' + fmtFn(q1) + '</span><span style="font-weight:700;color:var(--ts)">M\u00e9diane: ' + fmtFn(q2) + '</span><span>P75: ' + fmtFn(q3) + '</span></div>'
      + (note ? '<div style="font-size:10px;color:' + barCol + ';font-weight:600;margin-top:3px">\u2192 ' + posLabel + '</div>' : '')
      + '</div>';
  }
  function aiSlot(text, fallback) {
    if (aiLoading) return '<div style="font-size:13px;color:var(--pr);padding:12px 16px;background:var(--bga);border-radius:8px;border-left:3px solid var(--pr);font-style:italic;margin:10px 0">\u2026</div>';
    if (!text) return fallback ? '<div style="font-size:13px;color:var(--tm);line-height:1.7;margin:10px 0;font-style:italic">' + fallback + '</div>' : '';
    return '<div style="font-size:14px;color:#333;line-height:1.8;margin:12px 0">' + text + '</div>';
  }
  ...
  function card(inner, s) { return '<div style="background:var(--bgc);border:1px solid var(--bd);border-radius:12px;padding:20px;margin-bottom:14px;box-shadow:0 1px 3px rgba(0,0,0,.03);' + (s || '') + '">' + inner + '</div>'; }
  function kp(v, l, c, sub) { return '<div style="text-align:center;padding:18px 12px 14px;border:1px solid var(--bd);border-radius:12px;background:var(--bgc);border-top:3px solid ' + (c || 'var(--g)') + '"><div style="font-family:monospace;font-size:20px;font-weight:800;color:' + (c || 'var(--g)') + '">' + v + '</div><div style="font-size:11px;color:#666;margin-top:5px;font-weight:600;line-height:1.3">' + l + '</div>' + (sub ? '<div style="font-size:10px;color:#999;margin-top:2px">' + sub + '</div>' : '') + '</div>'; }
  function kvr(k, v) { return '<div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--bdl);font-size:13px"><span style="color:var(--ts)">' + k + '</span><span style="font-family:monospace;font-weight:600">' + v + '</span></div>'; }
  function obs(title, text, type) {
    ...
    return '<div style="background:' + c[0] + ';border:1px solid ' + c[1] + ';border-left:4px solid ' + c[1] + ';border-radius:8px;padding:14px 16px;margin:10px 0">'
      + '<div style="font-size:12px;font-weight:700;color:' + c[1] + ';margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px">' + title + '</div>'
      + '<div style="font-size:13px;color:#333;line-height:1.75">' + text + '</div></div>';
  }
  ...
  // ═══ HEADER ═══
  h += '<div style="max-width:760px;margin:0 auto;padding:0 20px 60px">';
  h += '<div style="text-align:center;margin-bottom:32px;padding:28px 20px;background:linear-gradient(135deg,#FFF8ED,#FDF3E0);border-radius:16px;border:1px solid rgba(196,148,74,.2)">'
    + '<div style="font-size:11px;font-weight:700;color:var(--tm);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">buildfi.ca \u2014 ' + t("Rapport Interm\u00e9diaire", "Intermediate Report") + '</div>'
    + '<div style="font-size:10px;color:var(--tm);margin-bottom:12px">v1 \u2014 ' + (D.nSim || 5000).toLocaleString() + ' simulations \u2014 ' + D.prov + ' \u2014 ' + new Date().toLocaleDateString(fr ? "fr-CA" : "en-CA") + '</div>'
    + '<div style="display:flex;justify-content:center;gap:32px;align-items:center;flex-wrap:wrap">'
    + '<svg width="120" height="120" viewBox="0 0 120 120"><circle cx="60" cy="60" r="48" fill="none" stroke="var(--bdl)" stroke-width="10"/><circle cx="60" cy="60" r="48" fill="none" stroke="' + sC + '" stroke-width="10" stroke-dasharray="' + Math.round(circ) + '" stroke-dashoffset="' + Math.round(dashVal) + '" stroke-linecap="round" transform="rotate(-90 60 60)"/><text x="60" y="53" text-anchor="middle" font-size="26" font-weight="800" fill="' + sC + '" font-family="DM Sans,sans-serif">' + D.grade + '</text><text x="60" y="70" text-anchor="middle" font-size="12" fill="var(--ts)" font-family="DM Sans,sans-serif">' + D.successPct + '%</text></svg>'
    + '<div style="text-align:left">'
    + '<div style="font-size:13px;font-weight:700;margin-bottom:8px;color:var(--ts)">' + t("Probabilit\u00e9 de succ\u00e8s", "Success probability") + ': <span style="color:' + sC + ';font-size:20px">' + D.successPct + '%</span></div>'
    + '<div style="font-size:13px;color:var(--ts);margin-bottom:4px">' + t("Patrimoine m\u00e9dian estim\u00e9 (retraite)", "Est. median wealth at retirement") + ': <strong>' + f$(D.rMedF) + '</strong></div>'
    + '<div style="font-size:13px;color:var(--ts);margin-bottom:4px">' + t("Revenu cible", "Target income") + ': <strong>' + f$(D.retSpM) + t("/mois", "/mo") + '</strong></div>'
    + '<div style="font-size:13px;color:var(--ts)">' + t("Retraite cible", "Target retirement") + ': <strong>' + D.retAge + '</strong></div>'
    + '</div></div></div>';
  h += aiSlot(ai.snapshot_intro);

  // ═══ S1: Dashboard ═══
  h += secH("1", t("Tableau de bord", "Dashboard"), t("10 indicateurs cl\u00e9s de votre plan", "10 key indicators of your plan"));
  h += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px" class="rpt-grid2">';
  h += kp(f$(totalLiq), t("Patrimoine liquide total", "Total liquid wealth"), "var(--g)");
  h += kp(f$(D.rMedF), t("Patrimoine m\u00e9dian (retraite)", "Median wealth at retirement"), "var(--g)");
  ...
  h += kp(D.avgDeath + " " + t("ans", "yrs"), t("Esp\u00e9rance de vie MC", "MC life expectancy"), "var(--bl)", t("Tables CPM-2014", "CPM-2014 tables"));
  h += kp(mc.p5Ruin >= 999 ? t("Aucun", "None") : (mc.p5Ruin || "\u2014") + " " + t("ans", "yrs"), t("\u00c2ge de d\u00e9tresse P5", "P5 distress age"), (!mc.p5Ruin || mc.p5Ruin >= 999) ? "var(--gn)" : mc.p5Ruin > (D.deathAge - 10) ? "var(--am)" : "var(--rd)", t("Sc\u00e9nario pessimiste (5%)", "Pessimistic scenario (5%)"));
  h += '</div>';
  ...
  h += card(
    ...
    + '<div style="font-size:11px;color:var(--tm);margin-bottom:14px">' + t("Source : StatCan ESF 2023, dollars 2026. Groupe d\'age", "Source: StatCan SFS 2023, 2026 dollars. Age group") + ' <strong>' + bGroup + '</strong> \u2014 ' + t("Canadiens avec revenus similaires", "Canadians with similar income") + '.</div>'
    ...
    + (bMul ? '<div style="margin-top:10px;padding:10px 12px;background:var(--gbg);border-radius:8px;font-size:12px">'
      ...
      + '</div></div>' : '')
    + (ai.benchmark_context ? aiSlot(ai.benchmark_context) : "")
  );
  h += secEnd();

  // ═══ S2: Profile & Trajectory ═══
  h += secH("2", t("Votre profil et trajectoire", "Your profile and trajectory"));
  h += card(
  ...
  (function() {
    ...
    var svg = '<div style="overflow-x:auto"><svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;min-width:400px;height:auto;margin-top:8px">';
    ...
    var markers = [
      ...
      { age: D.oasAge !== D.qppAge ? D.oasAge : null, label: "PSV " + D.oasAge, c: "var(--bl)", r: 5, above: false }
  ...
  h += secH("3", t("\u00c9volution de votre \u00e9pargne", "Your savings trajectory"), t("Projection du patrimoine jusqu'\u00e0 la retraite", "Projected wealth to retirement"));
  (function() {
    var rd = D.medRevData || [];
    ...
    svg += '<polygon points="' + stackPts(rrTfNrLV, rrTfNrV) + '" fill="rgba(124,96,184,0.3)"/>';
    ...
    svg += '<polygon points="' + pts(rrV) + ' ' + P + ',' + (PT + CH) + '" fill="rgba(196,148,74,0.45)"/>';
    ...
    var legX = P + 4, legItems = [{ c: "rgba(196,148,74,0.55)", l: "REER" }, { c: "rgba(42,140,70,0.45)", l: "CELI" }, { c: "rgba(184,152,48,0.45)", l: t("NR", "NR") }, { c: "rgba(124,96,184,0.4)", l: "CRI" }, { c: "rgba(70,128,192,0.4)", l: "DC/Corp" }];
  ...
  h += secH("4", t("Revenus \u00e0 la retraite", "Retirement income"), t("Sources et couverture", "Sources and coverage"));
  (function() {
    ...
    var items = [{ v: qppMo, c: "#2A8C46", l: isQC ? "RRQ" : "RPC" }, { v: oasMo, c: "#4680C0", l: "PSV/OAS" }, { v: penM, c: "#7C60B8", l: t("Pension PD", "DB Pension") }, { v: savM, c: "#C4944A", l: t("\u00c9pargne", "Savings") }].filter(function(i) { return i.v > 0; });
    ...
    h += card('<div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap" class="rpt-donut">'
      ...
      + items.map(function(item) { return '<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--bdl)">'
        ...
        + '<span style="font-family:monospace;font-size:13px;font-weight:700">' + f$(item.v) + t("/mois", "/mo") + '</span>'
  ...
  h += aiSlot(ai.income_mix);
  h += secEnd();

  // ═══ S5: Tax Anatomy ═══
  h += secH("5", t("Anatomie fiscale", "Tax anatomy"), t("Taux effectifs et strat\u00e9gie", "Effective rates and strategy"));
  h += card(
  ...
  h += aiSlot(ai.tax_context);
  h += secEnd();

  // ═══ S6: Wealth Longevity ═══
  h += secH("6", t("Long\u00e9vit\u00e9 du patrimoine", "Wealth longevity"), t("Distribution des 5 000 sc\u00e9narios", "Distribution of 5,000 scenarios"));
  (function() {
    var rd2 = D.medRevData || [];
    var ages2 = []; for (var a2 = D.retAge; a2 <= D.deathAge; a2++) ages2.push(a2);
    var n2 = ages2.length, start2 = D.retBal || 0;
    var W3 = 700, H3 = 240, P3 = 50, PR3 = 12, PT3 = 22, PB3 = 30, CW3 = W3 - P3 - PR3, CH3 = H3 - PT3 - PB3;
    var ends2 = { p5: D.rP5F, p25: D.rP25F, p50: D.rMedF, p75: D.rP75F, p95: D.rP95F };
    var mx2 = Math.max(start2, ends2.p95 || 0, (ends2.p75 || 0) * 1.5, 100000) * 1.05;
    var curve2 = function(endV) { return ages2.map(function(_, i) { var tt2 = i / (n2 > 1 ? n2 - 1 : 1), k2 = tt2 * tt2 * (3 - 2 * tt2); return Math.max(0, start2 + (endV - start2) * k2); }); };
    var cp95 = curve2(ends2.p95), cp75 = curve2(ends2.p75), cp50 = curve2(ends2.p50), cp25 = curve2(ends2.p25), cp5 = curve2(ends2.p5);
    var sx3 = function(i) { return P3 + i / (n2 > 1 ? n2 - 1 : 1) * CW3; };
    var sy3 = function(v) { return PT3 + CH3 - Math.min(v, mx2) / mx2 * CH3; };
    var pts3 = function(arr) { return arr.map(function(v, i) { return sx3(i) + "," + sy3(v); }).join(" "); };
    var svg3 = '<svg viewBox="0 0 ' + W3 + ' ' + H3 + '" style="width:100%;height:auto">';
    for (var g2 = 0; g2 <= 4; g2++) { var yy2 = PT3 + CH3 - CH3 * g2 / 4; svg3 += '<line x1="' + P3 + '" x2="' + (W3 - PR3) + '" y1="' + yy2 + '" y2="' + yy2 + '" stroke="#E8E0D4" stroke-width="0.5"/><text x="' + (P3 - 5) + '" y="' + (yy2 + 3) + '" text-anchor="end" font-size="9" fill="#aaa" font-family="monospace">' + Math.round(mx2 * g2 / 4 / 1000) + 'K</text>'; }
    svg3 += '<polygon points="' + pts3(cp95) + ' ' + pts3(cp5.slice().reverse()) + '" fill="rgba(196,148,74,0.08)"/>';
    svg3 += '<polygon points="' + pts3(cp75) + ' ' + pts3(cp25.slice().reverse()) + '" fill="rgba(196,148,74,0.22)"/>';
    svg3 += '<polyline points="' + pts3(cp50) + '" fill="none" stroke="var(--g)" stroke-width="2.5" stroke-linejoin="round"/>';
    ages2.forEach(function(a3, i) { if (i % 5 === 0 || i === n2 - 1) svg3 += '<text x="' + sx3(i) + '" y="' + (H3 - 8) + '" text-anchor="middle" font-size="8" fill="#888" font-family="monospace">' + a3 + '</text>'; });
    svg3 += '</svg>';
    h += card(svg3);
    ...
    stress.forEach(function(s) {
      h += '<div style="background:var(--rdbg);border:1px solid rgba(204,68,68,.15);border-radius:10px;padding:14px">'
        ...
        + '<div style="font-family:monospace;font-size:16px;font-weight:800;color:var(--rd)">' + s.delta + '%</div></div>';
    });
    h += '</div>';
  })();
  h += aiSlot(ai.longevity_risk);
  ...
  (function() {
    ...
    h += card(
      ...
      + thermoSvg
      + '<div style="flex:1">'
      + '<div style="font-size:32px;font-weight:900;color:' + seqLevel.c + ';font-family:monospace;margin-bottom:4px">' + seqScore + '<span style="font-size:14px">/100</span></div>'
      ...
      + [
        ...
        return '<div style="background:var(--bgc);border:1px solid var(--bd);border-radius:8px;padding:10px;text-align:center">'
          ...
          + '<div style="font-size:18px;font-weight:800;color:' + cc.c + ';font-family:monospace">' + cc.v + '</div>'
          + '<div style="font-size:10px;color:var(--tm);margin-top:2px">' + cc.sub + '</div></div>';
      }).join("")
      + '</div>'
      + (seqScore >= 40 ? '<div style="margin-top:12px;padding:10px 12px;background:var(--ambg);border-radius:8px;font-size:12px;color:var(--am);font-weight:600">'
        + t("Strat\u00e9gies : buffer 2 ans en liquidit\u00e9s, glide path vers 60% actions \u00e0 la retraite, flexibilit\u00e9 du taux de retrait.", "Strategies: 2-year cash buffer, glide path to 60% equity at retirement, flexible withdrawal rate.")
        + '</div>' : '')
  ...
  // ═══ S7: Observations ═══
  h += secH("7", t("Observations personnalis\u00e9es", "Personalized observations"), t("5 analyses adapt\u00e9es \u00e0 votre profil", "5 analyses adapted to your profile"));
  var obsMap = [{ k: "obs_1", ty: "insight", tl: t("Levier principal", "Primary lever") }, { k: "obs_2", ty: "info", tl: t("Couverture gouvernementale", "Government coverage") }, { k: "obs_3", ty: "watch", tl: t("Impact des frais", "Fee impact") }, { k: "obs_4", ty: "info", tl: t("Analyse contextuelle", "Contextual analysis") }, { k: "obs_5", ty: "watch", tl: t("Risque et long\u00e9vit\u00e9", "Risk & longevity") }];
  var obsFall = {
    ...
    obs_2: fr ? "" + gP + " et " + oN + " couvrent " + D.coveragePct + "% de votre revenu cible." : gP + " and " + oN + " cover " + D.coveragePct + "% of your target income.",
    obs_3: fr ? "Les frais de gestion estim\u00e9s repr\u00e9sentent " + f$(D.feeCostLifetime) + " sur votre horizon." : "Estimated management fees represent " + f$(D.feeCostLifetime) + " over your horizon.",
    obs_4: fr ? "Votre profil a \u00e9t\u00e9 int\u00e9gr\u00e9 dans les 5 000 simulations Monte Carlo." : "Your profile was integrated into the 5,000 Monte Carlo simulations.",
  ...
  obsMap.forEach(function(o) { h += obs(o.tl, ai[o.k] || obsFall[o.k], o.ty); });
  h += secEnd();

  // ═══ S8: Priority Cascade ═══
  h += secH("8", t("Cascade de priorit\u00e9s", "Priority cascade"), t("Class\u00e9 par impact financier estim\u00e9", "Ranked by estimated financial impact"));
  ...
  if (costDelay > 0) {
    ...
    h += card(
      ...
      + '<div style="font-size:30px;font-weight:900;font-family:monospace;color:' + cdColor + ';margin-bottom:8px">' + f$(costDelay) + '</div>'
      ...
      + '<strong>' + t("Ce que cela signifie :", "What this means:") + '</strong> ' + t(
        "Chaque ann\u00e9e d'attente co\u00fbte davantage que la pr\u00e9c\u00e9dente, car la croissance compos\u00e9e travaille contre vous. La meilleure action n'est pas n\u00e9cessairement la plus grosse \u2014 c'est la plus rapide.",
        "Each year of waiting costs more than the last, because compound growth works against you. The best action isn't necessarily the biggest \u2014 it's the fastest."
      ) + '</div></div></div>',
  ...
  if (priorities.length > 0) {
    ...
    h += card(priorities.map(function(pr, i) {
      var bw = pr.impact ? Math.max(4, Math.round(pr.impact / maxImp * 100)) : 4;
      return '<div style="margin-bottom:16px">'
        + '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">'
        + '<span style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:' + pr.color + ';color:#fff;font-size:10px;font-weight:800;flex-shrink:0">' + (i + 1) + '</span>'
        + '<span style="font-weight:700;font-size:13px;flex:1">' + pr.name + '</span>'
        + '<span style="font-family:monospace;font-size:12px;color:var(--ts)">' + pr.rate + '</span>'
        + (pr.impact ? '<span style="font-family:monospace;font-size:12px;font-weight:700;color:' + pr.color + '">~' + f$(pr.impact) + '</span>' : '')
        + '</div>'
        + '<div style="height:6px;background:var(--bdl);border-radius:3px;margin:0 0 6px 30px"><div style="height:100%;background:' + pr.color + ';border-radius:3px;width:' + bw + '%;opacity:.7"></div></div>'
        + '<div style="font-size:11px;color:var(--ts);margin-left:30px;line-height:1.5">' + pr.why + '</div></div>';
    }).join(""));
  }
  ...
  // ═══ S9: Five Strategies ═══
  h += secH("9", t("Comparaison de 5 strat\u00e9gies", "5-strategy comparison"), t("500 simulations ind\u00e9pendantes par strat\u00e9gie", "500 independent simulations per strategy"));
  if (stratData && stratData.length === 5) {
    var baseSucc = stratData[0].succ;
    h += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px;min-width:500px">';
    h += '<thead><tr style="background:var(--gbg)">'
      + '<th style="text-align:left;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Strat\u00e9gie", "Strategy") + '</th>'
      + '<th style="text-align:center;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Succ\u00e8s", "Success") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Patrim. m\u00e9dian", "Median wealth") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Imp\u00f4t d\u00e9c\u00e8s", "Death tax") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Succession", "Estate") + '</th>'
      + '</tr></thead><tbody>';
    stratData.forEach(function(s, i) {
      var sc = s.succ >= 0.85 ? "var(--gn)" : s.succ >= 0.70 ? "var(--am)" : "var(--rd)";
      var diff = i === 0 ? "\u2014" : ((s.succ - baseSucc >= 0 ? "+" : "") + Math.round((s.succ - baseSucc) * 100) + "pp");
      var diffC = i > 0 && s.succ - baseSucc > 0.02 ? "var(--gn)" : i > 0 && s.succ - baseSucc < -0.02 ? "var(--rd)" : "var(--ts)";
      h += '<tr style="border-bottom:1px solid var(--bdl);background:' + (i === 0 ? "var(--gbg)" : i % 2 === 0 ? "#FAFAF8" : "var(--bgc)") + '">'
        + '<td style="padding:10px 12px;font-weight:' + (i === 0 ? "700" : "400") + '">' + (fr ? s.fr : s.en) + (i === 0 ? " \u2713" : "") + ' <span style="font-size:11px;color:' + diffC + '">' + diff + '</span></td>'
        + '<td style="text-align:center;padding:10px;font-weight:800;color:' + sc + ';font-family:monospace">' + Math.round(s.succ * 100) + '%</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace">' + f$(s.medF) + '</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;color:var(--rd)">' + f$(s.medEstateTax) + '</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;font-weight:600">' + f$(s.medEstateNet) + '</td></tr>';
    });
    h += '</tbody></table></div>';
  ...
  if (params._report.homeVal > 0) {
    h += secH("10", t("Analyse immobili\u00e8re", "Real estate analysis"), t("R\u00e9sidence principale et strat\u00e9gie", "Primary residence and strategy"));
  ...
  if (params.cOn) {
    h += secH("11a", t("Analyse conjugale", "Couple analysis"), t("M\u00e9nage et optimisation", "Household optimization"));
    ...
    h += card(
      ...
      + kvr(t("\u00c2ge de retraite \u2014 conjoint\u00b7e", "Partner retirement age"), params.cRetAge + " " + t("ans", "yrs"))
      + kvr(t("D\u00e9calage de retraite", "Retirement gap"), Math.abs(D.retAge - params.cRetAge) + " " + t("ans", "yrs"))
      ...
      + kvr(t("RRQ/RPC conjoint\u00b7e", "Partner QPP/CPP start"), params.cQppAge + " " + t("ans", "yrs"))
    );
  ...
  if (params._report.bizOn) {
    h += secH("11b", t("Analyse corporative \u2014 CCPC", "Corporate analysis \u2014 CCPC"), t("Strat\u00e9gie de r\u00e9mun\u00e9ration", "Compensation strategy"));
    ...
    h += card(
      ...
      + kvr(t("Strat\u00e9gie de r\u00e9mun\u00e9ration", "Compensation strategy"), quiz.bizRemun === "mix" ? t("Mixte (" + (quiz.bizSalaryPct || 50) + "% salaire)", "Mixed (" + (quiz.bizSalaryPct || 50) + "% salary)") : quiz.bizRemun === "salary" ? t("Salaire pur", "Salary only") : t("Dividendes purs", "Dividends only"))
    );
  ...
  if ((D.medEstateNet || 0) > 100000 || (params.lifeInsBenefit || 0) > 0) {
    h += secH("12", t("Analyse successorale", "Estate analysis"), t("Patrimoine net au d\u00e9c\u00e8s (sc\u00e9nario m\u00e9dian)", "Net estate at death (median scenario)"));
    var succObj = (params._quiz || {}).succObjective || "neutral";
    var succObjBanner = succObj === "maximize"
      ? '<div style="background:var(--gbg);border-left:4px solid var(--gn);border-radius:8px;padding:12px 16px;margin-bottom:12px;font-size:13px;color:var(--g)">\ud83c\udfdb\ufe0f ' + t("Objectif : Maximiser la succession \u2014 ce plan optimise la pr\u00e9servation du patrimoine pour vos h\u00e9ritiers.", "Objective: Maximize estate \u2014 this plan optimizes wealth preservation for your heirs.") + '</div>'
      : succObj === "consume"
    ...
    h += succObjBanner;
    h += card(
      kvr(t("Patrimoine m\u00e9dian estim\u00e9 au d\u00e9c\u00e8s", "Est. median estate at death"), f$(D.medEstateNet))
  ...
  // ═══ S13: QPP Optimizer ═══
  h += secH("13", t("Optimiseur RRQ/RPC et PSV", "QPP/CPP and OAS optimizer"), t("Analyse du seuil de rentabilit\u00e9", "Break-even analysis"));
  (function() {
    var base65Q = D.qppMonthly;
    ...
    h += card(
      ...
      + [{ age: 60, v: q60, delta: "\u221236%" }, { age: 65, v: base65Q, delta: t("r\u00e9f\u00e9rence", "reference") }, { age: 70, v: q70, delta: "+42%" }].map(function(o) {
        ...
        return '<div style="padding:12px;border-radius:8px;border:2px solid ' + (sel ? "var(--g)" : "var(--bd)") + ';background:' + (sel ? "var(--gbg)" : "var(--bgc)") + ';text-align:center">'
          ...
          + '<div style="font-family:monospace;font-size:14px;font-weight:700;margin:4px 0">' + f$(o.v) + t("/mois", "/mo") + '</div>'
  ...
  h += secH("14", t("Projection patrimoniale aux 5 ans", "5-year wealth projection"), t("Sc\u00e9narios P25 / P50 / P75 \u2014 5 000 simulations", "P25 / P50 / P75 scenarios \u2014 5,000 simulations"));
  if (D.projTable && D.projTable.length > 0) {
    ...
    h += '<thead><tr style="background:var(--gbg)">'
      + '<th style="text-align:left;padding:10px 12px;border-bottom:2px solid var(--g)">' + t("\u00c2ge", "Age") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--rd)">P25 ' + t("(pessimiste)", "(pessimistic)") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--g);font-weight:800">P50 ' + t("(m\u00e9dian)", "(median)") + '</th>'
      + '<th style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--gn)">P75 ' + t("(optimiste)", "(optimistic)") + '</th>'
      + '</tr></thead><tbody>';
    D.projTable.forEach(function(row, i) {
      var ruin25 = row.p25 <= 0, ruin50 = row.p50 <= 0;
      h += '<tr style="border-bottom:1px solid var(--bdl);background:' + (i % 2 === 0 ? "var(--bgc)" : "#FAFAF8") + '">'
        + '<td style="padding:10px 12px;font-weight:700">' + row.age + ' ' + t("ans", "yrs") + '</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;color:var(--rd)">' + (ruin25 ? '<span style="color:var(--rd);font-weight:800">0 $ \u26a0\ufe0f</span>' : f$(row.p25)) + '</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;font-weight:700;color:var(--g)">' + (ruin50 ? '<span style="color:var(--rd);font-weight:800">0 $ \u26a0\ufe0f</span>' : f$(row.p50)) + '</td>'
        + '<td style="text-align:right;padding:10px 12px;font-family:monospace;color:var(--gn)">' + f$(row.p75) + '</td></tr>';
    });
    h += '</tbody></table></div>';
    h += '<div style="font-size:11px;color:var(--tm);margin-top:8px">' + t("P25 = 25% des simulations ont un r\u00e9sultat inf\u00e9rieur. \u26a0\ufe0f = patrimoine \u00e9puis\u00e9 dans ce sc\u00e9nario. Valeurs en dollars constants 2026.", "P25 = 25% of simulations have a lower result. \u26a0\ufe0f = wealth depleted in this scenario. Values in 2026 constant dollars.") + '</div>';
  }
  h += secEnd();

  // ═══ S15: Methodology ═══
  h += secH("15", t("M\u00e9thodologie et hypoth\u00e8ses", "Methodology and assumptions"), t("Comment ce rapport a \u00e9t\u00e9 calcul\u00e9", "How this report was calculated"));
  h += card(
    ...
    + t("CPM-2014 (tables actuarielles canadiennes). Mortalit\u00e9 stochastique activ\u00e9e \u2014 l'esp\u00e9rance de vie varie d'une simulation \u00e0 l'autre selon ces tables. L'\u00e2ge de planification maximum est 95 ans.", "CPM-2014 (Canadian actuarial tables). Stochastic mortality enabled \u2014 life expectancy varies across simulations based on these tables. Maximum planning age is 95.") + '</p>'
  ...
  // ═══ S16: Next Steps ═══
  h += secH("16", t("Prochaines \u00e9tapes", "Next steps"), t("Passer \u00e0 l'action", "Take action"));

  // Action #1 recap
  var a1 = priorities[0] || { name: t("Commencer \u00e0 \u00e9pargner", "Start saving"), why: t("La meilleure action est la plus rapide.", "The best action is the fastest.") };
  h += card('<div style="display:flex;gap:14px;align-items:flex-start">'
    + '<div style="font-size:28px;flex-shrink:0">\ud83c\udfaf</div>'
    + '<div><div style="font-size:14px;font-weight:700;margin-bottom:6px">' + t("Votre action no 1 \u2014 cette semaine", "Your action #1 \u2014 this week") + '</div>'
    + '<div style="font-size:14px;color:var(--ts);line-height:1.7">' + a1.name + '. ' + a1.why + '</div></div></div>');

  // Expert Simulator upsell CTA
  h += '<div style="background:linear-gradient(135deg,#1A1208,#2C1F0A);border-radius:16px;padding:32px 28px;margin-top:24px;color:#fff;position:relative;overflow:hidden">'
    + '<div style="position:absolute;top:-20px;right:-20px;width:140px;height:140px;border-radius:50%;background:rgba(196,148,74,0.12)"></div>'
    + '<div style="position:absolute;bottom:-40px;left:30%;width:200px;height:200px;border-radius:50%;background:rgba(196,148,74,0.06)"></div>'
    ...
    + [
      { icon: "\ud83c\udf93", tx: fr ? "RESP \u2014 \u00e9pargne-\u00e9tudes" : "RESP \u2014 education savings" },
      { icon: "\ud83d\udd04", tx: fr ? "Conversion REER \u2192 FERR optimis\u00e9e" : "RRSP \u2192 RRIF optimized conversion" },
      { icon: "\ud83d\udcca", tx: fr ? "5 profils de risque distincts" : "5 distinct risk profiles" },
      { icon: "\ud83c\udfec", tx: fr ? "Planification corporative avanc\u00e9e" : "Advanced corporate planning" },
      { icon: "\u2696\ufe0f", tx: fr ? "Strat\u00e9gie FRV / CRI avanc\u00e9e" : "Advanced LIF / LIRA strategy" },
      { icon: "\ud83c\udfdb\ufe0f", tx: fr ? "Analyse successorale compl\u00e8te" : "Full estate analysis" }
    ].map(function(f2) {
      return '<div style="display:flex;align-items:center;gap:8px;font-size:13px;color:rgba(255,255,255,0.85)">'
        + '<span>' + f2.icon + '</span><span>' + f2.tx + '</span></div>';
    }).join("")
    + '</div>'
    + '<div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">'
    + '<div><span style="font-size:36px;font-weight:900;color:var(--g)">129 $</span>'
    + '<span style="font-size:13px;color:rgba(255,255,255,0.5);margin-left:6px">' + t("acc\u00e8s unique", "one-time access") + '</span></div>'
    + '<button style="background:linear-gradient(135deg,var(--g),var(--gl));color:#1A1208;border:none;padding:14px 28px;border-radius:10px;font-size:15px;font-weight:800;cursor:pointer;letter-spacing:.3px;flex:1;min-width:200px" onclick="window.open(\'https://buildfi.ca\',\'_blank\')">' + t("Acc\u00e9der au Simulateur Expert \u2192", "Access Expert Simulator \u2192") + '</button>'
    + '</div>'
  ...
  h += '<div style="text-align:center;margin-top:24px;padding-top:16px;border-top:1px solid var(--bd);font-size:10px;color:var(--tm)">'
    ...
    + ' · <a href="https://www.buildfi.ca/avis-legal" style="color:var(--g);text-decoration:none">' + t("Avis l\u00e9gal","Legal") + '</a>'
    + '</div>';
...
// ══════════════════════════════════════════════════════════════════════
export function renderReportHTMLInter(D, mc, stratData, params, lang, ai, costDelay, minReturn, feedbackToken) {
  var fr = lang === "fr";
  var date = new Date().toLocaleDateString(fr ? "fr-CA" : "en-CA");

  var reportBody = renderReport(D, mc, stratData, params, ai || {}, false, lang, costDelay || 0, minReturn || 0);
...
+ '<title>buildfi.ca \u2014 ' + (fr ? "Rapport Interm\u00e9diaire" : "Intermediate Report") + ' \u2014 ' + date + '</title>'
...
+ ':root{'
+ '  --g:#C4944A;--gl:#D4A85A;--gd:#C4944A;--gbg:rgba(196,148,74,.07);'
...
+ '  --pr:#7C60B8;--bga:rgba(124,96,184,.04);'
+ '  --tx:#1A1A1A;--ts:#555;--tm:#888;--tl:#bbb;'
+ '  --bg:#FEFCF9;--bgc:#fff;--cd:#fff;--bd:#E8E0D4;--bdl:#F0ECE4;'
+ '}'
+ 'body{font-family:\'DM Sans\',-apple-system,BlinkMacSystemFont,system-ui,sans-serif;color:var(--tx);background:var(--bg);'
