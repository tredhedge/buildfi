...
function renderReport(D, mc, stratData, params, ai, aiLoading, lang, costDelay, minReturn, obsLabels) {
  ...
  var gradeLabel = D.successPct >= 85 ? t("Tr\u00e8s solide", "Very solid")
    : D.successPct >= 65 ? t("Solide", "Solid")
    : D.successPct >= 50 ? t("Fragile", "Fragile")
    : t("\u00c0 corriger", "Needs attention");
  var probTranslation = D.successPct === 0
    ? t("Autrement dit\u00a0: dans la totalit\u00e9 des sc\u00e9narios simul\u00e9s, l\u2019\u00e9pargne pourrait s\u2019\u00e9puiser avant la fin de la retraite.", "In other words: in all simulated scenarios, savings could be depleted before the end of retirement.")
    : D.successPct <= 25 ? t("Autrement dit\u00a0: dans environ 3 sc\u00e9narios sur 4, l\u2019\u00e9pargne pourrait s\u2019\u00e9puiser avant la fin de la retraite.", "In other words: in roughly 3 out of 4 scenarios, savings could run out before the end of retirement.")
    : D.successPct <= 50 ? t("Autrement dit\u00a0: dans environ 1 sc\u00e9nario sur 2, des ajustements pourraient \u00eatre n\u00e9cessaires en cours de retraite.", "In other words: in roughly 1 out of 2 scenarios, adjustments may be needed during retirement.")
    : D.successPct <= 74 ? t("Autrement dit\u00a0: dans environ 1 sc\u00e9nario sur 4, le plan pourrait n\u00e9cessiter des ajustements.", "In other words: in roughly 1 out of 4 scenarios, the plan may require some adjustments.")
    : D.successPct <= 99 ? t("Autrement dit\u00a0: le plan pourrait r\u00e9sister dans la grande majorit\u00e9 des sc\u00e9narios simul\u00e9s.", "In other words: the plan could hold in the vast majority of simulated scenarios.")
    : t("Autrement dit\u00a0: le plan pourrait r\u00e9sister dans la totalit\u00e9 des sc\u00e9narios simul\u00e9s.", "In other words: the plan could hold in all simulated scenarios.");
  ...
  function aiSlot(text, fallback) {
    if (aiLoading) return '<div style="font-size:13px;color:var(--pr);padding:14px 18px;background:rgba(124,96,184,.04);border-radius:8px;border-left:3px solid var(--pr);font-style:italic;margin:12px 0">\u2026</div>';
    if (!text) return fallback ? '<div style="font-size:13px;color:var(--tm);line-height:1.7;margin:12px 0;padding:14px 18px;background:rgba(124,96,184,.04);border-radius:8px;border-left:3px solid var(--pr);font-style:italic">' + fallback + '</div>' : '';
    return '<div style="font-size:13px;color:var(--ts);line-height:1.85;margin:14px 0;padding:14px 18px;background:rgba(124,96,184,.04);border-radius:8px;border-left:3px solid var(--pr)">' + text + '</div>';
  }
  ...
  function obs(title, text, type) {
    ...
    return '<div style="background:' + c[0] + ';border:1px solid ' + c[1] + ';border-left:4px solid ' + c[1] + ';border-radius:8px;padding:14px 16px;margin:10px 0">'
      ...
      + '<div style="font-size:13px;color:var(--ts);line-height:1.75">' + text + '</div></div>';
  }
  ...
  // ── Logo SVG ──
  var logoSVG = '<svg xmlns="http://www.w3.org/2000/svg" width="140" height="40" viewBox="0 0 180 52">'
    + '<defs><linearGradient id="embG" x1="0" y1="1" x2="0" y2="0"><stop offset="0%" stop-color="#C45A2C"/><stop offset="100%" stop-color="#E8A84C"/></linearGradient></defs>'
    + '<path d="M14 46 C14 46 8 30 14 18 C20 6 26 12 26 22 C26 12 32 0 38 10 C44 22 38 32 38 32" stroke="url(#embG)" stroke-width="3" fill="none" stroke-linecap="round"/>'
    + '<text x="48" y="36" font-family="DM Sans,sans-serif" font-size="32" font-weight="800" fill="#1A1A1A" letter-spacing="-1.2">build</text>'
    + '<text x="131" y="36" font-family="DM Sans,sans-serif" font-size="32" font-weight="800" fill="url(#embG)" letter-spacing="-1.2">fi</text>'
    + '</svg>';

  h += '<div style="margin-bottom:32px;padding:28px 24px;background:linear-gradient(135deg,#FFF8ED,#FDF3E0);border-radius:16px;border:1px solid rgba(196,148,74,.2)">'
    // Top row: logo + meta
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:8px">'
    + logoSVG
    + '<div style="font-size:10px;color:var(--tm);text-align:right">'
    + t("Rapport Interm\u00e9diaire", "Intermediate Report") + ' \u2014 ' + D.prov
  ...
  // Objectif callout (M-5)
  if (ai.objectif) {
    h += '<div class="co cog"><strong>' + t("Votre objectif", "Your objective") + '</strong> \u2014 ' + ai.objectif + '</div>';
  }

  // ═══ TOC (m-13) ═══
  var tocItems = [
    { n: "1", l: t("Tableau de bord", "Dashboard") },
    { n: "2", l: t("Profil", "Profile") },
    { n: "3", l: t("\u00c9pargne", "Savings") },
    { n: "4", l: t("Revenus", "Income") },
    { n: "5", l: t("Fiscalit\u00e9", "Taxes") },
    { n: "6", l: t("Long\u00e9vit\u00e9", "Longevity") },
    { n: "7", l: t("Observations", "Observations") },
    { n: "8", l: t("Priorit\u00e9s", "Priorities") },
    { n: "9", l: t("Strat\u00e9gies", "Strategies") },
  ];
  if (params._report.homeVal > 0) tocItems.push({ n: "10", l: t("Immobilier", "Real estate") });
  if (params.cOn) tocItems.push({ n: "11a", l: t("Couple", "Couple") });
  if (params._report.bizOn) tocItems.push({ n: "11b", l: "CCPC" });
  if ((D.medEstateNet || 0) > 100000 || (params.lifeInsBenefit || 0) > 0) tocItems.push({ n: "12", l: t("Succession", "Estate") });
  tocItems.push({ n: "13", l: t("RRQ/PSV", "QPP/OAS") });
  tocItems.push({ n: "14", l: t("Projection", "Projection") });
  tocItems.push({ n: "15", l: t("M\u00e9thodo", "Methodology") });
  tocItems.push({ n: "16", l: t("Actions", "Next steps") });
  ...
  h += kp(f$(D.rMedF), t("Patrimoine r\u00e9siduel m\u00e9dian", "Median residual wealth"), D.rMedF > 0 ? "var(--gn)" : "var(--rd)", t("\u00c0 " + D.avgDeath + " ans (esp. vie MC)", "At age " + D.avgDeath + " (MC life exp.)"));
  ...
  (function() {
    ...
    var legX = P + 4, legItems = [{ c: "rgba(196,148,74,0.55)", l: t("REER", "RRSP") }, { c: "rgba(42,140,70,0.45)", l: t("CELI", "TFSA") }, { c: "rgba(184,152,48,0.45)", l: t("NR", "NR") }, { c: "rgba(124,96,184,0.4)", l: t("CRI", "LIRA") }, { c: "rgba(70,128,192,0.4)", l: "DC/Corp" }];
  ...
  (function() {
    ...
    var items = [{ v: qppMo, c: "#2A8C46", l: isQC ? "RRQ" : t("RPC", "CPP") }, { v: oasMo, c: "#4680C0", l: t("PSV", "OAS") }, { v: penM, c: "#7C60B8", l: t("Pension PD", "DB Pension") }, { v: savM, c: "#C4944A", l: t("\u00c9pargne", "Savings") }].filter(function(i) { return i.v > 0; });
  ...
  h += secH("6", t("Long\u00e9vit\u00e9 du patrimoine", "Wealth longevity"), t("Distribution des " + nS + " sc\u00e9narios", "Distribution of " + nS + " scenarios"));
  ...
  h += '<div class="co cogn">' + t("Ces cinq observations sont tri\u00e9es par impact financier estim\u00e9 sur votre plan. La premi\u00e8re repr\u00e9sente votre levier principal.", "These five observations are ranked by estimated financial impact on your plan. The first represents your primary lever.") + '</div>';
  var oL = obsLabels || {};
  ...
  if (priorities.length > 0) {
    ...
    h += card(priorities.map(function(pr, i) {
      ...
      return '<div style="margin-bottom:16px">'
        ...
        + '<span style="font-family:var(--mono);font-size:12px;color:var(--ts)">' + pr.rate + '</span>'
  ...
  if (stratData && stratData.length === 5) {
    ...
    h += '<thead><tr style="background:var(--gbg)">'
      + '<th scope="col" style="text-align:left;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Strat\u00e9gie", "Strategy") + '</th>'
      + '<th scope="col" style="text-align:center;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Succ\u00e8s", "Success") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Patrim. m\u00e9dian", "Median wealth") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Imp\u00f4t d\u00e9c\u00e8s", "Death tax") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;font-weight:700;border-bottom:2px solid var(--g)">' + t("Succession", "Estate") + '</th>'
      + '</tr></thead><tbody>';
  ...
  h += secH("14", t("Projection patrimoniale aux 5 ans", "5-year wealth projection"), t("Sc\u00e9narios P25 / P50 / P75 \u2014 " + nS + " simulations", "P25 / P50 / P75 scenarios \u2014 " + nS + " simulations"));
  if (D.projTable && D.projTable.length > 0) {
    h += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px;min-width:440px">';
    h += '<thead><tr style="background:var(--gbg)">'
      + '<th scope="col" style="text-align:left;padding:10px 12px;border-bottom:2px solid var(--g)">' + t("\u00c2ge", "Age") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--rd)">P25 ' + t("(pessimiste)", "(pessimistic)") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--g);font-weight:800">P50 ' + t("(m\u00e9dian)", "(median)") + '</th>'
      + '<th scope="col" style="text-align:right;padding:10px 12px;border-bottom:2px solid var(--g);color:var(--gn)">P75 ' + t("(optimiste)", "(optimistic)") + '</th>'
      + '</tr></thead><tbody>';
  ...
  h += '<div class="no-print" style="background:linear-gradient(135deg,#1A1208,#2C1F0A);border-radius:16px;padding:32px 28px;margin-top:24px;color:#fff;position:relative;overflow:hidden">'
    ...
    + '<button style="background:linear-gradient(135deg,var(--g),var(--gl));color:#1A1208;border:none;padding:14px 28px;border-radius:10px;font-size:15px;font-weight:800;cursor:pointer;letter-spacing:.3px;flex:1;min-width:200px" onclick="window.open(\'https://buildfi.ca/expert\',\'_blank\')">' + t("Acc\u00e9der au Simulateur Expert \u2192", "Access Expert Simulator \u2192") + '</button>'
    + '</div>'
...
+ ':root{'
+ '  --g:#C4944A;--gd:#C4944A;--gl:#D4A85A;--gbg:rgba(196,148,74,.07);'
