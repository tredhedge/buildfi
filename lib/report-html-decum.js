...
`;
  ...
  var sec4 = '<section>' + secH("4", fr ? "Stratégie de décaissement" : "Drawdown Strategy", "") +
    ...
    (fr ? "L'ordre ci-dessous est celui utilisé dans les simulations. Il analyse chaque dollar de retrait selon la situation fiscale de l'année."
       : "The order below is the one used in the simulations. It analyzes each withdrawal dollar based on the year's tax situation.")
    + '</p>' +
  ...
  var oasThreshold = 90997;
  var aboveOAS = D.retIncome > oasThreshold;
  var sec4b = '<section>' + secH("5", fr ? "Efficacité fiscale du décaissement" : "Tax Efficiency of Drawdown", "") +
    '<div class="stats-grid">' +
    ...
    (fr ? "Les retraits minimaux du FERR augmentent avec l'âge : 5,28 % à 71 ans, 6,82 % à 80 ans, 10,21 % à 85 ans, 18,79 % à 93 ans. Ces retraits forcés s'ajoutent au revenu imposable."
       : "RRIF minimum withdrawals increase with age: 5.28% at 71, 6.82% at 80, 10.21% at 85, 18.79% at 93. These forced withdrawals add to taxable income.")
    + '</p>' : '') +
  ...
  if (!D.alreadyClaiming && D.mc60Succ !== null) {
    sec5 = '<section>' + secH("6", fr ? ("Timing " + fr_govSingle + " : 60, 65 ou 70 ans") : ("Timing " + fr_govSingle + ": 60, 65 or 70"), "") +
      '<p style="color:var(--gray);font-size:14px;margin-bottom:16px;">' +
  ...
  var sec6 = '<section>' + secH("7", fr ? "Analyse de robustesse : scénarios de réduction" : "Robustness Analysis: Reduction Scenarios", "") +
    '<p style="color:var(--gray);font-size:14px;margin-bottom:16px;">' +
  ...
  var sec7 = '<section>' + secH("8", fr ? "Répartition d'actifs et glissement" : "Asset Allocation & Glide Path", "") +
    '<div class="stats-grid">' +
    ...
    aiSlot("obs_2", fr ? "La répartition entre actions (" + Math.round(D.allocR * 100) + " %) et obligations (" + Math.round((1 - D.allocR) * 100) + " %) évolue graduellement vers une posture plus prudente avec l'âge." : "The split between equities (" + Math.round(D.allocR * 100) + "%) and bonds (" + Math.round((1 - D.allocR) * 100) + "%) gradually shifts toward a more conservative posture with age.") + '</section>';

  // ── Section 9: Profil de dépenses ───────────────────────
  var sec8 = '<section>' + secH("9", fr ? "Profil de dépenses — courbe naturelle" : "Spending Profile — Natural Curve", "") +
  ...
  var sec9 = '<section>' + secH("10", fr ? "Distribution de longévité" : "Longevity Distribution", "") +
    '<p style="color:var(--gray);font-size:14px;margin-bottom:16px;">' +
  ...
  var sec10 = '<section>' + secH("11", fr ? "Portrait successoral" : "Estate Overview", "") +
  ...
  // ── Section 12: Observations complémentaires ─────────────
  var hasObs = (ai && ai.obs_1 && ai.obs_1.trim()) || (ai && ai.obs_3 && ai.obs_3.trim());
  var sec11 = hasObs ? '<section>' + secH("12", fr ? "Observations complémentaires" : "Additional Observations", "") +
  ...
  // ── Section 13: Hypothèses et méthodologie ──────────────
  var sec12 = '<section>' + secH(hasObs ? "13" : "12", fr ? "Hypothèses et méthodologie" : "Assumptions & Methodology", "") +
    '<table class="data-table"><tbody>' +
