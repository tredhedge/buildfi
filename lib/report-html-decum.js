...
.ai-narration{background:var(--card);border-left:3px solid var(--gold);padding:14px 16px;border-radius:0 8px 8px 0;font-size:15px;color:#333;line-height:1.7;margin:16px 0;}
...
`;
  ...
  // ── Grade label ───────────────────────────────────────
  var gradeLabel = succPct >= 85 ? (fr ? "Très solide" : "Very solid")
    : succPct >= 65 ? (fr ? "Solide" : "Solid")
    : succPct >= 50 ? (fr ? "Fragile" : "Fragile")
    : (fr ? "À corriger" : "Needs attention");

  // ── probTranslation — human-readable probability ──────
  // Probability translation — decum version (revenue sustainability, not savings depletion)
  var probTranslation = succPct === 0
    ? (fr ? "Autrement dit\u00a0: dans tous les scénarios simulés, le revenu pourrait s\u2019avérer insuffisant avant la fin de la retraite." : "In other words: in all simulated scenarios, income could prove insufficient before the end of retirement.")
    : succPct <= 10 ? (fr ? "Autrement dit\u00a0: dans environ 9 scénarios sur 10, des ajustements de dépenses pourraient devenir nécessaires." : "In other words: in roughly 9 out of 10 scenarios, spending adjustments could become necessary.")
    : succPct <= 25 ? (fr ? "Autrement dit\u00a0: dans environ 3 scénarios sur 4, des ajustements pourraient être nécessaires." : "In other words: in roughly 3 out of 4 scenarios, adjustments may be needed.")
    : succPct <= 40 ? (fr ? "Autrement dit\u00a0: dans plus de 1 scénario sur 2, des ajustements de dépenses pourraient devenir nécessaires." : "In other words: in more than 1 out of 2 scenarios, spending adjustments may be needed.")
    : succPct <= 50 ? (fr ? "Autrement dit\u00a0: dans environ 1 scénario sur 2, des ajustements pourraient être nécessaires en cours de retraite." : "In other words: in roughly 1 out of 2 scenarios, adjustments may be needed during retirement.")
    : succPct <= 74 ? (fr ? "Autrement dit\u00a0: dans environ 1 scénario sur 4, le plan pourrait nécessiter des ajustements." : "In other words: in roughly 1 out of 4 scenarios, the plan may require some adjustments.")
    : succPct <= 89 ? (fr ? "Autrement dit\u00a0: dans environ 1 scénario sur 10, le plan pourrait nécessiter des ajustements." : "In other words: in roughly 1 out of 10 scenarios, the plan may need adjustments.")
    : succPct <= 99 ? (fr ? "Autrement dit\u00a0: le plan pourrait résister dans la grande majorité des scénarios simulés." : "In other words: the plan could hold in the vast majority of simulated scenarios.")
    : (fr ? "Autrement dit\u00a0: le plan pourrait résister dans la totalité des scénarios simulés." : "In other words: the plan could hold in all simulated scenarios.");
  ...
  if (levers.length > 0) {
    secLeviers = '<section>' + secH(nextSec(), fr ? "Leviers identifiés" : "Identified Levers", "") +
      ...
      levers.map(function(lev, i){
        var isCrown = i === 0;
        return '<div style="display:flex;gap:14px;align-items:flex-start;padding:16px 18px;margin-bottom:10px;background:' + (isCrown ? 'linear-gradient(135deg,#1A1208 0%,#2D2010 100%)' : '#F8F4EE') + ';border-radius:12px;border:1px solid ' + (isCrown ? '#C4944A' : '#E8E0D4') + ';">' +
          '<div style="width:32px;height:32px;border-radius:50%;background:' + lev.color + ';color:#fff;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' + (i + 1) + '</div>' +
          '<div style="flex:1;">' +
