...
// ============================================================
// Decision Card — Décaissement
// ============================================================
function buildDecisionCardDecum(D, ai, fr, govLabel, gColor, grade, succPct) {
  var t = function(f, e) { return fr ? f : e; };
  var gradeLabel = sharedGradeLabel(succPct, fr);
  var dcVuln = '', dcLever = '';

  // Vulnerability (first match wins)
  if (D.melt1Succ !== null && D.melt1Succ < succPct - 15) {
    dcVuln = t(
      "Un krach la premi\u00e8re ann\u00e9e de votre retraite ferait chuter le taux de r\u00e9ussite de " + succPct + "\u00a0% \u00e0 " + D.melt1Succ + "\u00a0%.",
      "A crash in your first year of retirement would drop the success rate from " + succPct + "% to " + D.melt1Succ + "%."
    );
  } else if (succPct < 70) {
    dcVuln = t(
      "Le mod\u00e8le indique que votre plan pourrait ne pas r\u00e9sister dans " + (100 - succPct) + "\u00a0% des sc\u00e9narios simul\u00e9s.",
      "The model indicates your plan may not hold in " + (100 - succPct) + "% of simulated scenarios."
    );
  } else if (D.initialRate > 5) {
    dcVuln = t(
      "Le taux de retrait initial de " + fmtPctRaw(D.initialRate) + "\u00a0% d\u00e9passe le seuil historiquement soutenable de 4\u00a0%.",
      "The initial withdrawal rate of " + fmtPctRaw(D.initialRate) + "% exceeds the historically sustainable 4% threshold."
    );
  } else if (D.ruinPct > 0 && D.medRuin < 999) {
    dcVuln = t(
      "Dans " + D.ruinPct + "\u00a0% des sc\u00e9narios, le capital pourrait s\u2019\u00e9puiser vers " + D.medRuin + " ans.",
      "In " + D.ruinPct + "% of scenarios, capital could deplete around age " + D.medRuin + "."
    );
  } else {
    dcVuln = t(
      "Votre strat\u00e9gie de d\u00e9caissement r\u00e9siste bien aux sc\u00e9narios test\u00e9s.",
      "Your drawdown strategy holds up well across tested scenarios."
    );
  }

  // Lever (first match wins)
  if (D.mc60Succ !== null && D.mc70Succ !== null && !D.alreadyClaiming) {
    var bestCpp = D.mc70Succ >= D.mc65Succ && D.mc70Succ >= D.mc60Succ ? 70 : D.mc65Succ >= D.mc60Succ ? 65 : 60;
    var worstCpp = D.mc60Succ <= D.mc65Succ && D.mc60Succ <= D.mc70Succ ? 60 : D.mc65Succ <= D.mc70Succ ? 65 : 70;
    var bestPct = bestCpp === 70 ? D.mc70Succ : bestCpp === 65 ? D.mc65Succ : D.mc60Succ;
    var worstPct = worstCpp === 60 ? D.mc60Succ : worstCpp === 65 ? D.mc65Succ : D.mc70Succ;
    if (bestPct - worstPct >= 5) {
      dcLever = t(
        "Le timing de la " + govLabel + " fait une diff\u00e9rence : demander \u00e0 " + bestCpp + " ans (" + bestPct + "\u00a0%) vs " + worstCpp + " ans (" + worstPct + "\u00a0%).",
        govLabel + " timing makes a difference: claiming at " + bestCpp + " (" + bestPct + "%) vs " + worstCpp + " (" + worstPct + "%)."
      );
    }
  }
  if (!dcLever && D.gkActive && D.gkCutFreq !== null && D.gkCutFreq > 0.1) {
    dcLever = t(
      "La flexibilit\u00e9 des d\u00e9penses (r\u00e9duction allant jusqu\u2019\u00e0 " + Math.round(D.gkMaxCut * 100) + "\u00a0%) am\u00e9liore la r\u00e9silience de votre plan.",
      "Spending flexibility (reductions up to " + Math.round(D.gkMaxCut * 100) + "%) improves your plan\u2019s resilience."
    );
  }
  if (!dcLever && D.initialRate > 4) {
    dcLever = t(
      "R\u00e9duire le revenu cible ou augmenter les revenus garantis pourrait abaisser le taux de retrait sous le seuil de 4\u00a0%.",
      "Reducing the target income or increasing guaranteed income could bring the withdrawal rate below the 4% threshold."
    );
  }
  if (!dcLever) {
    dcLever = t(
      "Consultez les observations ci-dessous pour les pistes identifi\u00e9es.",
      "See the observations below for identified paths."
    );
  }

  var dcInterp = (ai && ai.snapshot_intro) ? escHtml(ai.snapshot_intro) : t(
    "Votre note de " + gradeLabel + " (" + succPct + "\u00a0%) indique " + (succPct >= 80 ? "une position solide pour le d\u00e9caissement" : succPct >= 60 ? "une position correcte avec des pistes d\u2019ajustement" : "une situation qui m\u00e9rite attention") + ". Les sections suivantes d\u00e9taillent chaque dimension de votre strat\u00e9gie de retraite.",
    "Your grade of " + gradeLabel + " (" + succPct + "%) indicates " + (succPct >= 80 ? "a solid drawdown position" : succPct >= 60 ? "a reasonable position with adjustment paths" : "a situation that warrants attention") + ". The following sections detail each dimension of your retirement strategy."
  );

  return '<div style="background:#fff;border:1px solid #E8E0D4;border-left:4px solid #c49a1a;border-radius:12px;padding:24px 28px;margin:0 0 24px;box-shadow:0 2px 8px rgba(0,0,0,.03)">'
    + '<div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">'
    + '<div style="font-family:\'JetBrains Mono\',monospace;font-size:36px;font-weight:900;color:' + gColor + ';line-height:1">' + grade + '</div>'
    + '<div><div style="font-size:15px;font-weight:700;color:' + gColor + '">' + gradeLabel + '</div>'
    + '<div style="font-size:13px;color:#666">' + succPct + '\u00a0% ' + t("de r\u00e9ussite","success rate") + '</div></div></div>'
    + '<div style="height:1px;background:#E8E0D4;margin:16px 0"></div>'
    + '<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:700;color:#CC4444;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">' + t("Votre plus grand risque","Your biggest risk") + '</div>'
    + '<div style="font-size:14px;color:#333;line-height:1.65">' + dcVuln + '</div></div>'
    + '<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:700;color:#c49a1a;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">' + t("Votre meilleur levier","Your best lever") + '</div>'
    + '<div style="font-size:14px;color:#333;line-height:1.65">' + dcLever + '</div></div>'
    + '<div style="height:1px;background:#E8E0D4;margin:16px 0"></div>'
    + '<div style="font-size:12px;font-weight:700;color:#666;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">' + t("Ce que cela signifie","What this means") + '</div>'
    + '<div style="font-size:13px;color:#666;line-height:1.7">' + dcInterp + '</div>'
    + '</div>';
}
...
export function renderReportDecum(D, mc, params, lang, ai, feedbackToken, extraRuns) {
  ...
  function progBar(pct, color) {
    ...
    return '<div class="prog-bg"><div class="prog-fill" style="width:' + w + '%;background:' + (color || "#c49a1a") + ';"></div></div>';
  }
  ...
  function buildFanChart() {
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" width="100%" style="max-width:' + W + 'px;">' +
      '<polygon points="' + p90pts + ' ' + p10pts + '" fill="#c49a1a" fill-opacity="0.15"/>' +
      '<polyline points="' + p50line + '" fill="none" stroke="#c49a1a" stroke-width="2.5"/>' +
      axisLabels +
  ...
  function buildSmileChart() {
    ...
    var labelsSVG = labels.map(function(l){
      return '<text x="' + xp(l.age) + '" y="' + (PAD_T - 2) + '" font-size="9" fill="#c49a1a" text-anchor="middle">' + l.label + '</text>';
    }).join("");
    // Reference line at 100%
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" width="100%" style="max-width:' + W + 'px;">' +
      '<line x1="' + PAD_L + '" y1="' + yp(1) + '" x2="' + (PAD_L + chartW) + '" y2="' + yp(1) + '" stroke="#E8E0D4" stroke-dasharray="4,3" stroke-width="1"/>' +
      '<polyline points="' + linePts + '" fill="none" stroke="#c49a1a" stroke-width="2.5"/>' +
      labelsSVG +
    ...
    return '<table class="data-table" style="width:100%;">' +
      ...
      rows.map(function(r){
        ...
        return '<tr' + (isB ? ' class="best-row"' : '') + '><td>' + r.label + (isB ? ' <span class="badge-best">' + (fr ? "meilleur" : "best") + '</span>' : '') + '</td>' +
          '<td><strong>' + r.pct + ' %</strong> ' + progBar(r.pct, r.pct === best.pct ? "#2A8C46" : "#c49a1a") + '</td>' +
  ...
  function buildMeltdownBlock() {
    ...
    var rows = [
      ...
      { label: fr ? ("Réduction au premier palier (" + fmtN(meltTarget, fr) + " $)") : ("Reduction to first bracket (" + fmtN(meltTarget, fr) + ")"), val: meltTarget, pct: melt1, color: melt1 !== null && melt1 >= 75 ? "#2A8C46" : "#c49a1a" },
      { label: fr ? ("Réduction sévère (" + fmtN(melt2Target, fr) + " $)") : ("Severe reduction (" + fmtN(melt2Target, fr) + ")"), val: melt2Target, pct: melt2, color: melt2 !== null && melt2 >= 75 ? "#2A8C46" : "#c49a1a" }
    ];
    return '<table class="data-table" style="width:100%;">' +
  ...
  // ── CSS ──────────────────────────────────────────────────
  var css = `
:root{--gold:#c49a1a;--dark:#1A1208;--bg:#FEFCF9;--card:#F8F4EE;--border:#E8E0D4;--green:#2A8C46;--blue:#4680C0;--red:#CC4444;--gray:#666;--mono:'JetBrains Mono','SF Mono',monospace;}
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'DM Sans',-apple-system,BlinkMacSystemFont,sans-serif;background:var(--bg);color:var(--dark);line-height:1.7;font-size:16px;
...
.stat-value{font-size:22px;font-weight:700;color:var(--dark);font-family:var(--mono);font-variant-numeric:tabular-nums;}
...
.ai-narration{background:#FDFBF7;border-left:3px solid #c49a1a;padding:12px 16px;border-radius:4px;font-size:14px;color:#444;line-height:1.75;margin:12px 0;font-style:normal;}
...
.data-table td{padding:10px 12px;border-bottom:1px solid #F0EBE0;color:#444;font-variant-numeric:tabular-nums;}
...
.income-item .amt{font-size:20px;font-weight:700;font-family:var(--mono);font-variant-numeric:tabular-nums;}
...
.highlight-block .h-val{font-size:28px;font-weight:700;font-family:var(--mono);font-variant-numeric:tabular-nums;}
...
@media print{body{background:#fff;}.no-print{display:none!important;}section{break-inside:avoid;page-break-inside:avoid;}.disclaimer-section{display:block;}}
`;
  ...
  var mirrorBlock = '<div style="background:linear-gradient(135deg,#1A1208 0%,#2D2010 100%);color:#fff;border-radius:14px;padding:24px 28px;margin:20px 0;">' +
    '<div style="font-size:11px;text-transform:uppercase;letter-spacing:2px;color:#c49a1a;margin-bottom:12px;font-weight:600;">' +
  ...
  var sec3 = '<section>' + secH(nextSec(), fr ? "Durabilité du portefeuille" : "Portfolio Sustainability", "") +
    ...
    '<div class="legend-item"><div class="legend-dot" style="background:rgba(196,154,26,0.3);width:20px;height:12px;border-radius:3px;"></div> ' + (fr ? "Fourchette P10–P90" : "P10–P90 range") + '</div></div></div>' : '') +
  ...
  if (levers.length > 0) {
    secLeviers = '<section>' + secH(nextSec(), fr ? "Leviers identifiés" : "Identified Levers", "") +
      ...
      '<div style="background:#FFF9F0;border:2px solid #c49a1a;border-radius:8px;padding:20px 24px;margin:24px 0">' +
      levers.map(function(lev, i){
        var isCrown = i === 0;
        return '<div style="display:flex;gap:14px;align-items:flex-start;padding:16px 18px;margin-bottom:10px;background:' + (isCrown ? 'linear-gradient(135deg,#1A1208 0%,#2D2010 100%)' : '#FDFBF7') + ';border-radius:12px;border:1px solid ' + (isCrown ? '#c49a1a' : '#E8E0D4') + ';">' +
          '<div style="width:32px;height:32px;border-radius:50%;background:' + (isCrown ? '#c49a1a' : lev.color) + ';color:#fff;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' + (i + 1) + '</div>' +
          '<div style="flex:1;">' +
          '<div style="font-size:15px;font-weight:700;color:' + (isCrown ? '#c49a1a' : '#1A1208') + ';">' + lev.name + (isCrown ? ' <span style="font-size:11px;background:#c49a1a;color:#fff;padding:2px 8px;border-radius:10px;margin-left:8px;font-weight:600;">' + (fr ? "impact principal" : "primary impact") + '</span>' : '') + '</div>' +
...
<div class="container">
  ...
  ${buildDecisionCardDecum(D, ai, fr, fr_govSingle, gColor, grade, succPct)}
  ...
  <div class="no-print" style="margin-top:16px">
    <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-radius:10px;border:1px solid var(--border);background:var(--card);cursor:pointer" onclick="window.print()">
      <span style="font-size:18px">\ud83d\udda8\ufe0f</span>
      <div><div style="font-size:13px;font-weight:600;color:var(--gray)">${fr ? "Imprimer ou sauvegarder en PDF" : "Print or save as PDF"}</div>
      <div style="font-size:11px;color:var(--gray)">${fr ? "Conservez une copie de votre rapport" : "Keep a copy of your report"}</div></div>
    </div>
  </div>

  <!-- FEEDBACK_STARS -->

  <div class="no-print" style="text-align:center;margin:16px 0;padding:12px;border:1px solid var(--border);border-radius:8px;font-size:12px;color:var(--gray);line-height:1.8">
    ${fr ? "Partagez BuildFi avec un proche \u2014 15\u00a0% de rabais appliqu\u00e9 automatiquement via votre lien." : "Share BuildFi with someone you know \u2014 15% off applied automatically through your link."}
    <a href="https://www.buildfi.ca" style="color:var(--gold);text-decoration:none;font-weight:600">buildfi.ca</a>
  </div>

  <div style="text-align:center;margin-top:24px;padding-top:16px;border-top:1px solid var(--border);font-size:10px;color:var(--gray)">
    <a href="https://www.buildfi.ca/conditions" style="color:var(--gold);text-decoration:none">${fr ? "Conditions" : "Terms"}</a>
    \u00b7 <a href="https://www.buildfi.ca/confidentialite" style="color:var(--gold);text-decoration:none">${fr ? "Confidentialit\u00e9" : "Privacy"}</a>
    \u00b7 <a href="https://www.buildfi.ca/avis-legal" style="color:var(--gold);text-decoration:none">${fr ? "Avis l\u00e9gal" : "Legal"}</a>
  </div>
