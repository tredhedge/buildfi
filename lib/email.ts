// /lib/email.ts
// Email delivery via Resend — sends the report link after purchase
// PDF attachment disabled until Chromium resolved on Vercel
...
export async function sendReportEmail(params: SendReportParams) {
  ...
  const attachments = pdfBuffer && pdfFilename
    ? [{ filename: pdfFilename, content: pdfBuffer, contentType: "application/pdf" as const }]
    : [];
  ...
  <div style="max-width:580px;margin:0 auto;padding:40px 24px">
    ...
    <div style="text-align:center;margin-bottom:32px">
      <a href="${downloadUrl}"
        ...
        ${fr ? "Consulter mon rapport" : "View my report"}
      </a>
