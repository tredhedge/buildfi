...
// ══════════════════════════════════════════════════════════════════════
/* eslint-disable */
// @ts-nocheck — Mechanical port; strict TS comes in P4

import { translateToMC } from "./quiz-translator";
import { calcTax, calcQPP, calcOAS, getRRIFMin, sMul, blendRet, tRn, PROV_TAX } from "./engine";

// === HELPERS (from quiz-essentiel.html lines 2248-2500) ===
...
function extractReportData(mc, p) {
  ...
  return {
    ...
    // Expected return + after-tax (for debt priority)
...
// ============================================================
// DEBT PRIORITY WATERFALL (adapted from planner for Essentiel)
// ============================================================
function calcMinViableReturn(baseParams){
...
function calcCostOfDelay(baseParams){
...
function renderReport(D, mc, quiz, ai, aiLoading, lang, costDelay, minReturn) {
  ...
  // [Controls removed for PDF — no-print elements]
  ...
  h += '</div></div>';
  h += card(kvr(t("Âge","Age"), D.age + t(" ans"," years")) + kvr(t("Retraite prévue","Planned retirement"), D.retAge + t(" ans"," years") + ' (' + yrsToRet + t(" ans)"," years)")) + kvr(t("Revenu annuel","Annual income"), f$(D.sal)) + kvr("Province", D.prov) + kvr(t("Dépenses prévues à la retraite","Planned retirement spending"), f$(D.retSpM) + t(" $/mois"," $/mo")) + (D.hasPension ? kvr(t("Régime de pension d'employeur","Employer pension plan"), D.penType === "dc" ? t("Cotisations déterminées","Defined contribution") + " (" + f$(D.dcBal) + ")" : t("Prestations déterminées","Defined benefit") + " (" + f$(D.dbPensionMonthly) + t(" $/mois"," $/mo") + ")") : ''));
  h += secEnd();

  // === S3: SAVINGS with Wealth Chart + Account Breakdown ===
  h += secH(3, t("Votre épargne","Your Savings"));
  h += aiSlot(ai.savings_context);
  h += '<div class="rpt-grid3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">' + kp(f$(totalSav), t("Épargne totale","Total savings"), null, D.penType==="dc"?t("incluant pension","including pension"):null) + kp(f$(D.retYearBalance), t("Projeté à la retraite","Projected at retirement"), "var(--gn)") + kp(D.withdrawalRatePct + "%", t("Taux de retrait annuel","Annual withdrawal rate"), D.withdrawalRatePct <= 4 ? "var(--gn)" : "var(--am)", t("règle des 4% = repère","4% rule = benchmark")) + '</div>';
  // Wealth chart
  ...
  // Account breakdown
  h += card(kvr(t("Compte de retraite enregistré (REER)","Registered retirement account (RRSP)"), f$(D.rrsp)) + kvr(t("Compte d'épargne libre d'impôt (CELI)","Tax-free savings account (TFSA)"), f$(D.tfsa)) + kvr(t("Placements non enregistrés","Non-registered investments"), f$(D.nr)) + (D.penType === "dc" ? kvr(t("Régime à cotisations déterminées","Defined contribution plan"), f$(D.dcBal)) : ''));
  h += secEnd();
  ...
  if (rp.debtBal > 0 || rp.mortBal > 0) {
    ...
    if (rp.debtBal > 0) h += kp(f$(rp.debtBal), t("Dettes totales","Total debts"), "var(--rd)");
    if (rp.mortBal > 0) h += kp(f$(rp.mortBal), t("Hypothèque","Mortgage"), "var(--am)");
    h += '</div>';
  ...
  h += '<svg width="180" height="180" viewBox="0 0 180 180"><circle cx="90" cy="90" r="70" fill="none" stroke="#2A8C46" stroke-width="22" stroke-dasharray="' + gDash + ' ' + (dC-gDash) + '" transform="rotate(-90 90 90)" opacity=".85"/><circle cx="90" cy="90" r="70" fill="none" stroke="#C4944A" stroke-width="22" stroke-dasharray="' + (dC-gDash) + ' ' + gDash + '" stroke-dashoffset="' + (-gDash) + '" transform="rotate(-90 90 90)" opacity=".85"/><text x="90" y="86" text-anchor="middle" font-size="22" font-weight="800" fill="#1A1A1A" font-family="monospace">' + f$(totMo).replace(" $","") + '</text><text x="90" y="104" text-anchor="middle" font-size="10" fill="#888">/' + t("mois","mo") + '</text></svg>';
  h += '<div style="display:flex;flex-direction:column;gap:16px">';
  h += '<div style="display:flex;align-items:center;gap:10px"><div style="width:14px;height:14px;border-radius:4px;background:#2A8C46;flex-shrink:0"></div><div><div style="font-size:14px;font-weight:700;color:#333">' + t("Prestations gouvernementales","Government benefits") + '</div><div style="font-size:12px;color:#888;font-family:monospace">' + f$(gMo) + '/' + t("mois","mo") + ' — ' + govPct + '%</div></div></div>';
  h += '<div style="display:flex;align-items:center;gap:10px"><div style="width:14px;height:14px;border-radius:4px;background:#C4944A;flex-shrink:0"></div><div><div style="font-size:14px;font-weight:700;color:#333">' + t("Épargne et placements","Savings and investments") + '</div><div style="font-size:12px;color:#888;font-family:monospace">' + f$(sMo) + '/' + t("mois","mo") + ' — ' + (100-govPct) + '%</div></div></div>';
  h += '</div></div>';
  // Benefits detail
  h += card('<div style="padding:10px 0;border-bottom:1px solid var(--bdl)"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:14px;font-weight:600">' + gP + '</div><div style="font-size:11px;color:#999">' + t("Début à ","Starts at age ") + D.qppAge + t(" ans"," years") + '</div></div><span style="font-family:monospace;font-weight:700;font-size:15px">' + f$(D.qppMonthly) + '/' + t("mois","mo") + '</span></div></div><div style="padding:10px 0;border-bottom:1px solid var(--bdl)"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:14px;font-weight:600">' + oN + '</div><div style="font-size:11px;color:#999">' + t("Début à ","Starts at age ") + D.oasAge + t(" ans"," years") + '</div></div><span style="font-family:monospace;font-weight:700;font-size:15px">' + f$(D.oasMonthly) + '/' + t("mois","mo") + '</span></div></div>' + (D.dbPensionMonthly > 0 ? kvr(t("Pension d'employeur","Employer pension"), f$(D.dbPensionMonthly) + '/' + t("mois","mo")) : '') + '<div style="padding:10px 0"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:14px;font-weight:600">' + t("Dépenses mensuelles prévues","Planned monthly spending") + '</div><span style="font-family:monospace;font-weight:700;font-size:15px;color:var(--rd)">' + f$(D.retSpM) + '/' + t("mois","mo") + '</span></div></div>');
  // Gap paragraph
  h += '<div style="font-size:14px;color:#444;line-height:1.8;margin:12px 0">' + (ai.gap_explanation || t("La différence entre vos revenus gouvernementaux (" + f$(gMo) + " $) et vos dépenses (" + f$(D.retSpM) + " $) créerait un écart de " + f$(sMo) + " $ que vos économies personnelles devraient combler. Vos retraits représenteraient " + D.withdrawalRatePct + " % de votre portefeuille annuellement, un taux qui pourrait être " + (D.withdrawalRatePct <= 4 ? "viable" : "élevé") + " selon votre plan de " + (D.deathAge - D.retAge) + " ans.", "The difference between your government income (" + f$(gMo) + ") and spending (" + f$(D.retSpM) + ") could create a " + f$(sMo) + " gap that your personal savings would need to cover. Your withdrawals would represent " + D.withdrawalRatePct + "% of your portfolio annually, a rate that could be " + (D.withdrawalRatePct <= 4 ? "sustainable" : "elevated") + " over your " + (D.deathAge - D.retAge) + "-year plan.")) + '</div>';
  h += co(t("Les prestations gouvernementales pourraient couvrir " + govPct + "% de vos dépenses prévues. Les " + (100-govPct) + "% restants (" + f$(sMo) + " par mois) pourraient être puisés dans votre épargne et vos placements.", "Government benefits could cover " + govPct + "% of your planned spending. The remaining " + (100-govPct) + "% (" + f$(sMo) + " per month) could be drawn from your savings and investments."), govPct >= 50 ? "insight" : "warning");
  h += secEnd();
  ...
  if (txSav > 0) h += co(t(D.taxCurrentEffective + "% → " + D.taxRetirementEffective + "% = ~" + f$(txSav) + "/an d'économies potentielles.", D.taxCurrentEffective + "% → " + D.taxRetirementEffective + "% = ~" + f$(txSav) + "/yr potential savings."), "insight");
  h += secEnd();

  // === LONGEVITY with Fan Chart + Favorable/Watch boxes ===
  sn++;
  h += secH(sn, t("Durée de vie de votre plan","How Long Your Plan Lasts"));
  h += '<div class="rpt-grid3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">' + kp(f$(D.rP5F), t("Scénario pessimiste","Pessimistic scenario"), "var(--rd)", t("Pire 5% des cas","Worst 5% of cases")) + kp(f$(D.rMedF), t("Scénario médian","Median scenario"), "var(--g)", t("Résultat le plus probable","Most likely result")) + kp(f$(D.rP75F), t("Scénario optimiste","Optimistic scenario"), "var(--gn)", t("Meilleur 25% des cas","Best 25% of cases")) + '</div>';
  // Fan chart
  ...
  h += card('<div style="font-size:11px;color:#2A8C46;font-weight:700;text-transform:uppercase;margin-bottom:6px">' + t("Scénario favorable","Favorable scenario") + '</div><div style="font-size:13px;color:#444;line-height:1.75">' + (ai.longevity_good || t("Dans un scénario médian, vous pourriez terminer votre retraite avec " + f$(D.rMedF) + " encore disponibles. Dans les scénarios plus optimistes (25 % des cas), ce montant pourrait même atteindre " + f$(D.rP75F) + ".", "In the median scenario, you could end retirement with " + f$(D.rMedF) + " still available. In more optimistic scenarios (25% of cases), this could reach " + f$(D.rP75F) + ".")) + '</div>', 'border-top:3px solid #2A8C46;padding:16px');
  h += card('<div style="font-size:11px;color:#B89830;font-weight:700;text-transform:uppercase;margin-bottom:6px">' + t("À surveiller","To watch") + '</div><div style="font-size:13px;color:#444;line-height:1.75">' + (ai.longevity_watch || t("Dans les scénarios plus difficiles (5 % des cas), votre épargne pourrait descendre à " + f$(D.rP5F) + " vers la fin de votre plan. Les premières années de retraite avant " + D.qppAge + " ans sont les plus importantes, car elles précèdent vos revenus gouvernementaux.", "In tougher scenarios (5% of cases), your savings could drop to " + f$(D.rP5F) + " near the end of your plan. The first years of retirement before age " + D.qppAge + " are most critical, as they precede government income.")) + '</div>', 'border-top:3px solid #B89830;padding:16px');
  h += '</div>';
  h += secEnd();
  ...
  var observations = [
    ...
    {t2:t("Couverture gouvernementale","Government coverage"), tx:ai.obs_2 || t("Les programmes gouvernementaux couvriraient " + D.coveragePct + " % de vos dépenses mensuelles, laissant un écart de " + f$(D.gapMonthly) + " par mois à financer par vos économies personnelles.","Government programs could cover " + D.coveragePct + "% of your monthly spending, leaving a " + f$(D.gapMonthly) + " per month gap to fund from personal savings."), im:D.coveragePct+"%", c:"var(--gn)", m:t("Plus la part couverte par les prestations est élevée, moins votre plan dépend des rendements boursiers. À " + D.coveragePct + "%, vos placements devraient fournir " + f$(D.gapMonthly) + " par mois.","The more spending covered by benefits, the less your plan depends on market returns. At " + D.coveragePct + "%, your investments would need to provide " + f$(D.gapMonthly) + " per month.")},
    {t2:t("Frais de gestion","Management fees"), tx:ai.obs_3 || t("Les frais de gestion pondérés de " + (D.merWeighted*100).toFixed(2) + " % sur vos placements pourraient représenter environ " + f$(D.feeCostLifetime) + " en coûts cumulatifs sur la durée de votre plan de retraite.","The weighted average management fees of " + (D.merWeighted*100).toFixed(2) + "% on your investments could represent approximately " + f$(D.feeCostLifetime) + " in estimated cumulative costs over your retirement plan."), im:f$(D.feeCostLifetime), c:"var(--bl)", m:t("Les frais sont prélevés chaque année sur la valeur de vos placements. Sur " + planHorizon + " ans, même un pourcentage qui semble faible s'accumule de façon importante grâce aux intérêts composés.","Fees are charged each year on your investment balance. Over " + planHorizon + " years, even a seemingly small percentage compounds significantly.")}
  ];
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    @media(max-width:640px){
      ...
      .rpt-flex{flex-direction:column!important}
      .rpt-donut{flex-direction:column!important;align-items:center!important;gap:16px!important}
      .rpt-tax-label{width:100px!important;font-size:11px!important}
...
<body>
  <div style="font-family:'DM Sans',-apple-system,sans-serif;color:var(--tx);background:var(--bg);max-width:820px;margin:0 auto;padding:32px 28px 56px;font-size:15px;line-height:1.7">
    ${reportBody}
  </div>
</body>
</html>`;
