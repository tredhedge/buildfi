...
export function renderReportHTML360(D, mc, params, lang = "fr", ai = {}, phase = "ACCUM", feedbackToken = "", extraRuns = {}, buildfiData = {}, opts = {}) {
  ...
  // Risk softener (rec #2): contextualize depletion age with guaranteed-income cushion.
  // AMF-safe: conditional tense, observational. Surfaces only when coverage is meaningful.
  const covPct = Math.max(0, Math.min(100, Math.round(n(D.coveragePct, 0))));
  let filet = "";
  if (D.medDepletionAge && covPct >= 30) {
    filet = fr
      ? `Filet: même si le portefeuille s'épuisait vers ${D.medDepletionAge} ans, les revenus garantis (RRQ/PSV) couvriraient environ ${covPct} % des besoins de base.`
      : `Safety net: even if the portfolio were to deplete around age ${D.medDepletionAge}, guaranteed income (CPP/OAS) would cover roughly ${covPct}% of base needs.`;
  } else if (D.p25DepletionAge && covPct >= 30) {
    filet = fr
      ? `Filet: dans la zone prudente (P25), les revenus garantis (RRQ/PSV) couvriraient encore environ ${covPct} % des besoins de base.`
      : `Safety net: in the cautious band (P25), guaranteed income (CPP/OAS) would still cover roughly ${covPct}% of base needs.`;
  } else if (covPct >= 50) {
    filet = fr
  ...
  // Mitigation lever interpretations (rec #3): translate scenario data into actionable framings.
  // AMF-safe: conditional/observational, no prescription.
  const leverNotes = [];
  if (fr) {
    if (D.gapMonthly > 0) leverNotes.push(`Réduire les dépenses cibles d'environ 5 % (~${f$(Math.round(D.retSpM * 0.05), true)}/mois) pourrait alléger l'écart mensuel de ${f$(D.gapMonthly, true)}.`);
    if (D.mc60Succ != null && D.mc70Succ != null) {
      const delta = D.mc70Succ - D.mc60Succ;
      const sign = delta >= 0 ? "+" : "";
      leverNotes.push(`Reporter le RRQ de 60 à 70 ans déplacerait la réussite de ${sign}${delta} pts (${D.mc60Succ}% → ${D.mc70Succ}%), au prix de revenus retardés.`);
    } else if (D.mc65Succ != null && D.mc70Succ != null) {
      const delta = D.mc70Succ - D.mc65Succ;
      const sign = delta >= 0 ? "+" : "";
      leverNotes.push(`Reporter le RRQ de 65 à 70 ans modifierait la réussite de ${sign}${delta} pts (${D.mc65Succ}% → ${D.mc70Succ}%).`);
    }
    if (D.stressCrashSucc != null) leverNotes.push(`Le scénario 2008 (–${Math.max(0, D.successPct - D.stressCrashSucc)} pts vs base) mesure la sensibilité aux premières années de marché.`);
    if (D.withdrawalRatePct >= 4.5) leverNotes.push(`Un taux de retrait initial sous 4 % renforcerait la marge en début de décaissement.`);
  } else {
    if (D.gapMonthly > 0) leverNotes.push(`Trimming target spending by ~5% (~${f$(Math.round(D.retSpM * 0.05), false)}/mo) would shrink the monthly gap of ${f$(D.gapMonthly, false)}.`);
    if (D.mc60Succ != null && D.mc70Succ != null) {
      const delta = D.mc70Succ - D.mc60Succ;
      const sign = delta >= 0 ? "+" : "";
      leverNotes.push(`Deferring CPP from 60 to 70 would shift success by ${sign}${delta} pts (${D.mc60Succ}% → ${D.mc70Succ}%), at the cost of delayed income.`);
    } else if (D.mc65Succ != null && D.mc70Succ != null) {
      const delta = D.mc70Succ - D.mc65Succ;
      const sign = delta >= 0 ? "+" : "";
      leverNotes.push(`Deferring CPP from 65 to 70 would change success by ${sign}${delta} pts (${D.mc65Succ}% → ${D.mc70Succ}%).`);
    }
    if (D.stressCrashSucc != null) leverNotes.push(`The 2008 scenario (–${Math.max(0, D.successPct - D.stressCrashSucc)} pts vs baseline) gauges sensitivity to early-retirement market shocks.`);
    if (D.withdrawalRatePct >= 4.5) leverNotes.push(`Holding the initial withdrawal rate below 4% would widen the margin at the start of decumulation.`);
  }
