...
const REPORT_VERSION_360 = "dash-v2";
...
function gradeColor(g) {
  if (g === "A+" || g === "A") return "#2A8C46";
  if (g === "B+" || g === "B") return "#2F67A3";
  if (g === "C+" || g === "C") return "#D38A14";
  return "#C5372F";
}
...
export function renderReportHTML360(D, mc, params, lang = "fr", ai = {}, phase = "ACCUM", feedbackToken = "", extraRuns = {}, buildfiData = {}) {
  ...
  const html = `<!doctype html><html lang="${fr ? "fr" : "en"}"><head><meta charset="utf-8" /><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${escHtml(title)}</title>
+<style>
+:root{--bg:#F7F4EE;--card:#FFFFFF;--ink:#1E1A14;--muted:#6C6258;--line:#E7DED2;--gold:#C7A13A;--blue:#2F67A3}
+*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:"Avenir Next","Segoe UI",Arial,sans-serif;line-height:1.45}
