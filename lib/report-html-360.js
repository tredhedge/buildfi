...
function buildActions(D, params, fr) {
  ...
  const out = [];
  if (D.gapMonthly > 0) out.push({ title: fr ? "Budget cible" : "Target budget", body: fr ? `Une baisse graduelle de ${f$(Math.round(D.gapMonthly * 0.15), true)}/mois pourrait être envisagée.` : `A gradual reduction of ${f$(Math.round(D.gapMonthly * 0.15), false)}/month could be considered.` });
