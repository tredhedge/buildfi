...
function sectionHeadlines(D, fr) {
  ...
  return {
    ...
    synthesis: fr
      ? `${D.gapMonthly > 0 ? `Un écart mensuel de ${f$(Math.round(D.gapMonthly), true)} subsiste` : `Les dépenses cibles seraient couvertes`}; les revenus garantis représentent ${f$(D.govMonthly, true)}/mois.`
      : `${D.gapMonthly > 0 ? `A ${f$(Math.round(D.gapMonthly), false)}/mo gap remains` : `Target spending would be covered`}; guaranteed income totals ${f$(D.govMonthly, false)}/mo.`,
    levers: fr
...
function realEstateSection360(D, fr) {
  ...
  const cards = ps.map((pr) => {
    let label;
    if (pr.isPrimary) {
      label = pr.name || (fr ? "Résidence" : "Residence");
    } else {
      rIdx++;
...
export function renderReportHTML360(D, mc, params, lang = "fr", ai = {}, phase = "ACCUM", feedbackToken = "", extraRuns = {}, buildfiData = {}, opts = {}) {
  ...
  const fbLever = fr ? D.bestLever
    : (D.gapMonthly > 0 ? `Reducing spending by about ${f$(Math.round(D.gapMonthly * 0.15), false)}/month.`
      : "Government-benefit timing and spending flexibility are the strongest levers.");
  const aiRisk = cleanAISlot(ai?.biggest_risk, 220) || fbRisk || "";
  const aiLever = cleanAISlot(ai?.best_lever, 220) || fbLever || "";
