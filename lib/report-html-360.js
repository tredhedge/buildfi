...
// AMF compliance: this is the static fallback that engages when AI narration
// fails. It must use observational/conditional framing — no imperatives, no
// "plan d'action", no FORBIDDEN_TERMS. Section title is "Leviers identifiés"
// to mirror the Expert renderer (parity, no "plan d'exécution"). Reviewed
// during 2026-05-03 audit.
function buildActions(D, params, fr) {
  const out = [];
  if (D.gapMonthly > 0) out.push({
    title: fr ? "Ajustement potentiel du budget cible" : "Potential target-budget adjustment",
    body: fr
      ? `Une baisse graduelle d'environ ${f$(Math.round(D.gapMonthly * 0.15), true)}/mois pourrait alléger l'écart observé.`
      : `A gradual reduction of about ${f$(Math.round(D.gapMonthly * 0.15), false)}/month could ease the observed gap.`,
  });
  if ((D.phase === "TRANSITION" || D.phase === "DECUM") && (D.mc60Succ != null || D.mc65Succ != null || D.mc70Succ != null)) out.push({
    title: fr ? "Sensibilité au timing RRQ/PSV" : "CPP/OAS timing sensitivity",
    body: fr
      ? `Probabilités observées à 60/65/70 ans: ${D.mc60Succ ?? "—"}% / ${D.mc65Succ ?? "—"}% / ${D.mc70Succ ?? "—"}%.`
      : `Observed probabilities at 60/65/70: ${D.mc60Succ ?? "—"}% / ${D.mc65Succ ?? "—"}% / ${D.mc70Succ ?? "—"}%.`,
  });
  while (out.length < 3) out.push({
    title: fr ? "Revue annuelle des hypothèses" : "Annual review of assumptions",
    body: fr
      ? "Une mise à jour annuelle du bilan permettrait de refléter l'évolution réelle des paramètres."
      : "An annual refresh of the assessment would reflect the actual evolution of inputs.",
  });
...
<section class="card"><h2>${fr ? "Leviers identifiés" : "Identified levers"}</h2><div class="actions">${actions.map((a) => `<article class="action"><h4>${escHtml(a.title)}</h4><p>${escHtml(a.body)}</p></article>`).join("")}</div></section>
