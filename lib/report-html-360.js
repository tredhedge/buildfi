...
export function renderReportHTML360(D, mc, params, lang = "fr", ai = {}, phase = "ACCUM", feedbackToken = "", extraRuns = {}, buildfiData = {}, opts = {}) {
  ...
  const _capToday = D.retBal || ((D.rrsp || 0) + (D.tfsa || 0) + (D.nr || 0)) || 0;
  const _capRet = D.retYearBalance || D.retBal || 0;
  ...
  const _runway = _gapYr > 0 ? Math.round(_capRet / _gapYr * 10) / 10 : null; // null => guaranteed income covers the target
  const FACTS = {
    savings_analysis: fr
      ? `Épargne actuelle d'environ ${_f(_capToday)}; capital médian projeté à la retraite d'environ ${_f(_capRet)}${_runway ? `, soit environ ${_runway} années pour financer la part des dépenses non couverte par les revenus garantis` : ", les revenus garantis couvrant la cible"}.`
      : `Current savings about ${_f(_capToday)}; projected median capital at retirement about ${_f(_capRet)}${_runway ? `, roughly ${_runway} years to fund the portion of spending not covered by guaranteed income` : ", with guaranteed income covering the target"}.`,
    gov_explanation: fr
      ? `Au régime permanent (vers ${D.retSteadyAge || 65} ans), revenu garanti d'environ ${_f(D.retGovMonthly)}/mois (RRQ ${_f(D.retQppPrimaryMonthly)} + PSV ${_f(D.retOasPrimaryMonthly)}${D.retGisMonthly ? ` + SRG ${_f(D.retGisMonthly)}` : ""}${D.retPenMonthly ? ` + rente d'employeur ${_f(D.retPenMonthly)}` : ""}${D.retSpouseGovMonthly ? ` + conjoint ${_f(D.retSpouseGovMonthly)}` : ""}), couvrant environ ${D.retGovCoveragePct || 0}% de la cible de ${_f(_hhTgtMo)}/mois. Avant 65 ans, le revenu gouvernemental est faible ou nul.`
      : `At steady state (around age ${D.retSteadyAge || 65}), guaranteed income about ${_f(D.retGovMonthly)}/mo (CPP ${_f(D.retQppPrimaryMonthly)} + OAS ${_f(D.retOasPrimaryMonthly)}${D.retGisMonthly ? ` + GIS ${_f(D.retGisMonthly)}` : ""}${D.retPenMonthly ? ` + employer pension ${_f(D.retPenMonthly)}` : ""}${D.retSpouseGovMonthly ? ` + spouse ${_f(D.retSpouseGovMonthly)}` : ""}), covering about ${D.retGovCoveragePct || 0}% of the ${_f(_hhTgtMo)}/mo target. Before 65, government income is little or none.`,
