...
const DEBT_STORAGE_KEY = "buildfi_debts_v1_lab";
...
// ── SVG Mini Chart ──
function DebtChart({ timeline, compareTimeline, height = 200, color = DK.accent, id = "main", lang = "fr" }) {
  if (!timeline || timeline.length < 2) return null;
  const series = [timeline, compareTimeline].filter(Boolean);
  const maxVal = Math.max(...series.flatMap(s => s.map(t => t.total)));
  const maxMo = Math.max(...series.flatMap(s => s.map(t => t.month)));
  if (maxVal === 0 || maxMo === 0) return null;

  const W = 100, H = 100;
  const L = 8, R = 4, T = 8, B = 14;
  const cw = W - L - R;
  const ch = H - T - B;
  const gradId = `cg-${id}`;
  const gridTicks = [0, 0.25, 0.5, 0.75, 1];

  const sx = (m) => L + (m / maxMo) * cw;
  const sy = (v) => T + ch - (v / maxVal) * ch;
  const toPts = (arr) => arr.map(t => ({ x: sx(t.month), y: sy(t.total), month: t.month }));
  const toPath = (pts) => pts.map((p, i) => `${i === 0 ? "M" : "L"} ${p.x.toFixed(2)} ${p.y.toFixed(2)}`).join(" ");

  const selectedPts = toPts(timeline);
  const basePts = compareTimeline ? toPts(compareTimeline) : null;
  const selectedPath = toPath(selectedPts);
  const basePath = basePts ? toPath(basePts) : null;
  const areaPath = `${selectedPath} L ${sx(maxMo).toFixed(2)} ${sy(0).toFixed(2)} L ${sx(0).toFixed(2)} ${sy(0).toFixed(2)} Z`;
  const selectedEnd = selectedPts[selectedPts.length - 1];
  const baseEnd = basePts ? basePts[basePts.length - 1] : null;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height, display: "block" }}>
      ...
      <path d={selectedPath} fill="none" stroke={color} strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round" />

      <circle cx={selectedPts[0].x} cy={selectedPts[0].y} r="1.9" fill={color} />
      <circle cx={selectedEnd.x} cy={selectedEnd.y} r="2.4" fill={DK.green} />
      {baseEnd && <circle cx={baseEnd.x} cy={baseEnd.y} r="1.8" fill={DK.txMuted} />}

      <g fontFamily={FONT.sans} fontSize="4.3" fill={DK.txMuted}>
        <text x={L} y={H - 2}>{lang === "fr" ? "0 mois" : "0 mo"}</text>
        <text x={sx(maxMo / 2)} y={H - 2} textAnchor="middle">{Math.round(maxMo / 2)} {lang === "fr" ? "m" : "mo"}</text>
        <text x={W - R} y={H - 2} textAnchor="end">{maxMo} {lang === "fr" ? "m" : "mo"}</text>
      </g>

      <g fontFamily={FONT.sans} fontSize="4.2" fill={DK.txDim}>
        <text x={L} y={4.8}>{lang === "fr" ? "Solde" : "Balance"}</text>
      </g>

      <g fontFamily={FONT.sans} fontSize="4.1">
        <circle cx={W - 30} cy={8} r="1.3" fill={DK.txMuted} />
        <text x={W - 27} y={9.3} fill={DK.txMuted}>{lang === "fr" ? "Sans extra" : "No extra"}</text>
        <circle cx={W - 30} cy={14} r="1.3" fill={color} />
        <text x={W - 27} y={15.3} fill={color}>{lang === "fr" ? "Avec extra" : "With extra"}</text>
      </g>
    </svg>
  );
}
...
const Card = React.forwardRef(({ children, style, ...props }, ref) => (
  <div ref={ref} style={{ background: DK.card, borderRadius: 12, border: `1px solid ${DK.border}`, padding: 16, marginBottom: 12, ...style }} {...props}>{children}</div>
));
...
const SectionTitle = ({ children }) => (
  <div style={{ fontSize: 13, fontWeight: 700, color: DK.accent, borderBottom: `1px solid ${DK.border}`, paddingBottom: 6, marginBottom: 12, marginTop: 18, fontFamily: FONT.sans, letterSpacing: 0.9, textTransform: "uppercase" }}>{children}</div>
);

const InputRow = ({ label, tip, children }) => (
  <div style={{ marginBottom: 12 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
      <span style={{ fontSize: 13, color: DK.txDim, fontWeight: 600 }}>{label}</span>
...
export default function DebtTool() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(DEBT_STORAGE_KEY, JSON.stringify({ debts, mortgages, income, prov, expReturn, coupleOn, lang, spouseIncome, spouseProv, selectedStrategy, extraPay, snowflakeAmt }));
    } catch (e) {}
  ...
  const renderSimulator = () => {
    ...
    return (
      <div>
        ...
        <Card>
          <div style={{ fontSize: 12, color: DK.txDim, lineHeight: 1.55, marginBottom: 12 }}>
            {fr
              ? "Cette vue rend visible l'écart entre votre rythme actuel et un effort mensuel renforcé. On cherche surtout à lire l'effet sur la durée et le coût total, pas à créer une fausse précision."
              : "This view makes the gap between your current pace and a stronger monthly effort visible. The goal is to read the impact on timing and total cost, not to create false precision."}
          </div>
          ...
          {selectedResult.timeline && (
            <div style={{ marginTop: 16, padding: "14px 12px 10px", background: DK.bg, border: `1px solid ${DK.border}`, borderRadius: 10 }}>
              <DebtChart
                timeline={selectedResult.timeline}
                compareTimeline={basePayoff.timeline}
                color={DK.accent}
                lang={lang}
                height={230}
              />
              <div style={{ marginTop: 6, fontSize: 12, color: DK.txDim, lineHeight: 1.5 }}>
                {fr
                  ? "Le tracé pâle montre la trajectoire sans effort supplémentaire. Le tracé doré montre l'effet du paiement additionnel sur la vitesse de désendettement."
                  : "The muted path shows the payoff curve without extra effort. The gold path shows how the added payment shortens the debt runway."}
              </div>
            </div>
          )}
  ...
  // ══════════════════════════════════════════════════════════
  return (
    <div suppressHydrationWarning style={{ minHeight: "100vh", background: `linear-gradient(180deg, ${DK.bg} 0%, ${DK.bg2} 100%)`, color: DK.tx, fontFamily: FONT.sans }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 560px) {
          .mobile-bottom-bar { display: flex; position: fixed; left: 0; right: 0; bottom: 0; z-index: 50; padding: 10px 12px; gap: 10px; background: ${theme === "dark" ? "rgba(21,28,40,.96)" : "rgba(245,246,250,.96)"}; backdrop-filter: blur(10px); border-top: 1px solid ${DK.border}; }
      ...
      {/* Header */}
      <div className="no-print" style={{ borderBottom: `1px solid ${DK.border}`, padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap", background: DK.card, boxShadow: theme === "dark" ? "0 10px 28px rgba(0,0,0,.18)" : "0 10px 28px rgba(24,34,54,.05)" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: DK.accent, letterSpacing: 0.5 }}>BuildFi</span>
            <span style={{ fontSize: 12, color: DK.txDim, fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase" }}>{fr ? "Gestion de dettes" : "Debt Management"}</span>
          </div>
          <div style={{ fontSize: 12, color: DK.txMuted, lineHeight: 1.45 }}>
      ...
      {/* Tab bar */}
      <div className="no-print" style={{ display: "flex", gap: 4, padding: "10px 16px", overflowX: "auto", borderBottom: `1px solid ${DK.border}`, background: DK.card, alignItems: "center" }}>
        {tabs.map((t, idx) => {
          const isActive = activeTab === t.id;
          const hasData = payableDebts.length > 0 || t.id === 0;
          return (
            <React.Fragment key={t.id}>
              {idx === 0 && <span style={{ fontSize: 10, color: DK.txMuted, textTransform: "uppercase", letterSpacing: ".12em", fontWeight: 700, paddingRight: 6 }}>{fr ? "Base" : "Core"}</span>}
              {idx === 4 && <span style={{ fontSize: 10, color: DK.txMuted, textTransform: "uppercase", letterSpacing: ".12em", fontWeight: 700, borderLeft: `1px solid ${DK.border}`, paddingLeft: 10, marginLeft: 6 }}>{fr ? "Analyse" : "Analysis"}</span>}
      ...
      {/* Print-only summary */}
      <div className="print-only" style={{ maxWidth: 960, margin: "0 auto", padding: "28px 20px", fontFamily: FONT.sans }}>
      ...
      {/* Content */}
      <div className="dt-content-wrap" style={{ maxWidth: 960, margin: "0 auto", padding: "22px 20px 36px" }}>
      ...
      {/* Footer disclaimer */}
      <div className="no-print" style={{ borderTop: `1px solid ${DK.border}`, padding: "12px 20px", textAlign: "center", background: DK.card, marginTop: 28 }}>
        <div style={{ fontSize: 12, color: DK.txMuted, maxWidth: 960, margin: "0 auto", lineHeight: 1.5 }}>
          {fr
      ...
      <div className="no-print" style={{ background: DK.s2, borderTop: `1px solid ${DK.border}`, padding: "28px 20px" }}>
